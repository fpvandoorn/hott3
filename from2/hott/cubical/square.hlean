...
namespace eq
  ...
  variables {A B C : Type _} {a a' a'' a₀₀ a₂₀ a₄₀ a₀₂ a₂₂ a₂₄ a₀₄ a₄₂ a₄₄ a₁ a₂ a₃ a₄ : A}
            /-a₀₀-/ {p₁₀ p₁₀' : a₀₀ = a₂₀} /-a₂₀-/ {p₃₀ : a₂₀ = a₄₀} /-a₄₀-/
  ...
  @[hott] def ids      [reducible]         := @square.ids
  @[hott] def idsquare [reducible] (a : A) := @square.ids A a
  ...
  @[hott] def hrfl [reducible] {p : a = a'} : square idp idp p p :=
  !hrefl
  @[hott] def vrfl [reducible] {p : a = a'} : square p p idp idp :=
  !vrefl
  ...
  @[hott] def hdeg_square_idp (p : a = a') : hdeg_square (refl p) = hrfl :=
  by reflexivity

  @[hott] def vdeg_square_idp (p : a = a') : vdeg_square (refl p) = vrfl :=
  by reflexivity
  ...
  @[hott] def eq_of_hdeg_square [reducible] {p q : a = a'} (s : square idp idp p q) : p = q :=
  to_fun !hdeg_square_equiv' s

  @[hott] def eq_of_vdeg_square [reducible] {p q : a = a'} (s : square p q idp idp) : p = q :=
  ...
  @[hott] def loop_pathover {p : a = a'} {q : a = a} {r : a' = a'} (s : square q r p p) : q =[p] r :=
  ...
  -- @[hott] def eq_hconcat {p : a₀₀ = a₀₂} (r : p = p₀₁)
  --   (s₁₁ : square p₁₀ p₁₂ p₀₁ p₂₁) : square p₁₀ p₁₂ p p₂₁ :=
  -- by induction r; exact s₁₁

  -- @[hott] def hconcat_eq {p : a₂₀ = a₂₂}
  ...
  @[hott] def rec_on_b [recursor] {a₀₀ : A}
  ...
  @[hott] def rec_on_r [recursor] {a₀₀ : A}
  ...
  @[hott] def rec_on_l [recursor] {a₀₁ : A}
  ...
  @[hott] def rec_on_t [recursor] {a₁₀ : A}
  ...
  @[hott] def rec_on_tb [recursor] {a : A}
  ...
  @[hott] def rec_on_lr [recursor] {a : A}
  ...
  @[hott] def is_trunc_square [instance] (n : trunc_index) [H : is_trunc n .+2 A]
  ...
  @[hott] def aps_vrfl {A B : Type _} {a a' : A} (f : A → B) (p : a = a') :
    aps f (vrefl p) = vrefl (ap f p) :=
  by induction p; reflexivity

  @[hott] def aps_hrfl {A B : Type _} {a a' : A} (f : A → B) (p : a = a') :
  ...
  @[hott] def natural_square_refl {A B : Type _} {a a' : A} (f : A → B) (q : a = a')
