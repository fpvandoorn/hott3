...
namespace is_trunc
  ...
  -- @[hott] lemma 8.3.1
  @[hott] theorem trivial_homotopy_group_of_is_trunc (A : Type*) {n k : ℕ} [is_trunc n A] (H : n < k)
    : is_contr (π[k] A) :=
  begin
  ...
  @[hott] theorem trivial_ghomotopy_group_of_is_trunc (A : Type*) (n k : ℕ) [is_trunc n A] (H : n ≤ k)
    : is_contr (πg[k+1] A) :=
  trivial_homotopy_group_of_is_trunc A (lt_succ_of_le H)

  -- @[hott] lemma 8.3.2
  @[hott] theorem trivial_homotopy_group_of_is_conn (A : Type*) {k n : ℕ} (H : k ≤ n) [is_conn n A]
    : is_contr (π[k] A) :=
  begin
  ...
  @[hott] theorem homotopy_group_sphere_le (n k : ℕ) (H : k < n) : is_contr (π[k] (S n)) :=
  begin
    cases n with n,
  ...
  @[hott] theorem is_contr_HG_fiber_of_is_connected {A B : Type*} (k n : ℕ) (f : A →* B)
  ...
  -/
  @[hott] theorem is_equiv_π_of_is_connected'.{u} {A B : pType.{u}} {n k : ℕ} (f : A →* B)
  ...
  @[hott] theorem is_equiv_π_of_is_connected.{u v} {A : pType.{u}} {B : pType.{v}} {n k : ℕ} (f : A →* B)
    (H2 : k ≤ n) [H : is_conn_fun n f] : is_equiv (π→[k] f) :=
  begin
  ...
  @[hott] def π_equiv_π_of_is_connected {A B : Type*} {n k : ℕ} (f : A →* B)
  ...
  @[hott] theorem is_surjective_π_of_is_connected.{u} {A B : pType.{u}} (n : ℕ) (f : A →* B)
  ...
  /-
    @[hott] theorem 8.8.3: Whitehead's principle and its corollaries
  -/
  @[hott] def whitehead_principle (n : ℕ₋₂) {A B : Type _}
  ...
  begin
    ...
    begin
      ...
      begin
        intros b p, induction p, apply is_equiv.homotopy_closed, exact this,
  ...
  open pointed.ops
  @[hott] def is_contr_of_trivial_homotopy (n : ℕ₋₂) (A : Type _) [is_trunc n A] [is_conn 0 A]
  ...
  @[hott] def is_contr_of_trivial_homotopy_nat (n : ℕ) (A : Type _) [is_trunc n A] [is_conn 0 A]
    (H : Πk a, k ≤ n → is_contr (π[k] (pointed.MK A a))) : is_contr A :=
  begin
    apply is_contr_of_trivial_homotopy n,
    intros k a, apply @lt_ge_by_cases _ _ n k,
    { intro H', exact trivial_homotopy_group_of_is_trunc _ H'},
    { intro H', exact H k a H'}
  end

  @[hott] def is_contr_of_trivial_homotopy_pointed (n : ℕ₋₂) (A : Type*) [is_trunc n A]
  ...
  @[hott] def is_contr_of_trivial_homotopy_nat_pointed (n : ℕ) (A : Type*) [is_trunc n A]
    (H : Πk, k ≤ n → is_contr (π[k] A)) : is_contr A :=
  begin
    have is_conn 0 A, proof H 0 !zero_le qed,
    fapply is_contr_of_trivial_homotopy_nat n A,
    intros k a H', revert a, apply is_conn.elim -1,
    cases A with A a, exact H k H'
  end

  @[hott] def ab_group_homotopy_group_of_is_conn (n : ℕ) (A : Type*) [H : is_conn 1 A] :
    ab_group (π[n] A) :=
  begin
  ...
  @[hott] def is_contr_of_trivial_homotopy' (n : ℕ₋₂) (A : Type _) [is_trunc n A] [is_conn -1 A]
    (H : Πk a, is_contr (π[k] (pointed.MK A a))) : is_contr A :=
  begin
    assert aa : trunc -1 A,
    { apply center },
  ...
  @[hott] def is_conn_of_trivial_homotopy (n : ℕ₋₂) (m : ℕ) (A : Type _) [is_trunc n A] [is_conn 0 A]
  ...
  @[hott] def is_conn_of_trivial_homotopy_pointed (n : ℕ₋₂) (m : ℕ) (A : Type*) [is_trunc n A]
    (H : Π(k : ℕ), k ≤ m → is_contr (π[k] A)) : is_conn m A :=
  begin
    have is_conn 0 A, proof H 0 !zero_le qed,
    apply is_conn_of_trivial_homotopy n m A,
    intros k a H2, revert a, apply is_conn.elim -1,
    cases A with A a, exact H k H2
  end

  @[hott] def is_conn_fun_of_equiv_on_homotopy_groups.{u} (n : ℕ) {A B : Type u} (f : A → B)
  ...
  begin
    ...
    { intros k H, apply is_trunc_equiv_closed_rev, exact homotopy_group_ptrunc_of_le H _,
...
namespace is_conn
  ...
  @[hott] def is_conn_fun_inf_of_equiv_on_homotopy_groups.{u} {A B : Type u} (f : A → B)
    [is_equiv (trunc_functor 0 f)]
  ...
  begin
    ...
    { intros a k H, exact H1 a k},
    { intro a, apply is_surjective_of_is_equiv}
  end

  @[hott] def is_equiv_trunc_functor_of_is_conn_fun_inf.{u} (n : ℕ₋₂) {A B : Type u} (f : A → B)
    [is_conn_fun_inf f] : is_equiv (trunc_functor n f) :=
  _

  @[hott] def is_equiv_homotopy_group_functor_of_is_conn_fun_inf.{u} {A B : pType.{u}} (f : A →* B)
