...
@[hott] def family : Type _ := ΣX, X → Type _
...
namespace cellcomplex
  ...
  /-
    ...
    and the realization map fdcc n → Type _
  ...
  @[hott] def fdcc_family [reducible] : ℕ → family :=
  nat.rec
  ...
  @[hott] def fdcc (n : ℕ) : Type _ := pr1 (fdcc_family n)

  @[hott] def cell : Πn, fdcc n → Set :=
