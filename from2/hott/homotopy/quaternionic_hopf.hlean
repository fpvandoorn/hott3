...
namespace hopf
  ...
  @[hott] def circle_mul_neg (x y : S¹) : x * (-y) = - x * y :=
  by rewrite [circle_neg_id,circle_neg_id]

  @[hott] def circle_star_eq (x : S¹) : x* = circle_star x :=
  begin
    fapply (rec2_on x),
    { reflexivity },
  ...
  @[hott] def circle_norm (x : S¹) : x * x* = 1 :=
  begin
  ...
  @[hott] def circle_star_mul (x y : S¹) : (x * y)* = y* * x* :=
  begin
    induction x,
  ...
  @[hott] def imaginaroid_sphere_zero [instance]
    : imaginaroid (S 0) :=
  ...
  @[hott] def sphere_three_h_space [instance] : h_space (S 3) :=
  @h_space_equiv_closed (join S¹ S¹)
      (cd_h_space (S 0) circle_assoc) (S 3) (join_sphere 1 1)

  @[hott] def is_conn_sphere_three : is_conn 0 (S 3) :=
  begin
  ...
  @[hott] def quaternionic_hopf' : S 7 → S 4 :=
  begin
