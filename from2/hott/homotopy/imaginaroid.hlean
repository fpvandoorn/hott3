...
structure has_star [class] (A : Type _) :=
(star : A → A)

reserve postfix `*` : (max+1)
postfix `*` := has_star.star

structure involutive_neg [class] (A : Type _) extends has_neg A :=
...
section
  ...
  @[hott] theorem neg_neg (a : A) : - -a = a := !involutive_neg.neg_neg
end
...
section
  ...
  @[hott] def has_one_susp [instance] : has_one (susp A) :=
  ⦃ has_one, one := north ⦄

  variable [H : has_neg A]
  include H

  @[hott] def susp_neg : susp A → susp A :=
  susp.elim south north (λa, (merid (neg a))⁻¹)

  @[hott] def has_neg_susp [instance] : has_neg (susp A) :=
  ⦃ has_neg, neg := susp_neg⦄

  @[hott] def susp_star : susp A → susp A :=
  susp.elim north south (λa, merid (neg a))

  @[hott] def has_star_susp [instance] : has_star (susp A) :=
...
section
  ...
  @[hott] def susp_neg_neg (x : susp A) : - - x = x :=
  begin
    induction x with a,
    { reflexivity },
    { reflexivity },
  ...
  @[hott] def involutive_neg_susp [instance] : involutive_neg (susp A) :=
  ⦃ involutive_neg, neg_neg := susp_neg_neg ⦄

  @[hott] def susp_star_star (x : susp A) : x** = x :=
  begin
    induction x with a,
    { reflexivity },
    { reflexivity },
  ...
  @[hott] def susp_neg_star (x : susp A) : (-x)* = -x* :=
  begin
    induction x with a,
    { reflexivity },
    { reflexivity },
...
structure imaginaroid [class] (A : Type _)
...
section
  ...
  @[hott] theorem one_mul (x : susp A) : 1 * x = x := !imaginaroid.one_mul
  @[hott] theorem mul_one (x : susp A) : x * 1 = x := !imaginaroid.mul_one
  @[hott] theorem mul_neg (x y : susp A) : x * -y = -x * y := !imaginaroid.mul_neg
  ...
  @[hott] def imaginaroid_h_space : h_space (susp A) :=
  ⦃ h_space, one := one, mul := mul, one_mul := one_mul, mul_one := mul_one ⦄

  @[hott] theorem norm (x : susp A) : x * x* = 1 := !imaginaroid.norm
  @[hott] theorem star_mul (x y : susp A) : (x * y)* = y* * x* := !imaginaroid.star_mul
  @[hott] theorem one_star : 1* = 1 :> susp A := idp

  @[hott] theorem neg_mul (x y : susp A) : (-x) * y = -x * y :=
  calc
  ...
  @[hott] theorem norm' (x : susp A) : x* * x = 1 :=
  calc
...
section
  ...
  section lemmata
    ...
    @[hott] def lemma_1 : f (-1) = a * c :=
    calc
      a * c * (- -1) = a * c * 1  : idp
                 ... = a * c      : mul_one

    @[hott] def lemma_2 : f (c* * a* * d * b*) = - d * b* :=
    calc
      a * c * (-c* * a* * d * b*)
        = a * (-c * c* * a* * d * b*)     : mul_neg
    ... = -a * c * c* * a* * d * b*       : mul_neg
    ...
    @[hott] def lemma_3 : g 1 = c * b :=
    calc
      c * 1 * b = c * b : one_mul

    @[hott] def lemma_4 : g (c* * a* * d * b*) = a* * d :=
    calc
      c * (c* * a* * d * b*) * b
        = (c * c* * a* * d * b*) * b      : assoc
    ... = ((c * c*) * a* * d * b*) * b    : assoc
  ...
  -/
  @[hott] def cd_mul (x y : join (susp A) (susp A)) : join (susp A) (susp A) :=
  begin
  ...
  @[hott] def cd_one_mul (x : join (susp A) (susp A)) : cd_mul (inl 1) x = x :=
  begin
    induction x with a b a b,
  ...
  @[hott] def cd_mul_one (x : join (susp A) (susp A)) : cd_mul x (inl 1) = x :=
  begin
    induction x with a b a b,
  ...
  @[hott] def cd_h_space [instance] : h_space (join (susp A) (susp A)) :=
