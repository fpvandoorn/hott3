...
namespace hopf
  ...
  @[hott] def circle_h_space [instance] : h_space S¹ :=
  ⦃ h_space, one := base, mul := circle_mul,
    one_mul := circle_base_mul, mul_one := circle_mul_base ⦄

  @[hott] def circle_assoc (x y z : S¹) : (x * y) * z = x * (y * z) :=
  begin
    induction x,
    { reflexivity },
  ...
  @[hott] def pfiber_complex_hopf : pfiber complex_hopf ≃* S 1 :=
  begin
