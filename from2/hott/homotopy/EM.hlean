...
namespace EM
  ...
  variables {G : Group}
  @[hott] def EM1' (G : Group) : Type _ :=
  groupoid_quotient (Groupoid_of_Group G)
  @[hott] def EM1 (G : Group) : Type* :=
  pointed.MK (EM1' G) (elt star)

  @[hott] def base : EM1' G := elt star
  @[hott] def pth : G → base = base := pth
  @[hott] def resp_mul (g h : G) : pth (g * h) = pth g ⬝ pth h := resp_comp h g
  @[hott] def resp_one : pth (1 : G) = idp :=
  resp_id star

  @[hott] def resp_inv (g : G) : pth (g⁻¹) = (pth g)⁻¹ :=
  resp_inv g
  ...
  @[hott] def rec_pth {P : EM1' G → Type _} [H : Π(x : EM1' G), is_trunc 1 (P x)]
    {Pb : P base} {Pp : Π(g : G), Pb =[pth g] Pb}
  ...
  @[hott] theorem elim_set_pth {Pb : Set} {Pp : Π(g : G), Pb ≃ Pb}
...
attribute EM.rec_on EM.elim_on
attribute EM.set_rec EM.set_elim
attribute EM.prop_rec EM.prop_elim EM.elim_set

namespace EM
  open groupoid_quotient

  variables (G : Group)
  @[hott] def base_eq_base_equiv : (base = base :> EM1 G) ≃ G :=
  !elt_eq_elt_equiv

  @[hott] def fundamental_group_EM1 : π₁ (EM1 G) ≃g G :=
  begin
  ...
  @[hott] proposition is_trunc_EM1 [instance] : is_trunc 1 (EM1 G) :=
  !is_trunc_groupoid_quotient

  @[hott] proposition is_trunc_EM1' [instance] : is_trunc 1 (EM1' G) :=
  !is_trunc_groupoid_quotient

  @[hott] proposition is_conn_EM1' [instance] : is_conn 0 (EM1' G) :=
  ...
  variable {G}
  @[hott] def EM1_map {X : Type*} (e : G → Ω X)
  ...
  @[hott] def EM1_pmap {X : Type*} (e : G → Ω X)
    (r : Πg h, e (g * h) = e g ⬝ e h) [is_conn 0 X] [is_trunc 1 X] : EM1 G →* X :=
  pmap.mk (EM1_map e r) idp

  variable (G)
  @[hott] def loop_EM1 : G ≃* Ω (EM1 G) :=
  (pequiv_of_equiv (base_eq_base_equiv G) idp)⁻¹ᵉ*

  variable {G}
  @[hott] def loop_EM1_pmap {X : Type*} (e : G →* Ω X)
  ...
  @[hott] def EM1_pequiv'.{u} {G : Group.{u}} {X : pType.{u}} (e : G ≃* Ω X)
    (r : Πg h, e (g * h) = e g ⬝ e h) [is_conn 0 X] [is_trunc 1 X] : EM1 G ≃* X :=
  begin
  ...
  @[hott] def EM1_pequiv.{u} {G : Group.{u}} {X : pType.{u}} (e : G ≃g π₁ X)
  ...
  @[hott] def EM1_pequiv_type (X : Type*) [is_conn 0 X] [is_trunc 1 X] : EM1 (π₁ X) ≃* X :=
...
namespace EM
  ...
  @[hott] def EM1_mul (x x' : EM1' G) : EM1' G :=
  begin
    induction x,
    { exact x'},
  ...
  variable (G)
  @[hott] def EM1_mul_one (x : EM1' G) : EM1_mul x base = x :=
  begin
    induction x using EM.set_rec,
    { reflexivity},
  ...
  @[hott] def h_space_EM1 [instance] : h_space (EM1' G) :=
  begin
    fapply h_space.mk,
    { exact EM1_mul},
    { exact base},
  ...
  /- K(G, n+1) -/
  @[hott] def EMadd1 : ℕ → Type*
  | 0 := EM1 G
  | (n+1) := ptrunc (n+2) (susp (EMadd1 n))

  @[hott] def EMadd1_succ (n : ℕ) :
    EMadd1 G (succ n) = ptrunc (n.+2) (susp (EMadd1 G n)) :=
  idp

  @[hott] def loop_EM2 : Ω[1] (EMadd1 G 1) ≃* EM1 G :=
  hopf.delooping (EM1' G) idp

  @[hott] def is_conn_EMadd1 [instance] (n : ℕ) : is_conn n (EMadd1 G n) :=
  begin
  ...
  @[hott] def is_trunc_EMadd1 [instance] (n : ℕ) : is_trunc (n+1) (EMadd1 G n) :=
  begin
    cases n with n,
    { apply is_trunc_EM1 },
  ...
  @[hott] def loop_EMadd1 (n : ℕ) : EMadd1 G n ≃* Ω (EMadd1 G (succ n))  :=
  begin
    cases n with n,
    { exact !loop_EM2⁻¹ᵉ* },
  ...
  @[hott] def loopn_EMadd1_pequiv_EM1 (G : AbGroup) (n : ℕ) : EM1 G ≃* Ω[n] (EMadd1 G n) :=
  begin
  ...
  @[hott] def loopn_EMadd1 (G : AbGroup) (n : ℕ) : G ≃* Ω[succ n] (EMadd1 G n) :=
  begin
    induction n with n e,
    { apply loop_EM1 },
    { refine _ ⬝e* !loopn_succ_in⁻¹ᵉ*,
  ...
  @[hott] def loopn_EMadd1_succ (G : AbGroup) (n : ℕ) : loopn_EMadd1 G (succ n) ~*
    !loopn_succ_in⁻¹ᵉ* ∘* apn (succ n) !loop_EMadd1 ∘* loopn_EMadd1 G n :=
  by reflexivity

  @[hott] def EM_up {G : AbGroup} {X : Type*} {n : ℕ} (e : Ω[succ (succ n)] X ≃* G)
    : Ω[succ n] (Ω X) ≃* G :=
  !loopn_succ_in⁻¹ᵉ* ⬝e* e

  @[hott] def is_homomorphism_EM_up {G : AbGroup} {X : Type*} {n : ℕ}
    (e : Ω[succ (succ n)] X ≃* G)
  ...
  @[hott] def EMadd1_pmap_succ {G : AbGroup} {X : Type*} (n : ℕ) (e : Ω[succ (succ n)] X ≃* G)
  ...
  @[hott] def loop_EMadd1_pmap {G : AbGroup} {X : Type*} {n : ℕ} (e : Ω[succ (succ n)] X ≃* G)
    (r : Πp q, e (p ⬝ q) = e p * e q)
  ...
  @[hott] def loopn_EMadd1_pmap' {G : AbGroup} {X : Type*} {n : ℕ} (e : Ω[succ n] X ≃* G)
    (r : Πp q, e (p ⬝ q) = e p * e q)
    [H1 : is_conn n X] [H2 : is_trunc (n.+1) X] :
    Ω→[succ n](EMadd1_pmap n e r) ∘* loopn_EMadd1 G n ~* e⁻¹ᵉ* :=
  begin
    revert X e r H1 H2, induction n with n IH: intros X e r H1 H2,
    { apply loop_EM1_pmap },
  ...
  @[hott] def EMadd1_pequiv' {G : AbGroup} {X : Type*} (n : ℕ) (e : Ω[succ n] X ≃* G)
  ...
  @[hott] def EMadd1_pequiv {G : AbGroup} {X : Type*} (n : ℕ) (e : πg[n+1] X ≃g G)
    [H1 : is_conn n X] [H2 : is_trunc (n.+1) X] : EMadd1 G n ≃* X :=
  begin
    have is_set (Ω[succ n] X), from !is_set_loopn,
    apply EMadd1_pequiv' n ((ptrunc_pequiv _ _)⁻¹ᵉ* ⬝e* pequiv_of_isomorphism e),
    intros p q, esimp, exact to_respect_mul e (tr p) (tr q)
  end

  @[hott] def EMadd1_pequiv_succ {G : AbGroup} {X : Type*} (n : ℕ) (e : πag[n+2] X ≃g G)
    [H1 : is_conn (n.+1) X] [H2 : is_trunc (n.+2) X] : EMadd1 G (succ n) ≃* X :=
  EMadd1_pequiv (succ n) e

  @[hott] def ghomotopy_group_EMadd1 (n : ℕ) : πg[n+1] (EMadd1 G n) ≃g G :=
  begin
  ...
  @[hott] def EMadd1_pequiv_type (X : Type*) (n : ℕ) [is_conn (n+1) X] [is_trunc (n+1+1) X]
    : EMadd1 (πag[n+2] X) (succ n) ≃* X :=
  EMadd1_pequiv_succ n !isomorphism.refl

  /- K(G, n) -/
  @[hott] def EM (G : AbGroup) : ℕ → Type*
  | 0     := G
  | (k+1) := EMadd1 G k
  ...
  @[hott] def homotopy_group_EM (n : ℕ) : π[n] (EM G n) ≃* G :=
  begin
    cases n with n,
  ...
  @[hott] def ghomotopy_group_EM (n : ℕ) : πg[n+1] (EM G (n+1)) ≃g G :=
  ghomotopy_group_EMadd1 G n

  @[hott] def is_conn_EM [instance] (n : ℕ) : is_conn (n.-1) (EM G n) :=
  begin
    cases n with n,
  ...
  @[hott] def is_conn_EM_succ [instance] (n : ℕ) : is_conn n (EM G (succ n)) :=
  is_conn_EM G (succ n)

  @[hott] def is_trunc_EM [instance] (n : ℕ) : is_trunc n (EM G n) :=
  begin
    cases n with n,
  ...
  @[hott] def loop_EM (n : ℕ) : Ω (K G (succ n)) ≃* K G n :=
  begin
    cases n with n,
  ...
  open circle int
  @[hott] def EM_pequiv_circle : K agℤ 1 ≃* S¹* :=
  EM1_pequiv fundamental_group_of_circle⁻¹ᵍ

  /- Functorial action of Eilenberg-Maclane spaces -/

  @[hott] def EM1_functor {G H : Group} (φ : G →g H) : EM1 G →* EM1 H :=
  begin
    fapply pmap.mk,
    { intro g, induction g,
      { exact base },
  ...
  @[hott] def EMadd1_functor {G H : AbGroup} (φ : G →g H) (n : ℕ) :
  ...
  @[hott] def EM_functor {G H : AbGroup} (φ : G →g H) (n : ℕ) :
    K G n →* K H n :=
  begin
    cases n with n,
  ...
  @[hott] def ptruncconntype10_pequiv (X Y : 1-Type*[0]) (e : π₁ X ≃g π₁ Y) : X ≃* Y :=
  (EM1_pequiv !isomorphism.refl)⁻¹ᵉ* ⬝e* EM1_pequiv e

  @[hott] def EM1_pequiv_ptruncconntype10 (X : 1-Type*[0]) : EM1 (π₁ X) ≃* X :=
  EM1_pequiv_type X

  @[hott] def Group_equiv_ptruncconntype10 : Group ≃ 1-Type*[0] :=
  ...
  open trunc_index
  @[hott] def ptruncconntype_pequiv : Π(n : ℕ) (X Y : (n.+1)-Type*[n])
  ...
  @[hott] def EM1_pequiv_ptruncconntype (n : ℕ) (X : (n+1+1)-Type*[n+1]) :
    EMadd1 (πag[n+2] X) (succ n) ≃* X :=
  EMadd1_pequiv_type X n

  @[hott] def AbGroup_equiv_ptruncconntype' (n : ℕ) :
