...
structure h_space [class] (A : Type _) extends has_mul A, has_one A :=
...
section
  ...
  @[hott] def one_mul (a : A) : 1 * a = a := !h_space.one_mul
  @[hott] def mul_one (a : A) : a * 1 = a := !h_space.mul_one

  @[hott] def h_space_equiv_closed {B : Type _} (f : A ≃ B) : h_space B :=
  ...
  @[hott] def is_equiv_mul_left [instance] : Π(a : A), is_equiv (λx, a * x) :=
  begin
  ...
  @[hott] def is_equiv_mul_right [instance]  : Π(a : A), is_equiv (λx, x * a) :=
  begin
    apply is_conn_fun.elim -1 (is_conn_fun_from_unit -1 A 1)
...
/- If A is a K(G,1), then A is deloopable.
   Main @[hott] lemma of Licata-Finster. -/
section
  parameters (A : Type _) [T : is_trunc 1 A] [K : is_conn 0 A] [H : h_space A]
             (coh : one_mul 1 = mul_one 1 :> (1 * 1 = 1 :> A))
  @[hott] def P [reducible] : susp A → Type _ :=
  λx, trunc 1 (north = x)

  include K H T

  local abbreviation codes [reducible] : susp A → Type _ := hopf A

  @[hott] def transport_codes_merid (a a' : A)
    : transport codes (merid a) a' = a * a' :> A :=
  ap10 (elim_type_merid _ _ _ a) a'

  @[hott] def is_trunc_codes [instance] (x : susp A) : is_trunc 1 (codes x) :=
  begin
    induction x with a, do 2 apply T, apply is_prop.elimo
  end

  @[hott] def encode₀ {x : susp A} : north = x → codes x :=
  λp, transport codes p (by change A; exact 1)

  @[hott] def encode {x : susp A} : P x → codes x :=
  λp, trunc.elim encode₀ p

  @[hott] def decode' : A → P (@north A) :=
  λa, tr (merid a ⬝ (merid 1)⁻¹)

  @[hott] def transport_codes_merid_one_inv (a : A)
    : transport codes (merid 1)⁻¹ a = a :=
  ap10 (elim_type_merid_inv _ _ _ 1) a ⬝ begin apply to_inv_eq_of_eq, esimp, refine !one_mul⁻¹ end

  @[hott] proposition encode_decode' (a : A) : encode (decode' a) = a :=
  begin
  ...
  @[hott] proposition homomorphism : Πa a' : A,
  ...
  begin
    ...
    { apply ap02 tr, esimp, fapply concat2,
      ...
      { assert e : Π(X : Type _)(x y : X)(p : x = y),
  ...
  @[hott] def decode {x : susp A} : codes x → P x :=
  begin
    induction x,
    { exact decode' },
    { exact (λa, tr (merid a)) },
  ...
  @[hott] def decode_encode {x : susp A} : Πt : P x, decode (encode t) = t :=
  begin
    apply trunc.rec, intro p, cases p, apply ap tr, apply con.right_inv
  end

  /-
     We define main_@[hott] lemma by first defining its inverse, because normally equiv.MK changes
  ...
  -/
  @[hott] def main_@[hott] lemma : trunc 1 (north = north :> susp A) ≃ A :=
  (equiv.MK decode' encode decode_encode encode_decode')⁻¹ᵉ

  @[hott] def main_lemma_point
  ...
  @[hott] def to_pmap_main_lemma_point_pinv
  ...
  @[hott] def to_pmap_delooping_pinv :
  ...
  @[hott] def hopf_delooping_elim {B : Type*} (f : pointed.MK A 1 →* Ω B) [H2 : is_trunc 2 B] :
