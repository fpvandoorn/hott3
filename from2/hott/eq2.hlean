...
namespace eq
  variables {A B C : Type _} {f : A → B} {a a' a₁ a₂ a₃ a₄ : A} {b b' : B}
...
--    induction q, rewrite [↑ap_compose,ap_inv], apply hinverse, apply ap_con_right_inv_sq,
  ...
  @[hott] theorem ap_compose_inv {A B C : Type _} (g : B → C) (f : A → B)
    {x y : A} (p : x = y) :
  ...
  @[hott] theorem ap_compose_con (g : B → C) (f : A → B) (p : a₁ = a₂) (q : a₂ = a₃) :
  ...
  begin
    induction p, esimp,
    generalizes [H a, g a], intros ga Ha, induction Ha,
    reflexivity
  end
  ...
  @[hott] def hsquare [reducible] (f₁₀ : A₀₀ → A₂₀) (f₁₂ : A₀₂ → A₂₂)
                                 (f₀₁ : A₀₀ → A₀₂) (f₂₁ : A₂₀ → A₂₂) : Type _ :=
  f₂₁ ∘ f₁₀ ~ f₁₂ ∘ f₀₁
