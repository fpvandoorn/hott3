...
namespace choice
  ...
  @[hott] def AC [reducible] := Π (X : Type u) (A : X -> Type u) (P : Π x, A x -> Type u),
  ...
  @[hott] def AC_cart [reducible] := Π (X : Type u) (A : X -> Type u),
  ...
  @[hott] def AC_equiv_AC' : AC.{u} ≃ AC'.{u} :=
  equiv_of_is_prop
  ...
  @[hott] def AC_cart_of_AC' : AC'.{u} -> AC_cart.{u} :=
  ...
  @[hott] def AC'_of_AC_cart : AC_cart.{u} -> AC'.{u} :=
  by intros H X A P HX HA HP HI;
  apply H X (λ x, Σ a, P x a) HX (λ x, !is_trunc_sigma) HI

  -- Which is enough to show AC' ≃ AC_cart, since both are props.
  @[hott] def AC'_equiv_AC_cart : AC'.{u} ≃ AC_cart.{u} :=
  equiv_of_is_prop AC_cart_of_AC'.{u} AC'_of_AC_cart.{u}

  -- 3.8.2. AC ≃ AC_cart follows by transitivity.
  @[hott] def AC_equiv_AC_cart : AC.{u} ≃ AC_cart.{u} :=
  equiv.trans AC_equiv_AC' AC'_equiv_AC_cart

  namespace example385
  @[hott] def X : Type 1 := Σ A : Type 0, ∥ A = bool ∥

  @[hott] def x0 : X := ⟨bool, merely.intro _ rfl⟩

  @[hott] def Y : X -> Type 1 := λ x, x0 = x

  @[hott] def not_is_set_X : ¬ is_set X :=
  begin
  ...
  @[hott] def is_set_x1 (x : X) : is_set x.1 :=
  by cases x; induction a_1; cases a_1; exact _

  @[hott] def is_set_Yx (x : X) : is_set (Y x) :=
  begin
  ...
  @[hott] def X_must_be_set : Σ (X : Type 1) (Y : X -> Type 1)
