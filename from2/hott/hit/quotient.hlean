...
namespace quotient
  ...
  @[hott] theorem elim_type_eq_of_rel.{u} (Pc : A → Type u)
...
attribute quotient.elim_type
attribute quotient.elim_on
attribute quotient.elim_type_on
...
namespace quotient
  ...
  section
    ...
    begin
      ...
      { intros v x, induction v with i p, induction x,
          exact (i a),
          exact (p H)},
  ...
  /- the flattening @[hott] lemma -/

  namespace flattening
  section

    parameters {A : Type _} (R : A → A → Type _) (C : A → Type _) (f : Π⦃a a'⦄, R a a' → C a ≃ C a')
    include f
    variables {a a' : A} {r : R a a'}

    local abbreviation P := quotient.elim_type C f

    @[hott] def flattening_type : Type _ := Σa, C a
    local abbreviation X := flattening_type
    inductive flattening_rel : X → X → Type _ :=
    ...
    open flattening_rel
    @[hott] def flattening_@[hott] lemma : sigma P ≃ quotient flattening_rel :=
    begin
      fapply equiv.MK,
      { refine elim _ _,
