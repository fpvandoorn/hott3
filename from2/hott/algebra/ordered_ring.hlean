...
variable {A : Type _}
namespace algebra
private @[hott] def absurd_a_lt_a {B : Type _} {a : A} [s : strict_order A] (H : a < a) : B :=
absurd H (lt.irrefl a)

/- semiring structures -/

structure ordered_semiring (A : Type _)
...
@[hott] def add_ab_group_of_ordered_semiring [trans_instance] [reducible] (A : Type _)
  [H : ordered_semiring A] : semiring A :=
@ordered_semiring.to_semiring A H

@[hott] def monoid_of_ordered_semiring [trans_instance] [reducible] (A : Type _)
...
section
  ...
  @[hott] theorem mul_le_mul_of_nonneg_left {a b c : A} (Hab : a ≤ b) (Hc : 0 ≤ c) :
    c * a ≤ c * b := !ordered_semiring.mul_le_mul_of_nonneg_left Hab Hc

  @[hott] theorem mul_le_mul_of_nonneg_right {a b c : A} (Hab : a ≤ b) (Hc : 0 ≤ c) :
    a * c ≤ b * c := !ordered_semiring.mul_le_mul_of_nonneg_right Hab Hc

  -- TODO: there are four variations, depending on which variables we assume to be nonneg
  @[hott] theorem mul_le_mul {a b c d : A} (Hac : a ≤ c) (Hbd : b ≤ d) (nn_b : 0 ≤ b) (nn_c : 0 ≤ c) :
    a * b ≤ c * d :=
  calc
  ...
  @[hott] theorem mul_lt_mul_of_pos_left {a b c : A} (Hab : a < b) (Hc : 0 < c) :
    c * a < c * b := !ordered_semiring.mul_lt_mul_of_pos_left Hab Hc

  @[hott] theorem mul_lt_mul_of_pos_right {a b c : A} (Hab : a < b) (Hc : 0 < c) :
    a * c < b * c := !ordered_semiring.mul_lt_mul_of_pos_right Hab Hc

  -- TODO: once again, there are variations
  @[hott] theorem mul_lt_mul {a b c d : A} (Hac : a < c) (Hbd : b ≤ d) (pos_b : 0 < b) (nn_c : 0 ≤ c) :
    a * b < c * d :=
  calc
    a * b < c * b : mul_lt_mul_of_pos_right Hac pos_b
      ... ≤ c * d : mul_le_mul_of_nonneg_left Hbd nn_c

  @[hott] theorem mul_pos {a b : A} (Ha : a > 0) (Hb : b > 0) : a * b > 0 :=
  begin
  ...
  @[hott] theorem mul_neg_of_pos_of_neg {a b : A} (Ha : a > 0) (Hb : b < 0) : a * b < 0 :=
  begin
  ...
  @[hott] theorem mul_neg_of_neg_of_pos {a b : A} (Ha : a < 0) (Hb : b > 0) : a * b < 0 :=
  begin
...
structure linear_ordered_semiring [class] (A : Type _)
...
structure decidable_linear_ordered_semiring [class] (A : Type _)
  extends linear_ordered_semiring A, decidable_linear_order A

/- ring structures -/

structure ordered_ring (A : Type _) extends ring A, ordered_mul_ab_group A renaming
...
@[hott] def add_ab_group_of_ordered_ring [reducible] [trans_instance] (A : Type _)
  [H : ordered_ring A] : ring A :=
@ordered_ring.to_ring A H

@[hott] def monoid_of_ordered_ring [reducible] [trans_instance] (A : Type _)
  [H : ordered_ring A] : ordered_ab_group A :=
@ordered_ring.to_ordered_mul_ab_group A H

@[hott] def zero_ne_one_class_of_ordered_ring [reducible] [trans_instance] (A : Type _)
  [H : ordered_ring A] : zero_ne_one_class A :=
@ordered_ring.to_zero_ne_one_class A H

@[hott] theorem ordered_ring.mul_le_mul_of_nonneg_left [s : ordered_ring A] {a b c : A}
...
@[hott] theorem ordered_ring.mul_le_mul_of_nonneg_right [s : ordered_ring A] {a b c : A}
...
@[hott] theorem ordered_ring.mul_lt_mul_of_pos_left [s : ordered_ring A] {a b c : A}
       (Hab : a < b) (Hc : 0 < c) : c * a < c * b :=
...
@[hott] theorem ordered_ring.mul_lt_mul_of_pos_right [s : ordered_ring A] {a b c : A}
       (Hab : a < b) (Hc : 0 < c) : a * c < b * c :=
have H1 : 0 < b - a, from iff.elim_right !sub_pos_iff_lt Hab,
...
@[hott] def ordered_ring.to_ordered_semiring [reducible] [trans_instance] [s : ordered_ring A] :
  ordered_semiring A :=
⦃ ordered_semiring, s,
  mul_zero                   := mul_zero,
  zero_mul                   := zero_mul,
...
section
  ...
  @[hott] theorem mul_lt_mul_of_neg_left (H : b < a) (Hc : c < 0) : c * a < c * b :=
  ...
  @[hott] theorem mul_lt_mul_of_neg_right (H : b < a) (Hc : c < 0) : a * c < b * c :=
  have Hc' : -c > 0, from iff.mpr !neg_pos_iff_neg Hc,
  ...
  @[hott] theorem mul_pos_of_neg_of_neg (Ha : a < 0) (Hb : b < 0) : 0 < a * b :=
  begin
...
-- class instance
structure linear_ordered_ring [class] (A : Type _)
    extends ordered_ring A, linear_strong_order_pair A :=
  (zero_lt_one : lt zero one)

@[hott] def linear_ordered_ring.to_linear_ordered_semiring [reducible] [trans_instance]
...
structure linear_ordered_comm_ring [class] (A : Type _) extends linear_ordered_ring A, comm_monoid A

@[hott] theorem linear_ordered_comm_ring.eq_zero_sum_eq_zero_of_mul_eq_zero [s : linear_ordered_comm_ring A]
...
@[hott] def linear_ordered_comm_ring.to_integral_domain [reducible] [trans_instance]
...
section
  ...
  @[hott] theorem zero_le_one : 0 ≤ (1:A) := one_mul 1 ▸ mul_self_nonneg 1
  ...
  @[hott] theorem le_of_mul_le_of_ge_one {a b c : A} (H : a * c ≤ b) (Hb : b ≥ 0) (Hc : c ≥ 1) : a ≤ b :=
  ...
  @[hott] theorem nonneg_le_nonneg_of_squares_le {a b : A} (Ha : a ≥ 0) (Hb : b ≥ 0) (H : a * a ≤ b * b) :
      a ≤ b :=
    begin
      apply le_of_not_gt,
      intro Hab,
...
structure decidable_linear_ordered_comm_ring [class] (A : Type _) extends linear_ordered_comm_ring A,
...
@[hott] def linear_ordered_comm_ring_of_decidable_linear_ordered_comm_ring [reducible]
  [trans_instance] (A : Type _) [H : decidable_linear_ordered_comm_ring A] :
  linear_ordered_comm_ring A :=
@decidable_linear_ordered_comm_ring.to_linear_ordered_comm_ring A H

@[hott] def decidable_linear_ordered_ab_group_of_decidable_linear_ordered_comm_ring [reducible]
  [trans_instance] (A : Type _) [H : decidable_linear_ordered_comm_ring A] :
