...
namespace algebra
structure ordered_mul_cancel_comm_monoid [class] (A : Type _) extends comm_monoid A,
...
@[hott] def ordered_cancel_comm_monoid [class] : Type _ → Type _ := ordered_mul_cancel_comm_monoid

@[hott] def add_comm_monoid_of_ordered_cancel_comm_monoid [reducible] [trans_instance]
  (A : Type _) [H : ordered_cancel_comm_monoid A] : add_comm_monoid A :=
@ordered_mul_cancel_comm_monoid.to_comm_monoid A H

@[hott] def add_left_cancel_semigroup_of_ordered_cancel_comm_monoid [reducible] [trans_instance]
...
@[hott] def add_right_cancel_semigroup_of_ordered_cancel_comm_monoid [reducible] [trans_instance]
...
@[hott] def order_pair_of_ordered_cancel_comm_monoid [reducible] [trans_instance]
...
section
  ...
  @[hott] theorem add_lt_add_left (H : a < b) (c : A) : c + a < c + b :=
  @ordered_mul_cancel_comm_monoid.mul_lt_mul_left A s a b H c

  @[hott] theorem add_lt_add_right (H : a < b) (c : A) : a + c < b + c :=
  begin
    rewrite [add.comm, {b + _}add.comm],
    exact (add_lt_add_left H c)
  end

  @[hott] theorem add_le_add_left (H : a ≤ b) (c : A) : c + a ≤ c + b :=
  @ordered_mul_cancel_comm_monoid.mul_le_mul_left A s a b H c

  @[hott] theorem add_le_add_right (H : a ≤ b) (c : A) : a + c ≤ b + c :=
  ...
  @[hott] theorem lt_add_of_pos_right (H : b > 0) : a < a + b := !add_zero ▸ add_lt_add_left H a

  @[hott] theorem lt_add_of_pos_left (H : b > 0) : a < b + a := !zero_add ▸ add_lt_add_right H a

  -- here we start using le_of_add_le_add_left.
  @[hott] theorem le_of_add_le_add_left (H : a + b ≤ a + c) : b ≤ c :=
  @ordered_mul_cancel_comm_monoid.le_of_mul_le_mul_left A s a b c H

  @[hott] theorem le_of_add_le_add_right (H : a + b ≤ c + b) : a ≤ c :=
  le_of_add_le_add_left (show b + a ≤ b + c, begin rewrite [add.comm, {b + _}add.comm], exact H end)

  @[hott] theorem lt_of_add_lt_add_left (H : a + b < a + c) : b < c :=
  @ordered_mul_cancel_comm_monoid.lt_of_mul_lt_mul_left A s a b c H

  @[hott] theorem lt_of_add_lt_add_right (H : a + b < c + b) : a < c :=
  lt_of_add_lt_add_left ((add.comm a b) ▸ (add.comm c b) ▸ H)

  @[hott] theorem add_le_add_left_iff (a b c : A) : a + b ≤ a + c ↔ b ≤ c :=
  iff.intro le_of_add_le_add_left (assume H, add_le_add_left H _)

  @[hott] theorem add_le_add_right_iff (a b c : A) : a + b ≤ c + b ↔ a ≤ c :=
  iff.intro le_of_add_le_add_right (assume H, add_le_add_right H _)

  @[hott] theorem add_lt_add_left_iff (a b c : A) : a + b < a + c ↔ b < c :=
  iff.intro lt_of_add_lt_add_left (assume H, add_lt_add_left H _)

  @[hott] theorem add_lt_add_right_iff (a b c : A) : a + b < c + b ↔ a < c :=
...
structure ordered_mul_ab_group [class] (A : Type _) extends ab_group A, order_pair A :=
(mul_le_mul_left : Πa b, le a b → Πc, le (mul c a) (mul c b))
(mul_lt_mul_left : Πa b, lt a b → Π c, lt (mul c a) (mul c b))

@[hott] def ordered_ab_group [class] : Type _ → Type _ := ordered_mul_ab_group

@[hott] def add_ab_group_of_ordered_ab_group [reducible] [trans_instance] (A : Type _)
  [H : ordered_ab_group A] : add_ab_group A :=
@ordered_mul_ab_group.to_ab_group A H

@[hott] theorem ordered_mul_ab_group.le_of_mul_le_mul_left [s : ordered_mul_ab_group A] {a b c : A}
  (H : a * b ≤ a * c) : b ≤ c :=
have H' : a⁻¹ * (a * b) ≤ a⁻¹ * (a * c), from ordered_mul_ab_group.mul_le_mul_left _ _ H _,
by rewrite *inv_mul_cancel_left at H'; exact H'

@[hott] theorem ordered_mul_ab_group.lt_of_mul_lt_mul_left [s : ordered_mul_ab_group A] {a b c : A}
  (H : a * b < a * c) : b < c :=
have H' : a⁻¹ * (a * b) < a⁻¹ * (a * c), from ordered_mul_ab_group.mul_lt_mul_left _ _ H _,
by rewrite *inv_mul_cancel_left at H'; exact H'

@[hott] def ordered_mul_ab_group.to_ordered_mul_cancel_comm_monoid [reducible] [trans_instance]
...
@[hott] def ordered_ab_group.to_ordered_cancel_comm_monoid [reducible] [trans_instance]
...
section
  ...
  @[hott] theorem neg_of_neg_pos {a : A} (H : 0 < -a) : a < 0 :=
  lt_of_neg_lt_neg (neg_zero⁻¹ ▸ H)

  @[hott] theorem neg_pos_of_neg {a : A} (H : a < 0) : 0 < -a :=
  neg_zero ▸ neg_lt_neg H

  @[hott] theorem neg_pos_iff_neg : 0 < -a ↔ a < 0 :=
  iff.intro neg_of_neg_pos neg_pos_of_neg

  @[hott] theorem le_neg_iff_le_neg : a ≤ -b ↔ b ≤ -a := !neg_neg ▸ !neg_le_neg_iff_le

  @[hott] theorem le_neg_of_le_neg {a b : A} : a ≤ -b → b ≤ -a := iff.mp !le_neg_iff_le_neg

  @[hott] theorem neg_le_iff_neg_le : -a ≤ b ↔ -b ≤ a := !neg_neg ▸ !neg_le_neg_iff_le

  @[hott] theorem neg_le_of_neg_le {a b : A} : -a ≤ b → -b ≤ a := iff.mp !neg_le_iff_neg_le

  @[hott] theorem lt_neg_iff_lt_neg : a < -b ↔ b < -a := !neg_neg ▸ !neg_lt_neg_iff_lt

  @[hott] theorem lt_neg_of_lt_neg {a b : A} : a < -b → b < -a := iff.mp !lt_neg_iff_lt_neg

  @[hott] theorem neg_lt_iff_neg_lt : -a < b ↔ -b < a := !neg_neg ▸ !neg_lt_neg_iff_lt

  @[hott] theorem neg_lt_of_neg_lt {a b : A} : -a < b → -b < a := iff.mp !neg_lt_iff_neg_lt

  @[hott] theorem sub_nonneg_iff_le : 0 ≤ a - b ↔ b ≤ a := !sub_self ▸ !add_le_add_right_iff

  @[hott] theorem sub_nonneg_of_le {a b : A} : b ≤ a → 0 ≤ a - b := iff.mpr !sub_nonneg_iff_le

  @[hott] theorem le_of_sub_nonneg {a b : A} : 0 ≤ a - b → b ≤ a := iff.mp !sub_nonneg_iff_le

  @[hott] theorem sub_nonpos_iff_le : a - b ≤ 0 ↔ a ≤ b := !sub_self ▸ !add_le_add_right_iff

  @[hott] theorem sub_nonpos_of_le {a b : A} : a ≤ b → a - b ≤ 0 := iff.mpr !sub_nonpos_iff_le

  @[hott] theorem le_of_sub_nonpos {a b : A} : a - b ≤ 0 → a ≤ b := iff.mp !sub_nonpos_iff_le

  @[hott] theorem sub_pos_iff_lt : 0 < a - b ↔ b < a := !sub_self ▸ !add_lt_add_right_iff

  @[hott] theorem sub_pos_of_lt {a b : A} : b < a → 0 < a - b := iff.mpr !sub_pos_iff_lt

  @[hott] theorem lt_of_sub_pos {a b : A} : 0 < a - b → b < a := iff.mp !sub_pos_iff_lt

  @[hott] theorem sub_neg_iff_lt : a - b < 0 ↔ a < b := !sub_self ▸ !add_lt_add_right_iff

  @[hott] theorem sub_neg_of_lt {a b : A} : a < b → a - b < 0 := iff.mpr !sub_neg_iff_lt

  @[hott] theorem lt_of_sub_neg {a b : A} : a - b < 0 → a < b := iff.mp !sub_neg_iff_lt
  ...
  @[hott] theorem sub_le_sub_right {a b : A} (H : a ≤ b) (c : A) : a - c ≤ b - c := add_le_add_right H (-c)
  ...
  @[hott] theorem sub_lt_sub_right {a b : A} (H : a < b) (c : A) : a - c < b - c := add_lt_add_right H (-c)
...
structure decidable_linear_ordered_mul_ab_group [class] (A : Type _)
...
@[hott] def decidable_linear_ordered_ab_group [class] : Type _ → Type _ :=
decidable_linear_ordered_mul_ab_group

@[hott] def add_ab_group_of_decidable_linear_ordered_ab_group [reducible] [trans_instance] (A : Type _)
  [H : decidable_linear_ordered_ab_group A] : add_ab_group A :=
@decidable_linear_ordered_mul_ab_group.to_ab_group A H

@[hott] def decidable_linear_order_of_decidable_linear_ordered_ab_group [reducible]
  [trans_instance] (A : Type _) [H : decidable_linear_ordered_ab_group A] :
  decidable_linear_order A :=
@decidable_linear_ordered_mul_ab_group.to_decidable_linear_order A H

@[hott] def decidable_linear_ordered_mul_ab_group.to_ordered_mul_ab_group [reducible]
  [trans_instance] (A : Type _) [s : decidable_linear_ordered_mul_ab_group A] :
...
@[hott] def decidable_linear_ordered_ab_group.to_ordered_ab_group [reducible] [trans_instance]
...
section
  ...
  @[hott] theorem abs_abs (a : A) : abs (abs a) = abs a := abs_of_nonneg !abs_nonneg
  ...
  section
    private @[hott] lemma aux1 {a b : A} (H1 : a + b ≥ 0) (H2 : a ≥ 0) : abs (a + b) ≤ abs a + abs b :=
    decidable.by_cases
      (assume H3 : b ≥ 0,
          calc
    ...
    private @[hott] lemma aux2 {a b : A} (H1 : a + b ≥ 0) : abs (a + b) ≤ abs a + abs b :=
