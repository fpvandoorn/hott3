...
structure is_reflexive [class] {T : Type _} (R : T → T → Type _) := (refl : reflexive R)
structure is_symmetric [class] {T : Type _} (R : T → T → Type _) := (symm : symmetric R)
structure is_transitive [class] {T : Type _} (R : T → T → Type _) := (trans : transitive R)

structure is_equivalence [class] {T : Type _} (R : T → T → Type _)
extends is_reflexive R, is_symmetric R, is_transitive R

-- partial equivalence relation
structure is_PER {T : Type _} (R : T → T → Type _) extends is_symmetric R, is_transitive R
...
@[hott] def congruence_const [instance] {T2 : Type _} (R2 : T2 → T2 → Type _)
    [C : is_reflexive R2] ⦃T1 : Type _⦄ (R1 : T1 → T1 → Type _) (c : T2) :
  is_congruence R1 R2 (λu : T1, c) :=
is_congruence.const R2 (is_reflexive.refl R2) R1 c

@[hott] def congruence_star [instance] {T : Type _} (R : T → T → Type _) :
...
structure mp_like [class] (R : Type _ → Type _ → Type _) :=
