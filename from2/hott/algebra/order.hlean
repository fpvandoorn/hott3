...
namespace algebra
structure weak_order [class] (A : Type _) extends has_le A :=
(le_refl : Πa, le a a)
...
section
  ...
  @[hott] def le.refl [refl] (a : A) : a ≤ a := !weak_order.le_refl

  @[hott] def le_of_eq {a b : A} (H : a = b) : a ≤ b := H ▸ le.refl a

  @[hott] def le.trans [trans] {a b c : A} : a ≤ b → b ≤ c → a ≤ c := !weak_order.le_trans

  @[hott] def ge.trans [trans] {a b c : A} (H1 : a ≥ b) (H2: b ≥ c) : a ≥ c := le.trans H2 H1

  @[hott] def le.antisymm {a b : A} : a ≤ b → b ≤ a → a = b := !weak_order.le_antisymm

  -- Alternate syntax. (Abbreviations do not migrate well.)
  @[hott] def eq_of_le_of_ge {a b : A} : a ≤ b → b ≤ a → a = b := !le.antisymm
end

structure linear_weak_order [class] (A : Type _) extends weak_order A :=
(le_total : Πa b, le a b ⊎ le b a)

section
  variables [linear_weak_order A]

  @[hott] theorem le.total (a b : A) : a ≤ b ⊎ b ≤ a := !linear_weak_order.le_total

  @[hott] theorem le_of_not_ge {a b : A} (H : ¬ a ≥ b) : a ≤ b := sum.resolve_left !le.total H
...
structure strict_order [class] (A : Type _) extends has_lt A :=
(lt_irrefl : Πa, ¬ lt a a)
...
section
  ...
  @[hott] def lt.irrefl (a : A) : ¬ a < a := !strict_order.lt_irrefl
  @[hott] def not_lt_self (a : A) : ¬ a < a := !lt.irrefl   -- alternate syntax

  @[hott] theorem lt_self_iff_empty (a : A) : a < a ↔ empty :=
  iff_empty_intro (lt.irrefl a)

  @[hott] def lt.trans [trans] {a b c : A} : a < b → b < c → a < c := !strict_order.lt_trans

  @[hott] def gt.trans [trans] {a b c : A} (H1 : a > b) (H2: b > c) : a > c := lt.trans H2 H1

  @[hott] theorem ne_of_lt {a b : A} (lt_ab : a < b) : a ≠ b :=
  assume eq_ab : a = b,
...
structure wf_strict_order [class] (A : Type _) extends strict_order A :=
...
structure order_pair [class] (A : Type _) extends weak_order A, has_lt A :=
...
section
  ...
  @[hott] def le_of_lt : a < b → a ≤ b := !order_pair.le_of_lt

  @[hott] def lt_of_lt_of_le [trans] : a < b → b ≤ c → a < c := !order_pair.lt_of_lt_of_le

  @[hott] def lt_of_le_of_lt [trans] : a ≤ b → b < c → a < c := !order_pair.lt_of_le_of_lt

  private @[hott] def lt_irrefl (s' : order_pair A) (a : A) : ¬ a < a := !order_pair.lt_irrefl

  private @[hott] theorem lt_trans (s' : order_pair A) (a b c: A) (lt_ab : a < b) (lt_bc : b < c) : a < c :=
    lt_of_lt_of_le lt_ab (le_of_lt lt_bc)

  @[hott] def order_pair.to_strict_order [trans_instance] : strict_order A :=
  ⦃ strict_order, s, lt_irrefl := lt_irrefl s, lt_trans := lt_trans s ⦄

  @[hott] def gt_of_gt_of_ge [trans] (H1 : a > b) (H2 : b ≥ c) : a > c := lt_of_le_of_lt H2 H1

  @[hott] def gt_of_ge_of_gt [trans] (H1 : a ≥ b) (H2 : b > c) : a > c := lt_of_lt_of_le H2 H1

  @[hott] theorem not_le_of_gt (H : a > b) : ¬ a ≤ b :=
  assume H1 : a ≤ b,
...
structure strong_order_pair [class] (A : Type _) extends weak_order A, has_lt A :=
...
@[hott] theorem lt_sum_eq_of_le [s : strong_order_pair A] {a b : A} (le_ab : a ≤ b) : a < b ⊎ a = b :=
iff.mp le_iff_lt_sum_eq le_ab

@[hott] theorem le_of_lt_sum_eq [s : strong_order_pair A] {a b : A} (lt_sum_eq : a < b ⊎ a = b) : a ≤ b :=
iff.mpr le_iff_lt_sum_eq lt_sum_eq

private @[hott] def lt_irrefl' [s : strong_order_pair A] (a : A) : ¬ a < a :=
!strong_order_pair.lt_irrefl

private @[hott] theorem le_of_lt' [s : strong_order_pair A] (a b : A) : a < b → a ≤ b :=
take Hlt, le_of_lt_sum_eq (sum.inl Hlt)

private @[hott] theorem lt_iff_le_prod_ne [s : strong_order_pair A] {a b : A} : a < b ↔ (a ≤ b × a ≠ b) :=
iff.intro
...
private @[hott] theorem ne_of_lt' [s : strong_order_pair A] {a b : A} (H : a < b) : a ≠ b :=
prod.pr2 ((iff.mp (@lt_iff_le_prod_ne _ _ _ _)) H)

private @[hott] theorem lt_of_lt_of_le' [s : strong_order_pair A] (a b c : A) : a < b → b ≤ c → a < c :=
assume lt_ab : a < b,
assume le_bc : b ≤ c,
...
@[hott] theorem lt_of_le_of_lt' [s : strong_order_pair A] (a b c : A) : a ≤ b → b < c → a < c :=
assume le_ab : a ≤ b,
assume lt_bc : b < c,
...
@[hott] def strong_order_pair.to_order_pair [trans_instance] [s : strong_order_pair A] : order_pair A :=
⦃ order_pair, s,
  lt_irrefl := lt_irrefl',
  le_of_lt := le_of_lt',
...
structure linear_order_pair [class] (A : Type _) extends order_pair A, linear_weak_order A

structure linear_strong_order_pair [class] (A : Type _) extends strong_order_pair A,
    linear_weak_order A

@[hott] def linear_strong_order_pair.to_linear_order_pair [trans_instance]
...
section
  ...
  @[hott] theorem lt.trichotomy : a < b ⊎ a = b ⊎ b < a :=
  ...
  @[hott] theorem le_of_not_gt {a b : A} (H : ¬ a > b) : a ≤ b :=
...
structure decidable_linear_order [class] (A : Type _) extends linear_strong_order_pair A :=
...
section
  ...
  @[hott] def decidable_lt [instance] : decidable (a < b) :=
    @decidable_linear_order.decidable_lt _ _ _ _

  @[hott] def decidable_le [instance] : decidable (a ≤ b) :=
  by_cases
  ...
  @[hott] def has_decidable_eq [instance] : decidable (a = b) :=
  by_cases
    (assume H : a ≤ b,
      by_cases
