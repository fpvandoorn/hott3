...
structure distrib [class] (A : Type _) extends has_mul A, has_add A :=
(left_distrib : Πa b c, mul a (add b c) = add (mul a b) (mul a c))
(right_distrib : Πa b c, mul (add a b) c = add (mul a c) (mul b c))

@[hott] theorem left_distrib [s : distrib A] (a b c : A) : a * (b + c) = a * b + a * c :=
!distrib.left_distrib

@[hott] theorem right_distrib [s: distrib A] (a b c : A) : (a + b) * c = a * c + b * c :=
!distrib.right_distrib

structure mul_zero_class [class] (A : Type _) extends has_mul A, has_zero A :=
(zero_mul : Πa, mul zero a = zero)
(mul_zero : Πa, mul a zero = zero)

@[hott] theorem zero_mul [s : mul_zero_class A] (a : A) : 0 * a = 0 := !mul_zero_class.zero_mul
@[hott] theorem mul_zero [s : mul_zero_class A] (a : A) : a * 0 = 0 := !mul_zero_class.mul_zero

structure zero_ne_one_class [class] (A : Type _) extends has_zero A, has_one A :=
(zero_ne_one : zero ≠ one)

@[hott] theorem zero_ne_one [s: zero_ne_one_class A] : 0 ≠ (1:A) := @zero_ne_one_class.zero_ne_one A s

/- semiring -/
structure semiring (A : Type _) extends comm_monoid A renaming
...
@[hott] def add_comm_monoid_of_semiring [reducible] [trans_instance] (A : Type _)
  [H : semiring A] : add_comm_monoid A :=
@semiring.to_comm_monoid A H

@[hott] def monoid_of_semiring [reducible] [trans_instance] (A : Type _)
  [H : semiring A] : monoid A :=
@semiring.to_monoid A H

@[hott] def distrib_of_semiring [reducible] [trans_instance] (A : Type _)
  [H : semiring A] : distrib A :=
@semiring.to_distrib A H

@[hott] def mul_zero_class_of_semiring [reducible] [trans_instance] (A : Type _)
...
structure comm_semiring [class] (A : Type _) extends semiring A, comm_monoid A
...
section comm_semiring
  ...
  @[hott] theorem dvd.refl : a ∣ a := dvd.intro !mul_one
  ...
  @[hott] theorem dvd_zero : a ∣ 0 := dvd.intro !mul_zero

  @[hott] theorem one_dvd : 1 ∣ a := dvd.intro !one_mul

  @[hott] theorem dvd_mul_right : a ∣ a * b := dvd.intro rfl

  @[hott] theorem dvd_mul_left : a ∣ b * a := mul.comm a b ▸ dvd_mul_right a b
...
structure ring (A : Type _) extends ab_group A renaming mul→add mul_assoc→add_assoc
...
@[hott] def add_ab_group_of_ring [reducible] [trans_instance] (A : Type _)
  [H : ring A] : add_ab_group A :=
@ring.to_ab_group A H

@[hott] def monoid_of_ring [reducible] [trans_instance] (A : Type _)
  [H : ring A] : monoid A :=
@ring.to_monoid A H

@[hott] def distrib_of_ring [reducible] [trans_instance] (A : Type _)
  [H : ring A] : distrib A :=
@ring.to_distrib A H

@[hott] theorem ring.mul_zero [s : ring A] (a : A) : a * 0 = 0 :=
...
@[hott] theorem ring.zero_mul [s : ring A] (a : A) : 0 * a = 0 :=
...
@[hott] def ring.to_semiring [reducible] [trans_instance] [s : ring A] : semiring A :=
⦃ semiring, s,
...
section
  ...
  @[hott] theorem neg_mul_eq_neg_mul : -(a * b) = -a * b :=
  neg_eq_of_add_eq_zero
    begin
      rewrite [-right_distrib, add.right_inv, zero_mul]
    end

  @[hott] theorem neg_mul_eq_mul_neg : -(a * b) = a * -b :=
   neg_eq_of_add_eq_zero
     begin
       rewrite [-left_distrib, add.right_inv, mul_zero]
     end

  @[hott] theorem neg_mul_eq_neg_mul_symm : - a * b = - (a * b) := inverse !neg_mul_eq_neg_mul
  @[hott] theorem mul_neg_eq_neg_mul_symm : a * - b = - (a * b) := inverse !neg_mul_eq_mul_neg
  ...
  @[hott] theorem neg_mul_comm : -a * b = a * -b := !neg_mul_eq_neg_mul⁻¹ ⬝ !neg_mul_eq_mul_neg

  @[hott] theorem neg_eq_neg_one_mul : -a = -1 * a :=
  calc
    -a = -(1 * a)  : by rewrite one_mul
      ... = -1 * a : by rewrite neg_mul_eq_neg_mul

  @[hott] theorem mul_sub_left_distrib : a * (b - c) = a * b - a * c :=
  calc
    a * (b - c) = a * b + a * -c : left_distrib
      ... = a * b + - (a * c)    : by rewrite -neg_mul_eq_mul_neg
      ... = a * b - a * c        : rfl

  @[hott] theorem mul_sub_right_distrib : (a - b) * c = a * c - b * c :=
  calc
  ...
  @[hott] theorem mul_add_eq_mul_add_iff_sub_mul_add_eq : a * e + c = b * e + d ↔ (a - b) * e + c = d :=
  calc
  ...
  @[hott] theorem mul_add_eq_mul_add_of_sub_mul_add_eq : (a - b) * e + c = d → a * e + c = b * e + d :=
    iff.mpr !mul_add_eq_mul_add_iff_sub_mul_add_eq

  @[hott] theorem sub_mul_add_eq_of_mul_add_eq_mul_add : a * e + c = b * e + d → (a - b) * e + c = d :=
...
structure comm_ring [class] (A : Type _) extends ring A, comm_semigroup A

@[hott] def comm_ring.to_comm_semiring [reducible] [trans_instance] [s : comm_ring A] :
  comm_semiring A :=
⦃ comm_semiring, s,
  mul_zero := mul_zero,
  zero_mul := zero_mul ⦄
...
structure no_zero_divisors [class] (A : Type _) extends has_mul A, has_zero A :=
...
structure integral_domain [class] (A : Type _) extends comm_ring A, no_zero_divisors A,
    zero_ne_one_class A
