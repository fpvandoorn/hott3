...
namespace yoneda
  ...
  @[hott] def contravariant_yoneda_embedding [reducible]
  ...
  -/
  @[hott] def yoneda_embedding (C : Precategory.{u v}) : C ⇒ cset ^c Cᵒᵖ :=
--(functor_curry_rev !hom_functor)
  ...
  @[hott] def yoneda_lemma_hom (c : C) (F : Cᵒᵖ ⇒ cset)
    (x : trunctype.carrier (F c)) : ɏ c ⟹ F :=
  begin
    fapply nat_trans.mk,
    { exact yoneda_lemma_hom_fun c F x},
    { intros c₁ c₂ f, exact yoneda_lemma_hom_nat c F x f}
  end

  @[hott] def yoneda_lemma_equiv (c : C)
  ...
  @[hott] def yoneda_@[hott] lemma (c : C) (F : Cᵒᵖ ⇒ cset) :
    homset (ɏ c) F ≅ functor_lift (F c) :=
  begin
    apply iso_of_equiv, esimp, apply yoneda_lemma_equiv,
  end

  @[hott] theorem yoneda_lemma_natural_ob (F : Cᵒᵖ ⇒ cset) {c c' : C} (f : c' ⟶ c)
    (η : ɏ c ⟹ F) :
     to_fun_hom (functor_lift ∘f F) f (to_hom (yoneda_@[hott] lemma c F) η) =
     to_hom (yoneda_@[hott] lemma c' F) (η ∘n to_fun_hom ɏ f) :=
  begin
  ...
  -- attribute yoneda_@[hott] lemma functor_lift Precategory_Set precategory_Set homset
  --   yoneda_embedding nat_trans.compose functor_nat_trans_compose [reducible]
  -- attribute tlift functor.compose [reducible]
  @[hott] theorem yoneda_lemma_natural_functor (c : C) (F F' : Cᵒᵖ ⇒ cset)
    (θ : F ⟹ F') (η : to_fun_ob ɏ c ⟹ F) :
     (functor_lift.{v u} ∘fn θ) c (to_hom (yoneda_@[hott] lemma c F) η) =
     proof to_hom (yoneda_@[hott] lemma c F') (θ ∘n η) qed :=
  by reflexivity

  -- @[hott] theorem xx.{u v} {C : Precategory.{u v}} (c : C) (F F' : Cᵒᵖ ⇒ set)
  --   (θ : F ⟹ F') (η : to_fun_ob ɏ c ⟹ F) :
  --    proof _ qed =
  --    to_hom (yoneda_@[hott] lemma c F') (θ ∘n η) :=
  -- by reflexivity

  -- @[hott] theorem yy.{u v} {C : Precategory.{u v}} (c : C) (F F' : Cᵒᵖ ⇒ set)
  --   (θ : F ⟹ F') (η : to_fun_ob ɏ c ⟹ F) :
  --    (functor_lift.{v u} ∘fn θ) c (to_hom (yoneda_@[hott] lemma c F) η) =
  --    proof _ qed :=
  -- by reflexivity
  ...
  open equiv
  @[hott] def fully_faithful_yoneda_embedding [instance] (C : Precategory.{u v}) :
    fully_faithful (ɏ : C ⇒ cset ^c Cᵒᵖ) :=
  begin
    intros c c',
    fapply is_equiv_of_equiv_of_homotopy,
    { symmetry, transitivity _, apply @equiv_of_iso (homset _ _),
      exact @yoneda_@[hott] lemma C c (ɏ c'), esimp [yoneda_embedding], exact !equiv_lift⁻¹ᵉ},
  ...
  @[hott] def is_embedding_yoneda_embedding (C : Category.{u v}) :
    is_embedding (ɏ : C → Cᵒᵖ ⇒ cset) :=
  begin
    intros c c', fapply is_equiv_of_equiv_of_homotopy,
  ...
  @[hott] def is_representable (F : Cᵒᵖ ⇒ cset) := Σ(c : C), ɏ c ≅ F

  section
    set_option apply.class_instance false
    open functor.ops
    @[hott] def is_prop_representable {C : Category.{u v}} (F : Cᵒᵖ ⇒ cset)
