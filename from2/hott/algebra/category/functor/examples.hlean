...
@[hott] def of functors involving at least two different constructions of categories
-/
...
namespace functor
  ...
  /- currying a functor -/
  @[hott] def functor_curry_ob [reducible] (c : C) : D ⇒ E :=
  F ∘f (constant_functor D c ×f 1)

  @[hott] def functor_curry_hom ⦃c c' : C⦄ (f : c ⟶ c')
    : functor_curry_ob F c ⟹ functor_curry_ob F c' :=
  F ∘fn (constant_nat_trans D f ×n 1)

  local abbreviation Fhom := @functor_curry_hom

  @[hott] theorem functor_curry_id (c : C) : Fhom F (ID c) = 1 :=
  nat_trans_eq (λd, respect_id F (c, d))

  @[hott] theorem functor_curry_comp ⦃c c' c'' : C⦄ (f' : c' ⟶ c'') (f : c ⟶ c')
  ...
  @[hott] def functor_curry : C ⇒ E ^c D :=
  ...
  @[hott] def functor_curry_rev_ob [reducible] (d : D) : C ⇒ E :=
  F ∘f (1 ×f constant_functor C d)

  @[hott] def functor_curry_rev_hom ⦃d d' : D⦄ (g : d ⟶ d')
  ...
  @[hott] theorem functor_curry_rev_id (d : D) : Fhomr F (ID d) = nat_trans.id :=
  nat_trans_eq (λc, respect_id F (c, d))

  @[hott] theorem functor_curry_rev_comp ⦃d d' d'' : D⦄ (g' : d' ⟶ d'') (g : d ⟶ d')
    : Fhomr F (g' ∘ g) = Fhomr F g' ∘n Fhomr F g :=
  begin
    apply nat_trans_eq, esimp, intro c, rewrite [-id_id at {1}], apply respect_comp F
  end

  @[hott] def functor_curry_rev : D ⇒ E ^c C :=
  ...
  @[hott] def functor_uncurry_ob [reducible] (p : C ×c D) : E :=
  to_fun_ob (G p.1) p.2

  @[hott] def functor_uncurry_hom ⦃p p' : C ×c D⦄ (f : hom p p')
  ...
  @[hott] theorem functor_uncurry_id (p : C ×c D) : Ghom G (ID p) = id :=
  calc
  ...
  @[hott] theorem functor_uncurry_comp ⦃p p' p'' : C ×c D⦄ (f' : p' ⟶ p'') (f : p ⟶ p')
  ...
  @[hott] def functor_uncurry : C ×c D ⇒ E :=
  functor.mk (functor_uncurry_ob G)
             (functor_uncurry_hom G)
             (functor_uncurry_id G)
             (functor_uncurry_comp G)

  @[hott] def functor_uncurry_functor_curry : functor_uncurry (functor_curry F) = F :=
  ...
  @[hott] def functor_curry_functor_uncurry_ob (c : C)
    : functor_curry (functor_uncurry G) c = G c :=
  begin
    fapply functor_eq,
     { intro d, reflexivity},
     { intros d d' g, refine !id_leftright ⬝ _, esimp,
       rewrite [▸*, ↑functor_uncurry_hom, respect_id, ▸*, id_right]}
  end

  @[hott] def functor_curry_functor_uncurry : functor_curry (functor_uncurry G) = G :=
  begin
  ...
  -/
  @[hott] def prod_functor_equiv_functor_functor (C D E : Precategory)
    : (C ×c D ⇒ E) ≃ (C ⇒ E ^c D) :=
  ...
  variables {F F' G G'}
  @[hott] def nat_trans_curry_nat (η : F ⟹ F') (c : C)
    : functor_curry_ob F c ⟹ functor_curry_ob F' c :=
  begin
    fapply nat_trans.mk: esimp,
    { intro d, exact η (c, d)},
    { intros d d' f, apply naturality}
  end

  @[hott] def nat_trans_curry (η : F ⟹ F')
    : functor_curry F ⟹ functor_curry F' :=
  begin
    fapply nat_trans.mk: esimp,
    { exact nat_trans_curry_nat η},
    { intros c c' f, apply nat_trans_eq, intro d, esimp, apply naturality}
  end

  @[hott] def nat_trans_uncurry (η : G ⟹ G')
    : functor_uncurry G ⟹ functor_uncurry G' :=
  begin
    fapply nat_trans.mk: esimp,
    { intro v, unfold functor_uncurry_ob, exact (η v.1) v.2},
    { intros v w f, unfold functor_uncurry_hom,
  ...
  @[hott] def hom_functor_assoc {C : Precategory} {a1 a2 a3 a4 a5 a6 : C}
  ...
  -- the functor hom(-,-)
  @[hott] def hom_functor.{u v} (C : Precategory.{u v}) : Cᵒᵖ ×c C ⇒ set.{v} :=
  functor.mk
  ...
  -- the functor hom(-, c)
  @[hott] def hom_functor_left.{u v} {C : Precategory.{u v}} (c : C)
    : Cᵒᵖ ⇒ set.{v} :=
  functor_curry_rev_ob !hom_functor c

  -- the functor hom(c, -)
  @[hott] def hom_functor_right.{u v} {C : Precategory.{u v}} (c : C)
    : C ⇒ set.{v} :=
  functor_curry_ob !hom_functor c

  @[hott] def nat_trans_hom_functor_left {C : Precategory}
