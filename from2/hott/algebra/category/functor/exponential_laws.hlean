...
namespace category
  ...
  @[hott] def functor_zero_iso_one (C : Precategory) : C ^c 0 ≅c 1 :=
  begin
  ...
  @[hott] def zero_functor_functor_zero (C : Precategory) (c : C) : 0 ^c C ⇒ 0 :=
  begin
  ...
  @[hott] def zero_functor_iso_zero (C : Precategory) (c : C) : 0 ^c C ≅c 0 :=
  begin
    fapply isomorphism.MK,
  ...
  @[hott] def functor_one_iso (C : Precategory) : C ^c 1 ≅c C :=
  begin
    fapply isomorphism.MK,
    ...
    { fapply functor_eq: esimp,
      { intro F, fapply functor_eq: esimp,
        ...
        { intros u v f, induction u, induction v, induction f, esimp, rewrite [+id_id,-respect_id]}},
      { intros F G η, apply nat_trans_eq, intro u, esimp,
        rewrite [natural_map_hom_of_eq _ u, natural_map_inv_of_eq _ u,▸*,+ap010_functor_eq _ _ u],
        induction u, rewrite [▸*, id_leftright]}},
    { fapply functor_eq: esimp,
      { intros c d f, rewrite [▸*, id_leftright]}},
  end

  /- 1 ^ C ≅ 1 -/

  @[hott] def one_functor_iso_one (C : Precategory) : 1 ^c C ≅c 1 :=
  begin
    fapply isomorphism.MK,
    { apply terminal_functor},
    { apply functor_curry, apply pr1_functor},
    { fapply functor_eq: esimp,
      { intro F, fapply functor_eq: esimp,
        ...
        { intros c d f, apply unit.eta}},
      { intros F G η, fapply nat_trans_eq, esimp, intro c, apply unit.eta}},
    { fapply functor_eq: esimp,
      { intro u, apply unit.eta},
      { intros u v f, apply unit.eta}},
  end

  /- C ^ 2 ≅ C × C -/

  @[hott] def functor_two_right (C : Precategory)
    : C ^c c2 ⇒ C ×c C :=
  begin
    fapply functor.mk: esimp,
    { intro F, exact (F ff, F tt)},
    { intros F G η, esimp, exact (η ff, η tt)},
    { intro F, reflexivity},
    { intros F G H η θ, reflexivity}
  end

  @[hott] def functor_two_left (C : Precategory)
    : C ×c C ⇒ C ^c c2 :=
  begin
    fapply functor.mk: esimp,
    { intro v, exact c2_functor C v.1 v.2},
    { intros v w f, exact c2_nat_trans f.1 f.2},
    { intro v, apply nat_trans_eq, esimp, intro b, induction b: reflexivity},
    { intros u v w g f, apply nat_trans_eq, esimp, intro b, induction b: reflexivity}
  end

  @[hott] def functor_two_iso (C : Precategory)
    : C ^c c2 ≅c C ×c C :=
  begin
    ...
    { fapply functor_eq: esimp,
      ...
      { intros F G η, fapply nat_trans_eq, intro b, esimp,
    ...
    { fapply functor_eq: esimp,
      ...
      { intros v w f, induction v, induction w, esimp, apply prod_eq: apply id_leftright}},
  end

  /- Cᵒᵖ ^ Dᵒᵖ ≅ (C ^ D)ᵒᵖ -/

  @[hott] def opposite_functor_opposite_iso (C D : Precategory)
  ...
  begin
    ...
    { fapply functor_eq: esimp,
      ...
      { intros F G η, fapply nat_trans_eq, esimp, intro d,
        rewrite [@natural_map_hom_of_eq _ _ _ G, @natural_map_inv_of_eq _ _ _ F,
  ...
  @[hott] def functor_sum_right (C D E : Precategory)
  ...
  @[hott] def functor_sum_left (C D E : Precategory)
    : C ^c D ×c C ^c E ⇒ C ^c (D +c E) :=
  begin
    fapply functor.mk: esimp,
    { intro V, exact V.1 +f V.2},
    { intros V W ν, apply sum_nat_trans, exact ν.1, exact ν.2},
    { intro V, apply nat_trans_eq, intro a, induction a: reflexivity},
    { intros U V W ν μ, apply nat_trans_eq, intro a, induction a: reflexivity}
      -- REPORT: cannot abstract
  end

  @[hott] def functor_sum_iso (C D E : Precategory)
  ...
  begin
    ...
    { fapply functor_eq: esimp,
      ...
      { intros F G η, fapply nat_trans_eq, intro a, esimp,
    ...
    { fapply functor_eq: esimp,
      ...
      { intros V W ν, induction V with F G, induction W with F' G', induction ν with η θ,
  ...
  @[hott] def prod_functor_right (C D E : Precategory)
  ...
  @[hott] def prod_functor_left (C D E : Precategory)
    : C ^c E ×c D ^c E ⇒ (C ×c D) ^c E :=
  begin
    fapply functor.mk: esimp,
    { intro V, exact V.1 ×f V.2},
    { intros V W ν, exact prod_nat_trans ν.1 ν.2},
    { intro V, apply nat_trans_eq, intro e, reflexivity},
    { intros U V W ν μ, apply nat_trans_eq, intro e, reflexivity}
  end

  @[hott] def prod_functor_iso (C D E : Precategory)
  ...
  begin
    ...
    { fapply functor_eq: esimp,
      ...
      { intros F G η, fapply nat_trans_eq, intro e, esimp,
    ...
    { fapply functor_eq: esimp,
      ...
      { intros V W ν, rewrite [@pr1_hom_of_eq (C ^c E) (D ^c E), @pr2_hom_of_eq (C ^c E) (D ^c E),
  ...
  @[hott] def functor_functor_right (C D E : Precategory)
    : (C ^c D) ^c E ⇒ C ^c (E ×c D) :=
  begin
    fapply functor.mk: esimp,
    ...
    { intros F G H η θ, apply nat_trans_eq, intro e, reflexivity}
  end

  @[hott] def functor_functor_left (C D E : Precategory)
    : C ^c (E ×c D) ⇒ (C ^c D) ^c E :=
  begin
    fapply functor.mk: esimp,
    { exact functor_curry},
  ...
  @[hott] def functor_functor_iso (C D E : Precategory)
  ...
  begin
    ...
    { fapply functor_eq: esimp,
      ...
      { intros F G η, fapply nat_trans_eq, esimp, intro v, induction v with c d,
        rewrite [@natural_map_hom_of_eq _ _ _ G, @natural_map_inv_of_eq _ _ _ F,
