...
structure functor (C D : Precategory) : Type _ :=
  (to_fun_ob : C → D)
...
namespace functor
  ...
  @[hott] def functor_mk_eq' {F₁ F₂ : C → D} {H₁ : Π(a b : C), hom a b → hom (F₁ a) (F₁ b)}
    {H₂ : Π(a b : C), hom a b → hom (F₂ a) (F₂ b)} (id₁ id₂ comp₁ comp₂)
    (pF : F₁ = F₂) (pH : pF ▸ H₁ = H₂)
      : functor.mk F₁ H₁ id₁ comp₁ = functor.mk F₂ H₂ id₂ comp₂ :=
  apdt01111 functor.mk pF pH !is_prop.elim !is_prop.elim

  @[hott] def functor_eq' {F₁ F₂ : C ⇒ D} : Π(p : to_fun_ob F₁ = to_fun_ob F₂),
  ...
  @[hott] def functor_mk_eq_constant {F : C → D} {H₁ : Π(a b : C), hom a b → hom (F a) (F b)}
    {H₂ : Π(a b : C), hom a b → hom (F a) (F b)} (id₁ id₂ comp₁ comp₂)
    (pH : Π(a b : C) (f : hom a b), H₁ a b f = H₂ a b f)
      : functor.mk F H₁ id₁ comp₁ = functor.mk F H₂ id₂ comp₂ :=
  functor_eq (λc, idp) (λa b f, !id_leftright ⬝ !pH)

  @[hott] def preserve_is_iso (F : C ⇒ D) {a b : C} (f : hom a b) [H : is_iso f]
    : is_iso (F f) :=
  begin
  ...
  @[hott] theorem respect_inv (F : C ⇒ D) {a b : C} (f : hom a b) [H : is_iso f] [H' : is_iso (F f)] :
    F (f⁻¹) = (F f)⁻¹ :=
  begin
  ...
  @[hott] def to_fun_iso (F : C ⇒ D) {a b : C} (f : a ≅ b) : F a ≅ F b :=
  iso.mk (F f) _

  @[hott] theorem respect_inv' (F : C ⇒ D) {a b : C} (f : hom a b) {H : is_iso f} : F (f⁻¹) = (F f)⁻¹ :=
  respect_inv F f

  @[hott] theorem respect_refl (F : C ⇒ D) (a : C) : to_fun_iso F (iso.refl a) = iso.refl (F a) :=
  iso_eq !respect_id

  @[hott] theorem respect_symm (F : C ⇒ D) {a b : C} (f : a ≅ b)
    : to_fun_iso F f⁻¹ⁱ = (to_fun_iso F f)⁻¹ⁱ :=
  iso_eq !respect_inv

  @[hott] theorem respect_trans (F : C ⇒ D) {a b c : C} (f : a ≅ b) (g : b ≅ c)
    : to_fun_iso F (f ⬝i g) = to_fun_iso F f ⬝i to_fun_iso F g :=
  iso_eq !respect_comp

  @[hott] def respect_iso_of_eq (F : C ⇒ D) {a b : C} (p : a = b) :
    to_fun_iso F (iso_of_eq p) = iso_of_eq (ap F p) :=
  by induction p; apply respect_refl

  @[hott] theorem respect_hom_of_eq (F : C ⇒ D) {a b : C} (p : a = b) :
    F (hom_of_eq p) = hom_of_eq (ap F p) :=
  by induction p; apply respect_id

  @[hott] def respect_inv_of_eq (F : C ⇒ D) {a b : C} (p : a = b) :
  ...
  @[hott] def change_fun (F : C ⇒ D) (Fob : C → D)
  ...
  @[hott] def functor_mk_eq'_idp (F : C → D) (H : Π(a b : C), hom a b → hom (F a) (F b))
  ...
  @[hott] def functor_eq'_idp (F : C ⇒ D) : functor_eq' idp idp = (idpath F) :=
  by (cases F; apply functor_mk_eq'_idp)

  @[hott] def functor_eq_eta' {F₁ F₂ : C ⇒ D} (p : F₁ = F₂)
  ...
  @[hott] theorem functor_eq2' {F₁ F₂ : C ⇒ D} {p₁ p₂ : to_fun_ob F₁ = to_fun_ob F₂} (q₁ q₂)
    (r : p₁ = p₂) : functor_eq' p₁ q₁ = functor_eq' p₂ q₂ :=
  by cases r; apply (ap (functor_eq' p₂)); apply is_prop.elim

  @[hott] theorem functor_eq2 {F₁ F₂ : C ⇒ D} (p q : F₁ = F₂) (r : ap010 to_fun_ob p ~ ap010 to_fun_ob q)
    : p = q :=
  begin
  ...
  @[hott] theorem ap010_apd01111_functor {F₁ F₂ : C → D} {H₁ : Π(a b : C), hom a b → hom (F₁ a) (F₁ b)}
    {H₂ : Π(a b : C), hom a b → hom (F₂ a) (F₂ b)} {id₁ id₂ comp₁ comp₂}
  ...
  @[hott] def ap010_functor_eq {F₁ F₂ : C ⇒ D} (p : to_fun_ob F₁ ~ to_fun_ob F₂)
  ...
  @[hott] def hom_pathover_functor {c₁ c₂ : C} {p : c₁ = c₂} (F G : C ⇒ D)
    {f₁ : F c₁ ⟶ G c₁} {f₂ : F c₂ ⟶ G c₂}
  ...
  @[hott] def hom_pathover_constant_left_functor_right {c₁ c₂ : C} {p : c₁ = c₂} {d : D} (F : C ⇒ D)
  ...
  @[hott] def hom_pathover_functor_left_constant_right {c₁ c₂ : C} {p : c₁ = c₂} {d : D} (F : C ⇒ D)
  ...
  @[hott] def hom_pathover_id_left_functor_right {c₁ c₂ : C} {p : c₁ = c₂} (F : C ⇒ C)
  ...
  @[hott] def hom_pathover_functor_left_id_right {c₁ c₂ : C} {p : c₁ = c₂} (F : C ⇒ C)
