...
namespace category
  ...
  @[hott] def is_left_adjoint_of_adjoint (H : F ⊣ G) : is_left_adjoint F :=
  begin
    induction H with η ε H K, exact is_left_adjoint.mk G η ε H K
  end

  @[hott] def adjoint_opposite (H : F ⊣ G) : Gᵒᵖᶠ ⊣ Fᵒᵖᶠ :=
  begin
    fconstructor,
  ...
  @[hott] def adjoint_of_opposite (H : Fᵒᵖᶠ ⊣ Gᵒᵖᶠ) : G ⊣ F :=
  begin
    fconstructor,
  ...
  @[hott] theorem is_prop_is_left_adjoint [instance] {C : Category} {D : Precategory} (F : C ⇒ D)
  ...
  @[hott] theorem adj_eq_unit (c : C) (d : D) (f : F c ⟶ d)
  ...
  @[hott] theorem adj_eq_counit (c : C) (d : D) (g : c ⟶ G d)
