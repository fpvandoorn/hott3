...
namespace category
  @[hott] def precategory_prod [instance] (obC obD : Type _)
  ...
  @[hott] def Precategory_prod [reducible] (C D : Precategory) : Precategory :=
  ...
  @[hott] def prod_functor_eta (F : X ⇒ C ×c D) : pr1_functor ∘f F ×f pr2_functor ∘f F = F :=
  begin
  fapply functor_eq: esimp,
  { intro e, apply prod_eq: reflexivity},
  { intros e e' f, apply prod_eq: esimp,
  ...
  @[hott] def pr1_functor_prod (F : X ⇒ C) (G : X ⇒ D) : pr1_functor ∘f (F ×f G) = F :=
  functor_eq (λx, idp)
             (λx y f, !id_leftright)

  @[hott] def pr2_functor_prod (F : X ⇒ C) (G : X ⇒ D) : pr2_functor ∘f (F ×f G) = G :=
  functor_eq (λx, idp)
             (λx y f, !id_leftright)

  -- @[hott] def universal_property_prod {C D X : Precategory} (F : X ⇒ C) (G : X ⇒ D)
  ...
  --       { intros x y f, apply prod_eq: esimp,
  --         { exact sorry},
  --         { exact sorry}}},
  --     { exact sorry}
  --   end

  @[hott] def prod_functor_prod (F : C ⇒ D) (G : C' ⇒ D') : C ×c C' ⇒ D ×c D' :=
  (F ∘f pr1_functor) ×f (G ∘f pr2_functor)

  @[hott] def prod_nat_trans {C D D' : Precategory}
    {F F' : C ⇒ D} {G G' : C ⇒ D'} (η : F ⟹ F') (θ : G ⟹ G') : F ×f G ⟹ F' ×f G' :=
  begin
    fapply nat_trans.mk: esimp,
    { intro c, exact (η c, θ c)},
    { intros c c' f, apply prod_eq: esimp:apply naturality}
  end

  infixr ` ×n `:70 := prod_nat_trans

  @[hott] def prod_flip_functor (C D : Precategory) : C ×c D ⇒ D ×c C :=
  functor.mk (λp, (p.2, p.1))
             (λp p' h, (h.2, h.1))
             (λp, idp)
             (λp p' p'' h' h, idp)

  @[hott] def functor_prod_flip_functor_prod_flip (C D : Precategory)
