...
namespace functor
  ...
  @[hott] def precategory_functor [instance] (D C : Precategory)
  ...
  @[hott] def Precategory_functor [reducible] (D C : Precategory) : Precategory :=
  ...
  @[hott] def nat_trans_inverse : G ⟹ F :=
  nat_trans.mk
    (λc, (η c)⁻¹)
    (λc d f,
    abstract begin
  ...
  @[hott] def nat_trans_left_inverse : nat_trans_inverse η ∘n η = 1 :=
  begin
  ...
  @[hott] def nat_trans_right_inverse : η ∘n nat_trans_inverse η = 1 :=
  begin
    fapply (apdt011 nat_trans.mk),
  ...
  @[hott] def is_natural_iso : is_iso η :=
  is_iso.mk _ (nat_trans_left_inverse η) (nat_trans_right_inverse η)

  variable (iso)
  @[hott] def natural_iso.mk : F ≅ G :=
  iso.mk _ (is_natural_iso η)

  omit iso

  variables (F G)
  @[hott] def is_natural_inverse (η : Πc, F c ≅ G c)
  ...
  @[hott] def is_natural_inverse' (η₁ : Πc, F c ≅ G c) (η₂ : F ⟹ G) (p : η₁ ~ η₂)
    {a b : C} (f : hom a b) : F f ∘ to_inv (η₁ a) = to_inv (η₁ b) ∘ G f :=
  is_natural_inverse F G η₁ abstract λa b g, (p a)⁻¹ ▸ (p b)⁻¹ ▸ naturality η₂ g end f

  variables {F G}
  @[hott] def natural_iso.MK
  ...
  include isoη
  @[hott] def componentwise_is_iso : is_iso (η c) :=
  ...
  variable {isoη}
  @[hott] def natural_map_inverse : natural_map η⁻¹ c = (η c)⁻¹ := idp
  variable [isoη]

  @[hott] def naturality_iso {c c' : C} (f : c ⟶ c') : G f = η c' ∘ F f ∘ (η c)⁻¹ :=
  calc
    G f = (G f ∘ η c) ∘ (η c)⁻¹  : by rewrite comp_inverse_cancel_right
    ... = (η c' ∘ F f) ∘ (η c)⁻¹ : by rewrite naturality
    ... = η c' ∘ F f ∘ (η c)⁻¹   : by rewrite assoc

  @[hott] def naturality_iso' {c c' : C} (f : c ⟶ c') : (η c')⁻¹ ∘ G f ∘ η c = F f :=
  calc
  ...
  @[hott] def componentwise_iso (η : F ≅ G) (c : C) : F c ≅ G c :=
  iso.mk (natural_map (to_hom η) c)
         (@componentwise_is_iso _ _ _ _ (to_hom η) (struct η) c)

  @[hott] def componentwise_iso_id (c : C) : componentwise_iso (iso.refl F) c = iso.refl (F c) :=
  iso_eq (idpath (ID (F c)))

  @[hott] def componentwise_iso_iso_of_eq (p : F = G) (c : C)
    : componentwise_iso (iso_of_eq p) c = iso_of_eq (ap010 to_fun_ob p c) :=
  eq.rec_on p !componentwise_iso_id

  @[hott] theorem naturality_iso_id {F : C ⇒ C} (η : F ≅ 1) (c : C)
  ...
  @[hott] def id_left_natural_rev (F : C ⇒ D) : F ⟹ functor.id ∘f F :=
  change_natural_map
  ...
  @[hott] def id_right_natural (F : C ⇒ D) : F ∘f functor.id ⟹ F :=
  change_natural_map
    (hom_of_eq !functor.id_right)
    (λc, id)
    (λc, by induction F; exact !natural_map_hom_of_eq ⬝ ap hom_of_eq !ap010_functor_mk_eq_constant)

  @[hott] def id_right_natural_rev (F : C ⇒ D) : F ⟹ F ∘f functor.id :=
  change_natural_map
  ...
  @[hott] def is_iso_nf_compose (G : D ⇒ E) (η : F ⟹ F') [H : is_iso η]
    : is_iso (G ∘fn η) :=
  is_iso.mk
  ...
  @[hott] def is_iso_fn_compose (η : G ⟹ G') (F : C ⇒ D) [H : is_iso η]
    : is_iso (η ∘nf F) :=
  is_iso.mk
  ...
  @[hott] def functor_iso_compose (G : D ⇒ E) (η : F ≅ F') : G ∘f F ≅ G ∘f F' :=
  iso.mk _ (is_iso_nf_compose G (to_hom η))

  @[hott] def iso_functor_compose (η : G ≅ G') (F : C ⇒ D) : G ∘f F ≅ G' ∘f F :=
...
/- TODO: also needs n_nf_distrib and id_nf for these compositions
  @[hott] def nidf_compose (η : J ⟹ 1) (F : C ⇒ D) [H : is_iso η]
    : is_iso (η ∘n1f F) :=
  is_iso.mk
  ...
  @[hott] def idnf_compose (η : 1 ⟹ J) (F : C ⇒ D) [H : is_iso η]
    : is_iso (η ∘1nf F) :=
  is_iso.mk _
            _
            _

  @[hott] def fnid_compose (F : D ⇒ E) (η : J ⟹ 1) [H : is_iso η]
    : is_iso (F ∘fn1 η) :=
  is_iso.mk _
            _
            _

  @[hott] def fidn_compose (F : D ⇒ E) (η : 1 ⟹ J) [H : is_iso η]
    : is_iso (F ∘f1n η) :=
  is_iso.mk _
            _
            _
-/
  ...
  namespace functor
    ...
    @[hott] def iso_of_eq_eq_of_iso (η : F ≅ G) : iso_of_eq (eq_of_iso η) = η :=
    begin
    apply iso_eq,
    apply nat_trans_eq,
    intro c,
    ...
    @[hott] def eq_of_iso_iso_of_eq (p : F = G) : eq_of_iso (iso_of_eq p) = p :=
    begin
    apply functor_eq2,
    intro c,
    esimp [eq_of_iso],
    ...
    @[hott] def is_univalent (D : Category) (C : Precategory) : is_univalent (D ^c C) :=
  ...
  @[hott] def category_functor [instance] (D : Category) (C : Precategory)
    : category (D ^c C) :=
  category.mk (D ^c C) (functor.is_univalent D C)

  @[hott] def Category_functor (D : Category) (C : Precategory) : Category :=
  category.Mk (D ^c C) !category_functor

  --this @[hott] def is only useful if the exponent is a category,
  --  and the elaborator has trouble with inserting the coercion
  @[hott] def Category_functor' (D C : Category) : Category :=
  ...
  namespace functor
    ...
    @[hott] def eq_of_pointwise_iso (η : F ⟹ G) (iso : Π(a : C), is_iso (η a)) : F = G :=
    eq_of_iso (natural_iso.mk η iso)

   @[hott] def iso_of_eq_eq_of_pointwise_iso (η : F ⟹ G) (iso : Π(c : C), is_iso (η c))
      : iso_of_eq (eq_of_pointwise_iso η iso) = natural_iso.mk η iso :=
   !iso_of_eq_eq_of_iso

   @[hott] def hom_of_eq_eq_of_pointwise_iso (η : F ⟹ G) (iso : Π(c : C), is_iso (η c))
      : hom_of_eq (eq_of_pointwise_iso η iso) = η :=
   !hom_of_eq_eq_of_iso

   @[hott] def inv_of_eq_eq_of_pointwise_iso (η : F ⟹ G) (iso : Π(c : C), is_iso (η c))
  ...
  @[hott] def constant2_functor (F : I ⇒ D ^c C) (c : C) : I ⇒ D :=
  ...
  @[hott] def eval_functor (C D : Precategory) (d : D) : C ^c D ⇒ C :=
  begin
    fapply functor.mk: esimp,
    { intro F, exact F d},
    { intros G F η, exact η d},
    { intro F, reflexivity},
    { intros H G F η θ, reflexivity},
  end

  @[hott] def precomposition_functor {C D} (E) (F : C ⇒ D)
    : E ^c D ⇒ E ^c C :=
  begin
    fapply functor.mk: esimp,
    { intro G, exact G ∘f F},
    { intros G H η, exact η ∘nf F},
    { intro G, reflexivity},
    { intros G H I η θ, reflexivity},
  end

  @[hott] def faithful_precomposition_functor [instance]
    {C D E} {H : C ⇒ D} [Hs : essentially_surjective H] : faithful (precomposition_functor E H) :=
  begin
    intros F G γ δ Hγδ, apply nat_trans_eq, intro b,
  ...
  private @[hott] def fully_faithful_precomposition_functor_prop [instance] (b) :
  ...
  private @[hott] def fully_faithful_precomposition_functor_pair [reducible] (b) :
    Σ g, Π a (f : H a ≅ b), γ a = G f⁻¹ⁱ ∘ g ∘ F f :=
  begin
    induction Hs b with Hb, induction Hb with a0 h, fconstructor,
    exact G h ∘ γ a0 ∘ F h⁻¹ⁱ, intros a f,
  ...
  --TODO speed this up
  private @[hott] def fully_faithful_precomposition_naturality {b b' : carrier D}
...
namespace functor
  ...
  structure essentially_surj_precomp_X (b : carrier B) : Type _ :=
    (c : carrier C)
  ...
  private @[hott] theorem X_eq : X.mk c k @k_coh = X.mk c' k' @k'_coh :=
  begin
    cases p,
    assert q' : k = k',
  ...
  private @[hott] theorem X_prop [instance] : is_prop (X b) :=
  begin
  ...
  private @[hott] def X_inh (b) : X b :=
  begin
    induction He.2 b with Hb, cases Hb with a0 Ha0,
    fconstructor, exact F a0,
    { intros a h, apply to_fun_iso F, apply reflect_iso H,
      exact h ⬝i Ha0⁻¹ⁱ },
    { intros a a' h h' f HH,
  ...
  private @[hott] def k := λ b, X.k (X_inh b)
  private @[hott] def k_coh := λ b, @X.k_coh b (X_inh b)

  private @[hott] def X_c_eq_of_eq {b} (t t' : X b) (p : t = t') : X.c t = X.c t' :=
  by cases p; reflexivity

  private @[hott] def X_k_eq_of_eq {b} (t t' : X b) (p : t = t') (a : carrier A) (h : H a ≅ b) :
    X_c_eq_of_eq t t' p ▸ X.k t a h = X.k t' a h:=
  by cases p; reflexivity

  private @[hott] def X_phi {b} (t : X b) : X.c t = X.c (X_inh b) :=
  X_c_eq_of_eq _ _ !is_prop.elim

  private @[hott] def X_phi_transp {b} (t : X b) (a : carrier A) (h : H a ≅ b) :
    (X_phi t) ▸ (X.k t a h) = k b a h :=
  by apply X_k_eq_of_eq t _ !is_prop.elim

  private @[hott] def X_phi_hom_of_eq' {b} (t t' : X b) (p : t = t') (a : carrier A) (h : H a ≅ b) :
    X.k t' a h ⬝i (iso_of_eq (X_c_eq_of_eq t t' p)⁻¹) = X.k t a h :=
  begin
    cases p, apply iso_eq, apply id_left
  end

  private @[hott] def X_phi_hom_of_eq {b} (t : X b) (a : carrier A) (h : H a ≅ b) :
    to_hom (k b a h ⬝i (iso_of_eq (X_phi t)⁻¹)) = to_hom (X.k t a h) :=
  begin
    apply ap to_hom, apply X_phi_hom_of_eq'
  end

  structure essentially_surj_precomp_Y {b b' : carrier B} (f : hom b b') : Type _ :=
    (g : hom (G0 b) (G0 b'))
  ...
  private @[hott] theorem Y_eq : Y.mk g @Hg = Y.mk g' @Hg' :=
  begin
  ...
  private @[hott] theorem Y_prop [instance] : is_prop (Y f) :=
  begin
  ...
  private @[hott] def G_hom := λ {b b'} (f : hom b b'), Y.g (Y_inh f)
  private @[hott] def G_hom_coh := λ {b b'} (f : hom b b'),
    @essentially_surj_precomp_Y.Hg b b' f (Y_inh f)

  private @[hott] theorem G_hom_id (b : carrier B) : G_hom (ID b) = ID (G0 b) :=
  begin
  ...
  private @[hott] theorem G_hom_comp {b0 b1 b2 : carrier B} (g : hom b1 b2) (f : hom b0 b1) :
  ...
  private @[hott] def G_functor : B ⇒ C :=
  begin
    fconstructor,
    { exact G0 },
    { intros b b' f, exact G_hom f },
    { intro b, apply G_hom_id },
    { intros a b c g f, apply G_hom_comp }
  end

  private @[hott] def XF (a0 : carrier A) : X (H a0) :=
  begin
    fconstructor,
    { exact F a0 },
    { intros a h, apply to_fun_iso F, apply reflect_iso, apply He.1, exact h },
    { intros a a' h h' f l, esimp,
  ...
  private @[hott] def G0_H_eq_F (a0 : carrier A) : G0 (H a0) = F a0 :=
  begin
    apply inverse, apply X_phi (XF a0)
  end

  private @[hott] theorem G_hom_H_eq_F {a0 a0' : carrier A} (f0 : hom a0 a0') :
  ...
  @[hott] def essentially_surjective_precomposition_functor [instance] {A B : Precategory}
  ...
  begin
    ...
    { intros a b f, exact G_hom_H_eq_F F f }
  end

  variables {C D E : Precategory}

  @[hott] def postcomposition_functor {C D} (E) (F : C ⇒ D)
    : C ^c E ⇒ D ^c E :=
  begin
    fapply functor.mk: esimp,
    { intro G, exact F ∘f G},
    { intros G H η, exact F ∘fn η},
    { intro G, apply fn_id},
    { intros G H I η θ, apply fn_n_distrib},
  end

  @[hott] def constant_diagram (C D) : C ⇒ C ^c D :=
  begin
    fapply functor.mk: esimp,
    { intro c, exact constant_functor D c},
    { intros c d f, exact constant_nat_trans D f},
    { intro c, fapply nat_trans_eq, reflexivity},
    { intros c d e g f, fapply nat_trans_eq, reflexivity},
  end

  @[hott] def opposite_functor_opposite_left (C D : Precategory)
    : (C ^c D)ᵒᵖ ⇒ Cᵒᵖ ^c Dᵒᵖ :=
  begin
    fapply functor.mk: esimp,
    { exact opposite_functor},
    { intros F G, exact opposite_nat_trans},
    { intro F, apply nat_trans_eq, reflexivity},
    { intros u v w g f, apply nat_trans_eq, reflexivity}
  end

  @[hott] def opposite_functor_opposite_right (C D : Precategory)
    : Cᵒᵖ ^c Dᵒᵖ ⇒ (C ^c D)ᵒᵖ :=
  begin
    fapply functor.mk: esimp,
    ...
    { intros F G H η θ, apply nat_trans_eq, intro d, reflexivity}
  end

  @[hott] def constant_diagram_opposite (C D)
    : (constant_diagram C D)ᵒᵖᶠ = opposite_functor_opposite_right C D ∘f constant_diagram Cᵒᵖ Dᵒᵖ :=
  begin
    fapply functor_eq,
    { reflexivity },
    { intros c c' f, esimp at *, refine !nat_trans.id_right ⬝ !nat_trans.id_left ⬝ _,
