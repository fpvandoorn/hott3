...
namespace category
  ...
  @[hott] def cone_to_obj := @cone_obj.c
  @[hott] def cone_to_nat (c : cone_obj F) : constant_functor I (cone_to_obj c) ⟹ F :=
  cone_obj.η c
  ...
  @[hott] def cone_to_hom := @cone_hom.f
  @[hott] def cone_to_eq (f : cone_hom x y) (i : I)
    : cone_to_nat y i ∘ (cone_to_hom f) = cone_to_nat x i :=
  cone_hom.p f i

  local attribute cone_to_hom [coercion]

  @[hott] def cone_id (x : cone_obj F) : cone_hom x x :=
  cone_hom.mk id
              (λi, !id_right)

  @[hott] def cone_comp (g : cone_hom y z) (f : cone_hom x y) : cone_hom x z :=
  cone_hom.mk (cone_to_hom g ∘ cone_to_hom f)
              abstract λi, by rewrite [assoc, +cone_to_eq] end

  @[hott] def cone_obj_eq (p : cone_to_obj x = cone_to_obj y)
  ...
  @[hott] theorem c_cone_obj_eq (p : cone_to_obj x = cone_to_obj y)
  ...
  @[hott] theorem cone_hom_eq {f f' : cone_hom x y} (q : cone_to_hom f = cone_to_hom f') : f = f' :=
  begin
  ...
  @[hott] def precategory_cone [instance] : precategory (cone_obj F) :=
  ...
  @[hott] def cone : Precategory :=
  precategory.Mk (precategory_cone F)

  variable {F}
  @[hott] def cone_iso_pr1 (h : x ≅ y) : cone_to_obj x ≅ cone_to_obj y :=
  iso.MK
  ...
  @[hott] def cone_iso.mk (f : cone_to_obj x ≅ cone_to_obj y)
  ...
  variables (x y)
  @[hott] def cone_iso_equiv : (x ≅ y) ≃ Σ(f : cone_to_obj x ≅ cone_to_obj y),
  ...
  @[hott] def cone_eq_equiv : (x = y) ≃ Σ(f : cone_to_obj x = cone_to_obj y),
  ...
  section is_univalent
    ...
    @[hott] def is_univalent_cone {I : Precategory} {C : Category} (F : I ⇒ C)
      : is_univalent (cone F) :=
    begin
      intros x y,
...
(x = y) ≃ (Σ(f : cone_to_obj x = cone_to_obj y), Πi, cone_to_nat y i ∘ hom_of_eq f = cone_to_nat x i)
    ...
    @[hott] def category_cone [instance] {I : Precategory} {C : Category} (F : I ⇒ C)
      : category (cone_obj F) :=
    category.mk _ (is_univalent_cone F)

    @[hott] def Category_cone {I : Precategory} {C : Category} (F : I ⇒ C)
      : Category :=
    Category.mk _ (category_cone F)

  end is_univalent

  @[hott] def cone_obj_compose (G : C ⇒ D) (x : cone_obj F) : cone_obj (G ∘f F) :=
  begin
  fapply cone_obj.mk,
  { exact G x},
  { fapply change_natural_map,
    { refine ((G ∘fn cone_to_nat x) ∘n _), apply nat_trans_of_eq, fapply functor_eq: esimp,
      intros i j k, esimp, rewrite [id_leftright,respect_id]},
