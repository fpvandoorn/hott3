...
namespace category
  ...
  @[hott] def comma_object_sigma_char : (Σ(a : A) (b : B), S a ⟶ T b) ≃ comma_object S T :=
  begin
    fapply equiv.MK,
  ...
  @[hott] theorem is_trunc_comma_object (n : trunc_index) [HA : is_trunc n A]
  ...
  variables {S T}
  @[hott] def comma_object_eq' {x y : comma_object S T} (p : ob1 x = ob1 y) (q : ob2 x = ob2 y)
  ...
  -- @[hott] def eq_comp_inverse_of_comp_eq' {ob : Type _} {C : precategory ob} {d c b : ob} {r : hom c d}
  ...
  @[hott] def ap_ob1_comma_object_eq' (x y : comma_object S T) (p : ob1 x = ob1 y) (q : ob2 x = ob2 y)
  ...
  @[hott] def ap_ob2_comma_object_eq' (x y : comma_object S T) (p : ob1 x = ob1 y) (q : ob2 x = ob2 y)
    (r : mor x =[ap011 (@hom C C) (ap (to_fun_ob S) p) (ap (to_fun_ob T) q)] mor y)
  ...
  @[hott] def comma_morphism_sigma_char :
  ...
  @[hott] theorem is_trunc_comma_morphism (n : trunc_index) [H1 : is_trunc n (ob1 x ⟶ ob1 y)]
  ...
  variables {x y z w}
  @[hott] def comma_morphism_eq {f f' : comma_morphism x y}
  ...
  @[hott] def comma_compose (g : comma_morphism y z) (f : comma_morphism x y) : comma_morphism x z :=
  comma_morphism.mk
    (mor1 g ∘ mor1 f)
    (mor2 g ∘ mor2 f)
  ...
  @[hott] def comma_id : comma_morphism x x :=
  comma_morphism.mk id id (by rewrite [+respect_id,id_left,id_right])

  @[hott] theorem comma_assoc (h : comma_morphism z w) (g : comma_morphism y z) (f : comma_morphism x y) :
    h ∘∘ (g ∘∘ f) = (h ∘∘ g) ∘∘ f :=
  comma_morphism_eq !assoc !assoc

  @[hott] theorem comma_id_left (f : comma_morphism x y) : comma_id ∘∘ f = f :=
  comma_morphism_eq !id_left !id_left

  @[hott] theorem comma_id_right (f : comma_morphism x y) : f ∘∘ comma_id = f :=
  comma_morphism_eq !id_right !id_right

  variables (S T)
  @[hott] def comma_category : Precategory :=
  ...
  --TODO: this @[hott] def doesn't use category structure of A and B
  @[hott] def strict_precategory_comma [HA : strict_precategory A] [HB : strict_precategory B] :
    strict_precategory (comma_object S T) :=
  strict_precategory.mk (comma_category S T) !is_trunc_comma_object

/-
  --set_option pp.notation false
  @[hott] def is_univalent_comma (HA : is_univalent A) (HB : is_univalent B)
