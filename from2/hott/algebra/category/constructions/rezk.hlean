...
namespace rezk
  ...
  inductive rezk_Q : Π ⦃a b : A⦄, e_closure iso a b → e_closure iso a b → Type _ :=
  | comp_con : Π ⦃a b c : A⦄ (g : b ≅ c) (f : a ≅ b) , rezk_Q [f ⬝i g] ([f] ⬝r [g])

  @[hott] def rezk_carrier := trunc_two_quotient 1 iso rezk_Q

  local attribute rezk_carrier [reducible]
  @[hott] def is_trunc_rezk_carrier [instance] : is_trunc 1 rezk_carrier := _

  variables {a b c : A}
  @[hott] def elt (a : A) : rezk_carrier := incl0 a
  @[hott] def pth (f : a ≅ b) : elt a = elt b := incl1 f

  @[hott] def resp_comp (g : b ≅ c) (f : a ≅ b) : pth (f ⬝i g) = pth f ⬝ pth g :=
  incl2 (rezk_Q.comp_con g f)

  @[hott] def resp_id (a : A) : pth (iso.refl a) = idp :=
  begin
  ...
  @[hott] def elim_pth {P : Type _} [is_trunc 1 P] {Pe : A → P} {Pp : Π⦃a b⦄ (f : a ≅ b), Pe a = Pe b}
...
attribute rezk.elt
attribute rezk.rec rezk.elim [recursor 8]
attribute rezk.rec_on rezk.elim_on
attribute rezk.set_rec rezk.set_elim
...
namespace rezk
  ...
  @[hott] def rezk_hom_left_pt (a : A) (b : @rezk_carrier A C) : Set.{k} :=
  begin
    refine rezk.elim_set _ _ _ b,
    { clear b, intro b, exact trunctype.mk' 0 (hom a b) },
    { clear b, intros b b' f, apply equiv_postcompose (iso.to_hom f) },
    { clear b, intros b b' b'' f g x, apply !assoc⁻¹ }
  end

  private @[hott] def pathover_rezk_hom_left_pt {a b c : A} (f : hom a b) (g : b ≅ c) :
  ...
  @[hott] def rezk_hom_left_pth_1_trunc [instance] (a a' : A) (f : a ≅ a') :
    Π b, is_trunc 1 (carrier (rezk_hom_left_pt a b) ≃ carrier (rezk_hom_left_pt a' b)) :=
  λ b, is_trunc_equiv _ _ _

  @[hott] def rezk_hom_left_pth (a a' : A) (f : a ≅ a') (b : rezk_carrier) :
  ...
  begin
    ...
    { intros b b' g, apply equiv_pathover, intros g' g'' H,
    ...
    intros b b' b'' g g', apply @is_prop.elim, apply is_trunc_pathover, apply is_trunc_equiv
  end

  @[hott] def rezk_hom (a b : @rezk_carrier A C) : Set.{k} :=
  begin
    refine rezk.elim_set _ _ _ a,
    { clear a, intro a, exact rezk_hom_left_pt a b },
    { clear a, intros a a' f, apply rezk_hom_left_pth a a' f },
    { clear a, intros a a' a'' Ef Eg Rfg, induction b using rezk.rec,
      apply assoc, apply is_prop.elimo, apply is_set.elimo }
  end

  private @[hott] def pathover_rezk_hom_left {a b c : A} (f : hom a c) (g : a ≅ b) :
  ...
  private @[hott] def pathover_rezk_hom_right {a b c : A} (f : hom a b) (g : b ≅ c) : --todo delete?
    pathover (rezk_hom (elt a)) f (pth g) ((to_hom g) ∘ f) :=
  begin
    apply pathover_rezk_hom_left_pt,
  end

  private @[hott] def transport_rezk_hom_eq_comp {a c : A} (f : hom a a) (g : a ≅ c) :
  ...
  @[hott] def rezk_id (a : @rezk_carrier A C) : rezk_hom a a :=
  begin
  ...
  @[hott] def rezk_comp_pt_pt [reducible] {c : rezk_carrier} {a b : A}
  ...
  @[hott] def rezk_comp_pt_pth [reducible] {c : rezk_carrier} {a b b' : A} {ib : iso b b'} :
  ...
  @[hott] def rezk_comp {a b c : @rezk_carrier A C} (g : rezk_hom b c) (f : rezk_hom a b) :
    rezk_hom a c :=
  begin
  ...
  @[hott] def rezk_precategory [instance] : precategory (@rezk_carrier A C) :=
  precategory.mk rezk_hom @rezk_comp rezk_id @assoc @id_left @id_right

  end

  @[hott] def to_rezk_Precategory.{l k} : Precategory.{l k} → Precategory.{(max l k) k} :=
  begin
    intro C, apply Precategory.mk (@rezk_carrier (Precategory.carrier C) C),
    apply rezk_precategory _ _,
  end

  @[hott] def rezk_functor (C : Precategory) : functor C (to_rezk_Precategory C) :=
  begin
  ...
  private @[hott] def hom_transport_eq_transport_hom {a b b' : @rezk_carrier A C} (f : a ≅ b)
    (p : b = b') : to_hom (transport (iso a) p f) = transport (λ x, hom _ _) p (to_hom f) :=
  by cases p; reflexivity

  private @[hott] def hom_transport_eq_transport_hom' {a a' b : @rezk_carrier A C} (f : a ≅ b)
    (p : a = a') : to_hom (transport (λ x, iso x b) p f) = transport (λ x, hom _ _) p (to_hom f) :=
  by cases p; reflexivity

  private @[hott] def pathover_iso_pth {a b b' : A} (f : elt a ≅ elt b)
  ...
  private @[hott] def pathover_iso_pth' {a a' b : A} (f : elt a ≅ elt b)
  ...
  private @[hott] def eq_of_iso_pt {a : A} {b : @rezk_carrier A C} :
    elt a ≅ b → elt a = b :=
  begin
    intro f,
    induction b using rezk.set_rec with b b b' ib,
    apply pth, apply iso_of_elt_iso f,
    apply arrow_pathover, intros f g p, apply eq_pathover,
  ...
  @[hott] def rezk_category.{l k} {A : Type l} [C : precategory.{l k} A] :
  ...
  @[hott] def fully_faithful_rezk_functor : fully_faithful (rezk_functor C) :=
  by intros; apply is_equiv.is_equiv_id

  open trunc
  @[hott] def essentially_surj_rezk_functor : essentially_surjective (rezk_functor C) :=
  begin
  ...
  @[hott] def is_weak_equiv_rezk_functor : is_weak_equivalence (rezk_functor C) :=
