...
namespace category
  ...
  @[hott] def is_complete_set_cone.{u v w}
    ...
    { with_options [elaborator.ignore_instances true] -- TODO: fix
      ( refine is_trunc_sigma _ _;
        ...
        ( intro s;
          ...
          apply is_trunc_eq))}},
  { fapply nat_trans.mk,
    { intros i x, esimp at x, exact x.1 i},
    { intros i j f, esimp, apply eq_of_homotopy, intro x, esimp at x, induction x with s p,
      esimp, apply p}}
  end

  @[hott] def is_complete_set.{u v w} [instance] : is_complete.{(max u v w)+1 (max u v w) v w} set :=
  begin
    intros I F, fapply has_terminal_object.mk,
  ...
  @[hott] def is_cocomplete_set_cone.{u v w}
  ...
  { fapply nat_trans.mk,
    { intros i x, esimp, apply class_of, exact ⟨i, x⟩},
    { intros i j f, esimp, apply eq_of_homotopy, intro y, apply eq_of_rel, esimp,
      exact exists.intro f idp}}
  end

  -- TODO: change this after induction tactic for trunc/set_quotient is implemented
  @[hott] def is_cocomplete_set.{u v w} [instance]
    ...
    { intro c, esimp at *, induction c with X η, induction η with η p, esimp at *,
      ...
      { fapply cone_hom.mk,
        { refine set_quotient.elim _ _,
          ...
          { intros v w r, induction v with i x, induction w with j y, esimp at *,
      ...
      { esimp, intro h, induction h with f q, apply cone_hom_eq, esimp at *,
        ...
        { intros v w r, apply is_prop.elimo}}},
  end
