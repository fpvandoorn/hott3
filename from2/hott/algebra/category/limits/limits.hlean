...
namespace category
  ...
  variables {ob : Type _} [C : precategory ob] {c c' : ob} (D I : Precategory)
  include C

  @[hott] def is_terminal [class] (c : ob) := Πd, is_contr (d ⟶ c)
  @[hott] def is_contr_of_is_terminal (c d : ob) [H : is_terminal d] : is_contr (c ⟶ d) :=
  H c

  local attribute is_contr_of_is_terminal [instance]

  @[hott] def terminal_morphism (c c' : ob) [H : is_terminal c'] : c ⟶ c' :=
  !center

  @[hott] def hom_terminal_eq [H : is_terminal c'] (f f' : c ⟶ c') : f = f' :=
  !is_prop.elim

  @[hott] def eq_terminal_morphism [H : is_terminal c'] (f : c ⟶ c') : f = terminal_morphism c c' :=
  !is_prop.elim

  @[hott] def terminal_iso_terminal (c c' : ob) [H : is_terminal c] [K : is_terminal c']
    : c ≅ c' :=
  iso.MK !terminal_morphism !terminal_morphism !hom_terminal_eq !hom_terminal_eq

  local attribute is_terminal [reducible]
  @[hott] theorem is_prop_is_terminal [instance] : is_prop (is_terminal c) :=
  _
  ...
  @[hott] def terminal_object [reducible] := @has_terminal_object.d
  attribute has_terminal_object.is_terminal [instance]

  variable {D}
  @[hott] def terminal_object_iso_terminal_object (H₁ H₂ : has_terminal_object D)
    : @terminal_object D H₁ ≅ @terminal_object D H₂ :=
  !terminal_iso_terminal

  @[hott] theorem is_prop_has_terminal_object [instance] (D : Category)
    : is_prop (has_terminal_object D) :=
  begin
    apply is_prop.mk, intros t₁ t₂, induction t₁ with d₁ H₁, induction t₂ with d₂ H₂,
    have p : d₁ = d₂,
  ...
  variable (D)
  @[hott] def has_limits_of_shape [class] := Π(F : I ⇒ D), has_terminal_object (cone F)
  ...
  @[hott] def is_complete.{o₁ h₁ o₂ h₂} [class] (D : Precategory.{o₁ h₁}) :=
  Π(I : Precategory.{o₂ h₂}), has_limits_of_shape D I

  @[hott] def has_limits_of_shape_of_is_complete [instance] [H : is_complete D] (I : Precategory)
    : has_limits_of_shape D I := H I

  section
    open pi
    @[hott] theorem is_prop_has_limits_of_shape [instance] (D : Category) (I : Precategory)
    ...
    @[hott] theorem is_prop_is_complete [instance] (D : Category) : is_prop (is_complete D) := _
  end

  variables {D I}
  @[hott] def has_terminal_object_cone [H : has_limits_of_shape D I]
  ...
  @[hott] def limit_cone : cone F := !terminal_object

  @[hott] def is_terminal_limit_cone [instance] : is_terminal (limit_cone F) :=
  ...
  @[hott] def to_limit_cone_hom {d : D} {η : Πi, d ⟶ F i}
  ...
  omit K
  @[hott] def to_limit_object_iso_to_limit_object (x y : cone_obj F)
  ...
  @[hott] def limit_object : D :=
  cone_to_obj (limit_cone F)

  @[hott] def limit_nat_trans : constant_functor I (limit_object F) ⟹ F :=
  cone_to_nat (limit_cone F)

  @[hott] def limit_morphism (i : I) : limit_object F ⟶ F i :=
  limit_nat_trans F i

  variable {H}
  @[hott] theorem limit_commute {i j : I} (f : i ⟶ j)
    : to_fun_hom F f ∘ limit_morphism F i = limit_morphism F j :=
  naturality (limit_nat_trans F) f ⬝ !id_right

  variable [H]
  @[hott] def limit_cone_obj {d : D} {η : Πi, d ⟶ F i}
    (p : Π⦃i j : I⦄ (f : i ⟶ j), to_fun_hom F f ∘ η i = η j) : cone_obj F :=
  cone_obj.mk d (nat_trans.mk η (λa b f, p f ⬝ !id_right⁻¹))

  variable {H}
  @[hott] def hom_limit {d : D} (η : Πi, d ⟶ F i)
  ...
  variable {F}
  @[hott] theorem eq_hom_limit {d : D} {η : Πi, d ⟶ F i}
    (p : Π⦃i j : I⦄ (f : i ⟶ j), to_fun_hom F f ∘ η i = η j) {h : d ⟶ limit_object F}
  ...
  @[hott] theorem limit_cone_unique {d : D} {η : Πi, d ⟶ F i}
    (p : Π⦃i j : I⦄ (f : i ⟶ j), to_fun_hom F f ∘ η i = η j)
  ...
  @[hott] def limit_hom_limit {F G : I ⇒ D} (η : F ⟹ G) : limit_object F ⟶ limit_object G :=
  hom_limit _ (λi, η i ∘ limit_morphism F i)
              abstract by intros i j f; rewrite [assoc,naturality,-assoc,limit_commute] end

  @[hott] theorem limit_hom_limit_commute {F G : I ⇒ D} (η : F ⟹ G)
  ...
  variable (F)
  @[hott] def limit_object_iso_limit_object (H₁ H₂ : has_limits_of_shape D I) :
    @(limit_object F) H₁ ≅ @(limit_object F) H₂ :=
  cone_iso_pr1 !terminal_object_iso_terminal_object

  @[hott] def limit_functor (D I : Precategory) [H : has_limits_of_shape D I]
    : D ^c I ⇒ D :=
  begin
    ...
    { intros F G H η θ, unfold limit_hom_limit, refine (eq_hom_limit _ _)⁻¹, intro i,
      rewrite [assoc, hom_limit_commute, -assoc, hom_limit_commute, assoc]}
  end

  section bin_products
  open bool prod.ops
  @[hott] def has_binary_products [reducible] (D : Precategory) := has_limits_of_shape D c2
  variables [K : has_binary_products D] (d d' : D)
  include K

  @[hott] def product_object : D :=
  limit_object (c2_functor D d d')

  infixr ` ×l `:75 := product_object

  @[hott] def pr1 : d ×l d' ⟶ d :=
  limit_morphism (c2_functor D d d') ff

  @[hott] def pr2 : d ×l d' ⟶ d' :=
  limit_morphism (c2_functor D d d') tt

  variables {d d'}
  @[hott] def hom_product {x : D} (f : x ⟶ d) (g : x ⟶ d') : x ⟶ d ×l d' :=
  hom_limit (c2_functor D d d') (bool.rec f g)
    (by intros b₁ b₂ f; induction b₁: induction b₂: esimp at *; try contradiction: apply id_left)

  @[hott] theorem pr1_hom_product {x : D} (f : x ⟶ d) (g : x ⟶ d') : !pr1 ∘ hom_product f g = f :=
  hom_limit_commute (c2_functor D d d') (bool.rec f g) _ ff

  @[hott] theorem pr2_hom_product {x : D} (f : x ⟶ d) (g : x ⟶ d') : !pr2 ∘ hom_product f g = g :=
  hom_limit_commute (c2_functor D d d') (bool.rec f g) _ tt

  @[hott] theorem eq_hom_product {x : D} {f : x ⟶ d} {g : x ⟶ d'} {h : x ⟶ d ×l d'}
    (p : !pr1 ∘ h = f) (q : !pr2 ∘ h = g) : h = hom_product f g :=
  eq_hom_limit _ (bool.rec p q)

  @[hott] theorem product_cone_unique {x : D} {f : x ⟶ d} {g : x ⟶ d'}
  ...
  @[hott] def product_functor : D ×c D ⇒ D :=
  functor.mk
    ...
    abstract begin intros x y z g f, symmetry, apply eq_hom_product,
  ...
  @[hott] def product_object_iso_product_object (H₁ H₂ : has_binary_products D) :
  ...
  @[hott] theorem equalizer_cone_unique {h : x ⟶ d} (p : f ∘ h = g ∘ h)
  ...
  omit K
  variables (f g)
  @[hott] def equalizer_object_iso_equalizer_object (H₁ H₂ : has_equalizers D) :
  ...
  @[hott] theorem eq_hom_pullback {h₁ : x ⟶ d₁} {h₂ : x ⟶ d₂} (p : f ∘ h₁ = g ∘ h₂)
  ...
  @[hott] theorem pullback_cone_unique {h₁ : x ⟶ d₁} {h₂ : x ⟶ d₂} (p : f ∘ h₁ = g ∘ h₂)
    {k₁ : x ⟶ pullback_object f g} (q₁ : pullback f g ∘ k₁ = h₂) (r₁ : pullback_rev f g ∘ k₁ = h₁)
    {k₂ : x ⟶ pullback_object f g} (q₂ : pullback f g ∘ k₂ = h₂) (r₂ : pullback_rev f g ∘ k₂ = h₁)
    : k₁ = k₂ :=
  ...
  variables (f g)
  @[hott] def pullback_object_iso_pullback_object (H₁ H₂ : has_pullbacks D) :
