...
namespace category
  ...
  @[hott] def is_initial [reducible] (c : ob) := @is_terminal _ (opposite C) c

  @[hott] def is_contr_of_is_initial (c d : ob) [H : is_initial d]
    : is_contr (d ⟶ c) :=
  H c

  local attribute is_contr_of_is_initial [instance]

  @[hott] def initial_morphism (c c' : ob) [H : is_initial c'] : c' ⟶ c :=
  !center

  @[hott] def hom_initial_eq [H : is_initial c'] (f f' : c' ⟶ c) : f = f' :=
  !is_prop.elim

  @[hott] def eq_initial_morphism [H : is_initial c'] (f : c' ⟶ c) : f = initial_morphism c c' :=
  !is_prop.elim

  @[hott] def initial_iso_initial {c c' : ob} (H : is_initial c) (K : is_initial c') : c ≅ c' :=
  iso_of_opposite_iso (@terminal_iso_terminal _ (opposite C) _ _ H K)

  @[hott] theorem is_prop_is_initial [instance] : is_prop (is_initial c) := _

  omit C

  @[hott] def has_initial_object [reducible] : Type _ := has_terminal_object Dᵒᵖ

  @[hott] def initial_object [reducible] [H : has_initial_object D] : D :=
  has_terminal_object.d Dᵒᵖ

  @[hott] def has_initial_object.is_initial [H : has_initial_object D]
    : is_initial (initial_object D) :=
  @has_terminal_object.is_terminal (Opposite D) H

  variable {D}
  @[hott] def initial_object_iso_initial_object (H₁ H₂ : has_initial_object D)
  ...
  @[hott] theorem is_prop_has_initial_object [instance] (D : Category)
  ...
  @[hott] def has_colimits_of_shape_of_is_cocomplete [instance] [H : is_cocomplete D]
    (I : Precategory) : has_colimits_of_shape D I := H Iᵒᵖ

  section
    open pi
    @[hott] theorem is_prop_has_colimits_of_shape [instance] (D : Category) (I : Precategory)
      : is_prop (has_colimits_of_shape D I) :=
    is_prop_has_limits_of_shape (Category_opposite D) _

    @[hott] theorem is_prop_is_cocomplete [instance] (D : Category) : is_prop (is_cocomplete D) :=
  ...
  @[hott] def has_initial_object_cocone [H : has_colimits_of_shape D I]
  ...
  @[hott] def colimit_cocone : cocone F := limit_cone Fᵒᵖᶠ

  @[hott] def is_initial_colimit_cocone [instance] : is_initial (colimit_cocone F) :=
  is_terminal_limit_cone Fᵒᵖᶠ

  @[hott] def colimit_object : D :=
  limit_object Fᵒᵖᶠ

  @[hott] def colimit_nat_trans : constant_functor Iᵒᵖ (colimit_object F) ⟹ Fᵒᵖᶠ :=
  limit_nat_trans Fᵒᵖᶠ

  @[hott] def colimit_morphism (i : I) : F i ⟶ colimit_object F :=
  limit_morphism Fᵒᵖᶠ i

  variable {H}
  @[hott] theorem colimit_commute {i j : I} (f : i ⟶ j)
    : colimit_morphism F j ∘ to_fun_hom F f = colimit_morphism F i :=
  by rexact limit_commute Fᵒᵖᶠ f

  variable [H]
  @[hott] def colimit_cone_obj {d : D} {η : Πi, F i ⟶ d}
    (p : Π⦃j i : I⦄ (f : i ⟶ j), η j ∘ to_fun_hom F f = η i) : cone_obj Fᵒᵖᶠ :=
  limit_cone_obj Fᵒᵖᶠ proof p qed

  variable {H}
  @[hott] def colimit_hom {d : D} (η : Πi, F i ⟶ d)
    (p : Π⦃j i : I⦄ (f : i ⟶ j), η j ∘ to_fun_hom F f = η i) : colimit_object F ⟶ d :=
  hom_limit Fᵒᵖᶠ η proof p qed

  @[hott] theorem colimit_hom_commute {d : D} (η : Πi, F i ⟶ d)
  ...
  @[hott] def colimit_cone_hom {d : D} {η : Πi, F i ⟶ d}
  ...
  @[hott] def colimit_hom_colimit [reducible] {F G : I ⇒ D} (η : F ⟹ G)
    : colimit_object F ⟶ colimit_object G :=
  colimit_hom _ (λi, colimit_morphism G i ∘ η i)
              abstract by intros i j f; rewrite [-assoc,-naturality,assoc,colimit_commute] end

  omit H

  variable (F)
  @[hott] def colimit_object_iso_colimit_object (H₁ H₂ : has_colimits_of_shape D I) :
    @(colimit_object F) H₁ ≅ @(colimit_object F) H₂ :=
  iso_of_opposite_iso (limit_object_iso_limit_object Fᵒᵖᶠ H₁ H₂)

  @[hott] def colimit_functor (D I : Precategory) [H : has_colimits_of_shape D I]
    : D ^c I ⇒ D :=
  (limit_functor Dᵒᵖ Iᵒᵖ ∘f opposite_functor_opposite_left D I)ᵒᵖ'

  section bin_coproducts
  open bool prod.ops
  @[hott] def has_binary_coproducts [reducible] (D : Precategory) := has_colimits_of_shape D c2
  variables [K : has_binary_coproducts D] (d d' : D)
  include K

  @[hott] def coproduct_object : D :=
  ...
  variables {d d'}
  @[hott] def coproduct_hom {x : D} (f : d ⟶ x) (g : d' ⟶ x) : d + d' ⟶ x :=
  colimit_hom (c2_functor D d d') (bool.rec f g)
    (by intros b₁ b₂ f; induction b₁: induction b₂: esimp at *; try contradiction: apply id_right)

  @[hott] theorem coproduct_hom_inl {x : D} (f : d ⟶ x) (g : d' ⟶ x) : coproduct_hom f g ∘ !inl = f :=
  colimit_hom_commute (c2_functor D d d') (bool.rec f g) _ ff

  @[hott] theorem coproduct_hom_inr {x : D} (f : d ⟶ x) (g : d' ⟶ x) : coproduct_hom f g ∘ !inr = g :=
  colimit_hom_commute (c2_functor D d d') (bool.rec f g) _ tt

  @[hott] theorem eq_coproduct_hom {x : D} {f : d ⟶ x} {g : d' ⟶ x} {h : d + d' ⟶ x}
    (p : h ∘ !inl = f) (q : h ∘ !inr = g) : h = coproduct_hom f g :=
  eq_colimit_hom _ (bool.rec p q)

  @[hott] theorem coproduct_cocone_unique {x : D} {f : d ⟶ x} {g : d' ⟶ x}
  ...
  @[hott] def coproduct_functor : D ×c D ⇒ D :=
  functor.mk
    ...
    abstract begin intros x y z g f, symmetry, apply eq_coproduct_hom,
  ...
  @[hott] def coproduct_object_iso_coproduct_object (H₁ H₂ : has_binary_coproducts D) :
  ...
  @[hott] def has_coequalizers [reducible] (D : Precategory) := has_equalizers Dᵒᵖ
  variables [K : has_coequalizers D]
  include K

  variables {d d' x : D} (f g : d ⟶ d')
  @[hott] def coequalizer_object : D :=
  !(@equalizer_object Dᵒᵖ) f g

  @[hott] def coequalizer : d' ⟶ coequalizer_object f g :=
  !(@equalizer Dᵒᵖ)

  @[hott] theorem coequalizes : coequalizer f g ∘ f = coequalizer f g ∘ g :=
  by rexact !(@equalizes Dᵒᵖ)

  variables {f g}
  @[hott] def coequalizer_hom (h : d' ⟶ x) (p : h ∘ f = h ∘ g) : coequalizer_object f g ⟶ x :=
  !(@hom_equalizer Dᵒᵖ) proof p qed

  @[hott] theorem coequalizer_hom_coequalizer (h : d' ⟶ x) (p : h ∘ f = h ∘ g)
    : coequalizer_hom h p ∘ coequalizer f g = h :=
  by rexact !(@equalizer_hom_equalizer Dᵒᵖ)

  @[hott] theorem eq_coequalizer_hom {h : d' ⟶ x} (p : h ∘ f = h ∘ g) {i : coequalizer_object f g ⟶ x}
    (q : i ∘ coequalizer f g = h) : i = coequalizer_hom h p :=
  by rexact !(@eq_hom_equalizer Dᵒᵖ) proof q qed

  @[hott] theorem coequalizer_cocone_unique {h : d' ⟶ x} (p : h ∘ f = h ∘ g)
  ...
  omit K
  variables (f g)
  @[hott] def coequalizer_object_iso_coequalizer_object (H₁ H₂ : has_coequalizers D) :
  ...
  @[hott] def has_pushouts [reducible] (D : Precategory) := has_pullbacks Dᵒᵖ
  variables [K : has_pushouts D]
  include K

  variables {d₁ d₂ d₃ x : D} (f : d₁ ⟶ d₂) (g : d₁ ⟶ d₃)
  @[hott] def pushout_object : D :=
  !(@pullback_object Dᵒᵖ) f g

  @[hott] def pushout : d₃ ⟶ pushout_object f g :=
  !(@pullback Dᵒᵖ)

  @[hott] def pushout_rev : d₂ ⟶ pushout_object f g :=
  !(@pullback_rev Dᵒᵖ)

  @[hott] theorem pushout_commutes : pushout_rev f g ∘ f = pushout f g ∘ g :=
  by rexact !(@pullback_commutes Dᵒᵖ)

  variables {f g}
  @[hott] def pushout_hom (h₁ : d₂ ⟶ x) (h₂ : d₃ ⟶ x) (p : h₁ ∘ f = h₂ ∘ g)
    : pushout_object f g ⟶ x :=
  !(@hom_pullback Dᵒᵖ) proof p qed

  @[hott] theorem pushout_hom_pushout (h₁ : d₂ ⟶ x) (h₂ : d₃ ⟶ x) (p : h₁ ∘ f = h₂ ∘ g)
    : pushout_hom h₁ h₂ p ∘ pushout f g = h₂ :=
  by rexact !(@pullback_hom_pullback Dᵒᵖ)

  @[hott] theorem pushout_hom_pushout_rev (h₁ : d₂ ⟶ x) (h₂ : d₃ ⟶ x) (p : h₁ ∘ f = h₂ ∘ g)
    : pushout_hom h₁ h₂ p ∘ pushout_rev f g = h₁ :=
  by rexact !(@pullback_rev_hom_pullback Dᵒᵖ)

  @[hott] theorem eq_pushout_hom {h₁ : d₂ ⟶ x} {h₂ : d₃ ⟶ x} (p : h₁ ∘ f = h₂ ∘ g)
    {i : pushout_object f g ⟶ x} (q : i ∘ pushout f g = h₂) (r : i ∘ pushout_rev f g = h₁)
    : i = pushout_hom h₁ h₂ p :=
  by rexact !(@eq_hom_pullback Dᵒᵖ) proof q qed proof r qed

  @[hott] theorem pushout_cocone_unique {h₁ : d₂ ⟶ x} {h₂ : d₃ ⟶ x} (p : h₁ ∘ f = h₂ ∘ g)
    {i₁ : pushout_object f g ⟶ x} (q₁ : i₁ ∘ pushout f g = h₂) (r₁ : i₁ ∘ pushout_rev f g = h₁)
    {i₂ : pushout_object f g ⟶ x} (q₂ : i₂ ∘ pushout f g = h₂) (r₂ : i₂ ∘ pushout_rev f g = h₁)
    : i₁ = i₂ :=
  ...
  omit K
  variables (f g)
  @[hott] def pushout_object_iso_pushout_object (H₁ H₂ : has_pushouts D) :
  ...
  @[hott] def has_limits_of_shape_op_op [H : has_limits_of_shape D Iᵒᵖᵒᵖ]
