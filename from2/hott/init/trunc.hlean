...
@[hott] def of is_trunc (n-truncatedness)
...
@[hott] def has_zero_trunc_index [instance] [priority 2000] : has_zero ℕ₋₂ :=
has_zero.mk (succ (succ minus_two))

@[hott] def has_one_trunc_index [instance] [priority 2000] : has_one ℕ₋₂ :=
...
namespace trunc_index
  ...
  @[hott] def add_plus_two [reducible] (n m : ℕ₋₂) : ℕ₋₂ :=
  ...
  inductive le (a : ℕ₋₂) : ℕ₋₂ → Type _ :=
  | tr_refl : le a a
  | step : Π {b}, le a b → le a (b.+1)

end trunc_index

@[hott] def has_le_trunc_index [instance] [priority 2000] : has_le ℕ₋₂ :=
has_le.mk trunc_index.le

attribute trunc_index.add [reducible]

@[hott] def has_add_trunc_index [instance] [priority 2000] : has_add ℕ₋₂ :=
has_add.mk trunc_index.add

namespace trunc_index

  @[hott] def sub_two [reducible] (n : ℕ) : ℕ₋₂ :=
  nat.rec_on n -2 (λ n k, k.+1)

  @[hott] def add_two [reducible] (n : ℕ₋₂) : ℕ :=
  trunc_index.rec_on n nat.zero (λ n k, nat.succ k)

  postfix `.-2`:(max+1) := sub_two
  postfix `.-1`:(max+1) := λn, (n .-2 .+1)

  @[hott] def of_nat [coercion] [reducible] (n : ℕ) : ℕ₋₂ :=
  n.-2.+2
...
namespace is_trunc
  ...
  structure contr_internal (A : Type _) :=
    (center : A)
    (center_eq : Π(a : A), center = a)

  @[hott] def is_trunc_internal (n : ℕ₋₂) : Type _ → Type _ :=
...
structure is_trunc [class] (n : ℕ₋₂) (A : Type _) :=
...
namespace is_trunc
  ...
  @[hott] def is_trunc_eq [instance] [priority 1200]
  ...
  @[hott] def center_eq [H : is_contr A] (a : A) : !center = a :=
  ...
  @[hott] theorem is_contr_eq {A : Type _} [H : is_contr A] (x y : A) : is_contr (x = y) :=
  ...
  @[hott] theorem is_trunc_succ [instance] [priority 900] (A : Type _) (n : ℕ₋₂)
  ...
  @[hott] theorem is_trunc_of_le.{l} (A : Type l) {n m : ℕ₋₂} (Hnm : n ≤ m)
  ...
  @[hott] theorem is_prop_of_imp_is_contr {A : Type _} (H : A → is_contr A) : is_prop A :=
  @is_trunc_succ_intro A -2
    (λx y,
      have H2 : is_contr A, from H x,
      !is_contr_eq)

  @[hott] theorem is_prop.mk {A : Type _} (H : ∀x y : A, x = y) : is_prop A :=
  is_prop_of_imp_is_contr (λ x, is_contr.mk x (H x))

  @[hott] theorem is_prop_elim_self {A : Type _} {H : is_prop A} (x : A) : is_prop.elim x x = idp :=
  !is_prop.elim

  /- sets -/

  @[hott] theorem is_set.mk (A : Type _) (H : ∀(x y : A) (p q : x = y), p = q) : is_set A :=
  ...
  @[hott] def is_contr_sigma_eq [instance] [priority 800] {A : Type _} (a : A)
    : is_contr (Σ(x : A), a = x) :=
  ...
  @[hott] def is_trunc_unit [instance] (n : ℕ₋₂) : is_trunc n unit :=
  !is_trunc_of_is_contr

  @[hott] def is_trunc_empty [instance] (n : ℕ₋₂) : is_trunc (n.+1) empty :=
  ...
  @[hott] theorem is_trunc_is_equiv_closed (n : ℕ₋₂) (f : A → B) [H : is_equiv f]
  ...
  @[hott] def is_trunc_lift [instance] [priority 1450] (A : Type _) (n : ℕ₋₂)
  ...
  @[hott] def is_trunc_pathover [instance]
  ...
  variables {p c c₂}
  @[hott] theorem is_set.elimo (q q' : c =[p] c₂) [H : is_set (C a)] : q = q' :=
  !is_prop.elim
...
notation n `-Type _` := trunctype n
abbreviation Prop := -1-Type _
abbreviation Set  := 0-Type _
...
namespace is_trunc
  ...
  @[hott] def tlift.{u v} {n : ℕ₋₂} (A : trunctype.{u} n)
    : trunctype.{max u v} n :=
