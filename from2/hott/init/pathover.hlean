...
variables {A A' : Type _} {B B' : A → Type _} {B'' : A' → Type _} {C : Π⦃a⦄, B a → Type _}
...
namespace eq
  ...
  @[hott] def idpo [reducible] : b =[refl a] b :=
  pathover.idpatho b

  @[hott] def idpatho [reducible] (b : B a) : b =[refl a] b :=
  pathover.idpatho b
  ...
  @[hott] def cono_idpo (r : b =[p] b₂) : r ⬝o idpo = r :=
  by reflexivity

  @[hott] def idpo_cono (r : b =[p] b₂) : idpo ⬝o r =[idp_con p] r :=
  by induction r; constructor
  ...
  @[hott] def cono.right_inv (r : b =[p] b₂) : r ⬝o r⁻¹ᵒ =[!con.right_inv] idpo :=
  by induction r; constructor

  @[hott] def cono.left_inv (r : b =[p] b₂) : r⁻¹ᵒ ⬝o r =[!con.left_inv] idpo :=
  by induction r; constructor

  @[hott] def eq_of_pathover {a' a₂' : A'} (q : a' =[p] a₂') : a' = a₂' :=
  by cases q;reflexivity

  @[hott] def pathover_of_eq (p : a = a₂) {a' a₂' : A'} (q : a' = a₂') : a' =[p] a₂' :=
  by cases p;cases q;constructor

  @[hott] def pathover_constant (p : a = a₂) (a' a₂' : A') : a' =[p] a₂' ≃ a' = a₂' :=
  begin
    fapply equiv.MK,
  ...
  @[hott] def pathover_of_eq_tr_constant_inv (p : a = a₂) (a' : A')
    : pathover_of_eq p (tr_constant p a')⁻¹ = pathover_tr p a' :=
  by cases p; constructor

  @[hott] def eq_of_pathover_idp {b' : B a} (q : b =[idpath a] b') : b = b' :=
  tr_eq_of_pathover q

  --should B be explicit in the next two definitions?
  @[hott] def pathover_idp_of_eq {b' : B a} (q : b = b') : b =[idpath a] b' :=
  pathover_of_tr_eq q

  @[hott] def pathover_idp (b : B a) (b' : B a) : b =[idpath a] b' ≃ b = b' :=
  equiv.MK eq_of_pathover_idp
           (pathover_idp_of_eq)
           (to_right_inv !pathover_equiv_tr_eq)
           (to_left_inv !pathover_equiv_tr_eq)

  @[hott] def eq_of_pathover_idp_pathover_of_eq {A X : Type _} (x : X) {a a' : A} (p : a = a') :
  ...
  -- @[hott] def eq_of_pathover_idp [reducible] {b' : B a} (q : b =[idpath a] b') : b = b' :=
  -- to_fun !pathover_idp q

  -- @[hott] def pathover_idp_of_eq [reducible] {b' : B a} (q : b = b') : b =[idpath a] b' :=
  -- to_inv !pathover_idp q

  @[hott] def idp_rec_on [recursor] {P : Π⦃b₂ : B a⦄, b =[idpath a] b₂ → Type _}
  ...
  @[hott] def rec_on_right [recursor] {P : Π⦃b₂ : B a₂⦄, b =[p] b₂ → Type _}
    {b₂ : B a₂} (r : b =[p] b₂) (H : P !pathover_tr) : P r :=
  by cases r; exact H

  @[hott] def rec_on_left [recursor] {P : Π⦃b : B a⦄, b =[p] b₂ → Type _}
  ...
  @[hott] def apod11 {f : Πb, C b} {g : Πb₂, C b₂} (r : f =[p] g)
    {b : B a} {b₂ : B a₂} (q : b =[p] b₂) : f b =[apd011 C p q] g b₂ :=
  by cases r; apply (idp_rec_on q); constructor

  @[hott] def apdo10 {f : Πb, C b} {g : Πb₂, C b₂} (r : f =[p] g)
    (b : B a) : f b =[apd011 C p !pathover_tr] g (p ▸ b) :=
  by cases r; constructor

  @[hott] def apo10 {f : B a → B' a} {g : B a₂ → B' a₂} (r : f =[p] g)
    (b : B a) : f b =[p] g (p ▸ b) :=
  by cases r; constructor

  @[hott] def apo10_constant_right {f : B a → A'} {g : B a₂ → A'} (r : f =[p] g)
    (b : B a) : f b = g (p ▸ b) :=
  by cases r; constructor

  @[hott] def apo10_constant_left {f : A' → B a} {g : A' → B a₂} (r : f =[p] g)
    (a' : A') : f a' =[p] g a' :=
  by cases r; constructor

  @[hott] def apo11 {f : B a → B' a} {g : B a₂ → B' a₂} (r : f =[p] g)
    (q : b =[p] b₂) : f b =[p] g b₂ :=
  ...
  @[hott] def pathover_tro (q : b =[p] b₂) (c : C b) : c =[apd011 C p q] q ▸o c :=
  by induction q; constructor
  ...
  @[hott] def pathover_of_fn_pathover_fn (f : Π{a}, B a ≃ B' a) (r : f b =[p] f b₂) : b =[p] b₂ :=
