...
namespace nat
  ...
  @[hott] def nat_has_mul [instance] [priority nat.prio] : has_mul nat :=
  has_mul.mk nat.mul
  ...
  @[hott] def le_succ_iff_unit [simp] (n : ℕ) : n ≤ succ n ↔ unit :=
  ...
  @[hott] theorem self_lt_succ (n : ℕ) : n < succ n := !nat.le_refl

  @[hott] theorem self_lt_succ_iff_unit [simp] (n : ℕ) : n < succ n ↔ unit :=
  iff_unit_intro (self_lt_succ n)

  @[hott] theorem lt.base (n : ℕ) : n < succ n := !nat.le_refl
  ...
  @[hott] theorem lt_of_succ_lt {a b : ℕ} : succ a < b → a < b :=
  le_of_succ_le

  @[hott] theorem lt_of_succ_lt_succ {a b : ℕ} : succ a < succ b → a < b :=
  le_of_succ_le_succ

  @[hott] def decidable_le [instance] [priority nat.prio] : Π a b : nat, decidable (a ≤ b)  :=
  ...
  @[hott] theorem lt_succ_of_le {a b : ℕ} : a ≤ b → a < succ b :=
  succ_le_succ

  @[hott] theorem lt_of_succ_le {a b : ℕ} (h : succ a ≤ b) : a < b := h

  @[hott] theorem succ_le_of_lt {a b : ℕ} (h : a < b) : succ a ≤ b := h

  @[hott] theorem succ_sub_succ_eq_sub [simp] (a b : ℕ) : succ a - succ b = a - b :=
  nat.rec (by esimp) (λ b, ap pred) b

  @[hott] theorem sub_eq_succ_sub_succ (a b : ℕ) : a - b = succ a - succ b :=
  inverse !succ_sub_succ_eq_sub

  @[hott] theorem zero_sub_eq_zero [simp] (a : ℕ) : 0 - a = 0 :=
  ...
  @[hott] theorem sub_le_iff_unit [simp] (a b : ℕ) : a - b ≤ a ↔ unit :=
  ...
  @[hott] theorem sub_lt_succ_iff_unit [simp] (a b : ℕ) : a - b < succ a ↔ unit :=
