...
section
  ...
  private @[hott] theorem ua_isequiv_postcompose {A B : Type l} {C : Type _}
  ...
  private @[hott] def diagonal [reducible] (B : Type _) : Type _
    := Σ xy : B × B, pr₁ xy = pr₂ xy

  private @[hott] def isequiv_src_compose {A B : Type _}
  ...
  private @[hott] def isequiv_tgt_compose {A B : Type _}
...
@[hott] theorem weak_funext_of_ua : weak_funext :=
  ...
  have p : P = U,
    from @nondep_funext_from_ua A Type _ P U psim,
...
namespace funext
  @[hott] theorem is_equiv_apdt [instance] (f g : Π x, P x) : is_equiv (@apd10 A P f g) :=
...
namespace eq
  ...
  @[hott] def eq_of_homotopy [reducible] : f ~ g → f = g :=
  (@apd10 A P f g)⁻¹
  ...
  @[hott] def homotopy.rec_idp [recursor] {A : Type _} {P : A → Type _} {f : Πa, P a}
