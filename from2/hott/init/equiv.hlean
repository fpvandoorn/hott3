...
structure is_equiv [class] {A B : Type _} (f : A → B) :=
mk' ::
  (inv : B → A)
...
namespace is_equiv
  ...
  @[hott] def is_equiv_compose [Hf : is_equiv f] [Hg : is_equiv g]
    : is_equiv (g ∘ f) :=
  is_equiv.mk (g ∘ f) (f⁻¹ ∘ g⁻¹)
  ...
  variable {f}
  @[hott] def is_equiv_eq_closed [Hf : is_equiv f] (Heq : f = f') : is_equiv f' :=
  eq.rec_on Heq Hf
  end

  section
  parameters {A B : Type _} (f : A → B) (g : B → A)
            (ret : Πb, f (g b) = b) (sec : Πa, g (f a) = a)

  @[hott] def adjointify_left_inv' (a : A) : g (f a) = a :=
  ap g (ap f (inverse (sec a))) ⬝ ap g (ret (f a)) ⬝ sec a

  @[hott] theorem adjointify_adj' (a : A) : ret (f a) = ap f (adjointify_left_inv' a) :=
 ...
 ... = ((retrfa⁻¹ ⬝ fgfinvsect) ⬝ fgretrfa) ⬝ ap f (sec a)           : by rewrite ap_compose
  ...
  @[hott] def is_equiv_up [instance] (A : Type _)
    : is_equiv (up : A → lift A) :=
  adjointify up down (λa, by induction a;reflexivity) (λa, idp)

  section
  variables {A B C : Type _} (f : A → B) {f' : A → B} [Hf : is_equiv f] (g : B → C)
  include Hf
  ...
  @[hott] theorem adj_inv (b : B) : left_inv f (f⁻¹ b) = ap f⁻¹ (right_inv f b) :=
  is_equiv_rect f _
  ...
  @[hott] def is_equiv_inv [instance] [priority 500] : is_equiv f⁻¹ :=
  ...
  @[hott] def is_equiv_ap [instance] (x y : A) : is_equiv (ap f : x = y → f x = f y) :=
  adjointify
    (ap f)
    (eq_of_fn_eq_fn' f)
    (ap_eq_of_fn_eq_fn' f)
  ...
  section
  variables {A B C : Type _} {f : A → B} [Hf : is_equiv f]
  include Hf
  ...
  @[hott] def is_equiv_tr {A : Type _} (P : A → Type _) {x y : A}
    (p : x = y) : (is_equiv (transport P p)) :=
  is_equiv.mk _ (transport P p⁻¹) (tr_inv_tr p) (inv_tr_tr p) (tr_inv_tr_@[hott] lemma p)

  -- a version where the transport is a cast. Note: A and B live in the same universe here.
  @[hott] def is_equiv_cast {A B : Type _} (H : A = B) : is_equiv (cast H) :=
  is_equiv_tr (λX, X) H

  section
  variables {A : Type _} {B C : A → Type _} (f : Π{a}, B a → C a) [H : Πa, is_equiv (@f a)]
...
namespace equiv
  ...
  @[hott] def to_inv [reducible] (f : A ≃ B) : B → A := f⁻¹
  @[hott] def to_right_inv [reducible] (f : A ≃ B) (b : B) : f (f⁻¹ b) = b :=
  right_inv f b
  ...
  @[hott] def to_inv_trans [reducible] (f : A ≃ B) (g : B ≃ C)
  ...
  @[hott] def equiv_change_inv (f : A ≃ B) {f' : B → A} (Heq : f⁻¹ ~ f')
    : A ≃ B :=
  ...
  @[hott] def equiv_of_eq {A B : Type _} (p : A = B) : A ≃ B :=
  equiv.mk (cast p) !is_equiv_tr

  @[hott] def equiv_of_eq_refl [reducible] (A : Type _)
  ...
  @[hott] def eq_of_fn_eq_fn_inv (f : A ≃ B) {x y : B} (q : f⁻¹ x = f⁻¹ y) : x = y :=
  ...
  --we need this @[hott] theorem for the funext_of_ua proof
  @[hott] theorem inv_eq {A B : Type _} (eqf eqg : A ≃ B) (p : eqf = eqg) : (to_fun eqf)⁻¹ = (to_fun eqg)⁻¹ :=
  eq.rec_on p idp

  @[hott] def equiv_of_equiv_of_eq [trans] {A B C : Type _} (p : A = B) (q : B ≃ C) : A ≃ C :=
  equiv_of_eq p ⬝e q
  @[hott] def equiv_of_eq_of_equiv [trans] {A B C : Type _} (p : A ≃ B) (q : B = C) : A ≃ C :=
  p ⬝e equiv_of_eq q

  @[hott] def equiv_lift (A : Type _) : A ≃ lift A := equiv.mk up _

  @[hott] def equiv_rect (f : A ≃ B) (P : B → Type _) (g : Πa, P (f a)) (b : B) : P b :=
  right_inv f b ▸ g (f⁻¹ b)

  @[hott] def equiv_rect' (f : A ≃ B) (P : A → B → Type _) (g : Πb, P (f⁻¹ b) b) (a : A) : P a (f a) :=
  left_inv f a ▸ g (f a)
  ...
  variables {A B : Type _} (f : A ≃ B) {a : A} {b : B}
  @[hott] def to_eq_of_eq_inv (p : a = f⁻¹ b) : f a = b :=
  ap f p ⬝ right_inv f b

  @[hott] def to_eq_of_inv_eq (p : f⁻¹ b = a) : b = f a :=
  (eq_of_eq_inv p⁻¹)⁻¹

  @[hott] def to_inv_eq_of_eq (p : b = f a) : f⁻¹ b = a :=
  ap f⁻¹ p ⬝ left_inv f a

  @[hott] def to_eq_inv_of_eq (p : f a = b) : a = f⁻¹ b :=
  (inv_eq_of_eq p⁻¹)⁻¹
  ...
  variables {A : Type _} {B C : A → Type _} (f : Π{a}, B a ≃ C a)
            {g : A → A} {g' : A → A} (h : Π{a}, B (g' a) → B (g a)) (h' : Π{a}, C (g' a) → C (g a))
