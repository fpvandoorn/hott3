...
variables {A U V W X Y Z : Type _} {B : A → Type _} {C : Πa, B a → Type _} {D : Πa b, C a b → Type _}
...
namespace eq
  /-
    Naming convention:
      The @[hott] theorem which states how to construct an path between two function applications is
        api₀i₁...iₙ.
      ...
      The functions are non-dependent, except when the @[hott] theorem name contains trailing zeroes
        (where the function is dependent only in the arguments where it doesn't result in any
         transports in the @[hott] theorem statement).
      ...
      We don't prove each @[hott] theorem systematically, but prove only the ones which we actually need.
  -/

  @[hott] def homotopy2 [reducible] (f g : Πa b, C a b)         : Type _ :=
  Πa b, f a b = g a b
  @[hott] def homotopy3 [reducible] (f g : Πa b c, D a b c)     : Type _ :=
  Πa b c, f a b c = g a b c
  @[hott] def homotopy4 [reducible] (f g : Πa b c d, E a b c d) : Type _ :=
...
namespace funext
  @[hott] def is_equiv_apd100 [instance] (f g : Πa b, C a b)
  ...
  @[hott] def is_equiv_apd1000 [instance] (f g : Πa b c, D a b c)
...
attribute funext.is_equiv_apd100 funext.is_equiv_apd1000
