...
@[hott] theorem le_add_right (n k : ℕ) : n ≤ n + k :=
nat.rec !nat.le_refl (λ k, le_succ_of_le) k

@[hott] theorem le_add_left (n m : ℕ): n ≤ m + n :=
!add.comm ▸ !le_add_right

@[hott] theorem le.intro {n m k : ℕ} (h : n + k = m) : n ≤ m :=
h ▸ !le_add_right

@[hott] theorem le.elim {n m : ℕ} : n ≤ m → Σ k, n + k = m :=
...
@[hott] theorem mul_le_mul_left {n m : ℕ} (k : ℕ) (H : n ≤ m) : k * n ≤ k * m :=
...
@[hott] def nat_has_dvd [instance] [priority nat.prio] : has_dvd nat :=
...
@[hott] theorem le_of_lt_succ {m n : nat} : m < succ n → m ≤ n :=
le_of_succ_le_succ
...
@[hott] theorem by_cases_zero_pos {P : ℕ → Type _} (y : ℕ) (H0 : P 0) (H1 : Π {y : nat}, y > 0 → P y) :
  P y :=
...
@[hott] theorem eq_zero_sum_eq_of_mul_eq_mul_right  {n m k : ℕ} (H : n * m = k * m) : m = 0 ⊎ n = k :=
...
@[hott] theorem min_zero [simp] (a : ℕ) : min a 0 = 0 :=
by rewrite [min_eq_right !zero_le]

@[hott] theorem zero_min [simp] (a : ℕ) : min 0 a = 0 :=
by rewrite [min_eq_left !zero_le]

@[hott] theorem max_zero [simp] (a : ℕ) : max a 0 = a :=
by rewrite [max_eq_left !zero_le]

@[hott] theorem zero_max [simp] (a : ℕ) : max 0 a = a :=
by rewrite [max_eq_right !zero_le]

@[hott] theorem min_succ_succ [simp] (a b : ℕ) : min (succ a) (succ b) = succ (min a b) :=
sum.elim !lt_sum_ge
...
@[hott] theorem max_succ_succ [simp] (a b : ℕ) : max (succ a) (succ b) = succ (max a b) :=
sum.elim !lt_sum_ge
