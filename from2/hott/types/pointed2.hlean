...
namespace pointed
  variables {A B C : Type*} {P : A → Type _} {p₀ : P pt} {k k' l m n : ppi P p₀}
  ...
  @[hott] def psquare [reducible] (f₁₀ : A₀₀ →* A₂₀) (f₁₂ : A₀₂ →* A₂₂)
                                 (f₀₁ : A₀₀ →* A₀₂) (f₂₁ : A₂₀ →* A₂₂) : Type _ :=
  f₂₁ ∘* f₁₀ ~* f₁₂ ∘* f₀₁
  ...
  @[hott] def phdeg_square {f f' : A →* A'} (p : f ~* f') : psquare !pid !pid f f' :=
  !pcompose_pid ⬝* p⁻¹* ⬝* !pid_pcompose⁻¹*
  @[hott] def pvdeg_square {f f' : A →* A'} (p : f ~* f') : psquare f f' !pid !pid :=
  !pid_pcompose ⬝* p ⬝* !pcompose_pid⁻¹*

  variables (f₀₁ f₁₀)
  @[hott] def phrefl : psquare !pid !pid f₀₁ f₀₁ := phdeg_square phomotopy.rfl
  @[hott] def pvrefl : psquare f₁₀ f₁₀ !pid !pid := pvdeg_square phomotopy.rfl
  variables {f₀₁ f₁₀}
  @[hott] def phrfl : psquare !pid !pid f₀₁ f₀₁ := phrefl f₀₁
  @[hott] def pvrfl : psquare f₁₀ f₁₀ !pid !pid := pvrefl f₁₀
  ...
  @[hott] def punit_pmap_phomotopy {A : Type*} (f : punit →* A) :
    f ~* pconst punit A :=
  !phomotopy_of_is_contr_dom

  @[hott] def punit_ppi (P : punit → Type*) (p₀ : P ⋆) : ppi P p₀ :=
  begin
    fapply ppi.mk, intro u, induction u, exact p₀,
    reflexivity
  end

  @[hott] def punit_ppi_phomotopy {P : punit → Type*} {p₀ : P ⋆} (f : ppi P p₀) :
    f ~* punit_ppi P p₀ :=
  !phomotopy_of_is_contr_dom

  @[hott] def is_contr_punit_ppi (P : punit → Type*) (p₀ : P ⋆) : is_contr (ppi P p₀) :=
  is_contr.mk (punit_ppi P p₀) (λf, eq_of_phomotopy (punit_ppi_phomotopy f)⁻¹*)

  @[hott] def is_contr_punit_pmap (A : Type*) : is_contr (punit →* A) :=
  !is_contr_punit_ppi
  ...
  @[hott] def trans_refl (p : k ~* l) : p ⬝* phomotopy.rfl = p :=
  begin
  ...
  @[hott] def pwhisker_left_refl {A B C : Type*} (g : B →* C) (f : A →* B) :
  ...
  @[hott] def pwhisker_right_refl {A B C : Type*} (f : A →* B) (g : B →* C) :
  ...
  @[hott] def phsquare [reducible] (p₁₀ : f₀₀ ~* f₂₀) (p₁₂ : f₀₂ ~* f₂₂)
                                  (p₀₁ : f₀₀ ~* f₀₂) (p₂₁ : f₂₀ ~* f₂₂) : Type _ :=
  p₁₀ ⬝* p₂₁ = p₀₁ ⬝* p₁₂
  ...
  @[hott] def ppmap_pbool_pequiv (B : Type*) : ppmap pbool B ≃* B :=
  begin
    fapply pequiv.MK',
    { exact papply B tt },
    { exact pbool_pmap },
  ...
  @[hott] def phinverse2 {f₁₀ : A₀₀ ≃* A₂₀} {f₁₂ : A₀₂ ≃* A₂₂} {p p' : psquare f₁₀ f₁₂ f₀₁ f₂₁}
    (r : p = p') : p⁻¹ʰ* = p'⁻¹ʰ* :=
  ap phinverse r

  @[hott] def pvinverse2 {f₀₁ : A₀₀ ≃* A₀₂} {f₂₁ : A₂₀ ≃* A₂₂} {p p' : psquare f₁₀ f₁₂ f₀₁ f₂₁}
    (r : p = p') : p⁻¹ᵛ* = p'⁻¹ᵛ* :=
  ap pvinverse r
  ...
  open sigma.ops prod
  @[hott] def pequiv.sigma_char {A B : Type*} :
  ...
  @[hott] def is_contr_pright_inv (f : A ≃* B) : is_contr (Σ(g : B →* A), f ∘* g ~* pid B) :=
  begin
  ...
  @[hott] def is_contr_pleft_inv (f : A ≃* B) : is_contr (Σ(h : B →* A), h ∘* f ~* pid A) :=
  begin
    fapply is_trunc_equiv_closed,
      { exact !fiber.sigma_char ⬝e sigma_equiv_sigma_right (λg, !pmap_eq_equiv) },
    fapply is_contr_fiber_of_is_equiv,
  ...
  @[hott] def pequiv_eq_equiv (f g : A ≃* B) : (f = g) ≃ f ~* g :=
