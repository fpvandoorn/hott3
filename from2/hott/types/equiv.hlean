...
namespace is_equiv
  variables {A B : Type _} (f : A → B) [H : is_equiv f]
  include H
  /- is_equiv f is a mere proposition -/
  @[hott] def is_contr_fiber_of_is_equiv [instance] (b : B) : is_contr (fiber f b) :=
  is_contr.mk
  ...
  @[hott] theorem is_prop_is_equiv [instance] : is_prop (is_equiv f) :=
  ...
  @[hott] def is_prop_is_contr_fun (f : A → B) : is_prop (is_contr_fun f) := _

  @[hott] def is_equiv_of_is_contr_fun [H : is_contr_fun f] : is_equiv f :=
  ...
  @[hott] def is_equiv_equiv_is_contr_fun : is_equiv f ≃ is_contr_fun f :=
...
namespace is_equiv
  ...
  @[hott] def is_fiberwise_equiv [reducible] := Πa, is_equiv (f a)
...
namespace equiv
  ...
  begin
    ...
    intros x y, cases x with fx Hx, cases y with fy Hy, generalize Hy,
    apply (eq_of_homotopy (λ a, !eq_of_is_contr)) ▸ (λ Hy, !is_prop.elim ▸ rfl),
    apply equiv_of_is_contr_of_is_contr
  end

  @[hott] def is_trunc_succ_equiv (n : trunc_index) (A B : Type _)
  ...
  @[hott] def is_trunc_equiv (n : trunc_index) (A B : Type _)
