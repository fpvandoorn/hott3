...
namespace Wtype
  ...
  @[hott] def Wtype_eq (p : w.1 = w'.1) (q : w.2 =[p] w'.2) : w = w' :=
  by cases w; cases w';exact (sup_eq_sup p q)

  @[hott] def Wtype_eq_pr1 (p : w = w') : w.1 = w'.1 :=
  by cases p;exact idp

  @[hott] def Wtype_eq_pr2 (p : w = w') : w.2 =[Wtype_eq_pr1 p] w'.2 :=
  ...
  @[hott] def sup_path_W (p : w.1 = w'.1) (q : w.2 =[p] w'.2)
    : ⟨(Wtype_eq p q)..1, (Wtype_eq p q)..2⟩ = ⟨p, q⟩ :=
  by cases w; cases w'; cases q; exact idp

  @[hott] def pr1_path_W (p : w.1 = w'.1) (q : w.2 =[p] w'.2) : (Wtype_eq p q)..1 = p :=
  !sup_path_W..1

  @[hott] def pr2_path_W (p : w.1 = w'.1) (q : w.2 =[p] w'.2)
      : (Wtype_eq p q)..2 =[pr1_path_W p q] q :=
  !sup_path_W..2

  @[hott] def eta_path_W (p : w = w') : Wtype_eq (p..1) (p..2) = p :=
  by cases p; cases w; exact idp

  @[hott] def transport_pr1_path_W {B' : A → Type _} (p : w.1 = w'.1) (q : w.2 =[p] w'.2)
      : transport (λx, B' x.1) (Wtype_eq p q) = transport B' p :=
  by cases w; cases w'; cases q; exact idp

  @[hott] def path_W_uncurried (pq : Σ(p : w.1 = w'.1), w.2 =[p] w'.2) : w = w' :=
  by cases pq with p q; exact (Wtype_eq p q)

  @[hott] def sup_path_W_uncurried (pq : Σ(p : w.1 = w'.1), w.2 =[p] w'.2)
      :  ⟨(path_W_uncurried pq)..1, (path_W_uncurried pq)..2⟩ = pq :=
  by cases pq with p q; exact (sup_path_W p q)

  @[hott] def pr1_path_W_uncurried (pq : Σ(p : w.1 = w'.1), w.2 =[p] w'.2)
      : (path_W_uncurried pq)..1 = pq.1 :=
  !sup_path_W_uncurried..1

  @[hott] def pr2_path_W_uncurried (pq : Σ(p : w.1 = w'.1), w.2 =[p] w'.2)
      : (path_W_uncurried pq)..2 =[pr1_path_W_uncurried pq] pq.2 :=
  !sup_path_W_uncurried..2

  @[hott] def eta_path_W_uncurried (p : w = w') : path_W_uncurried ⟨p..1, p..2⟩ = p :=
  !eta_path_W

  @[hott] def transport_pr1_path_W_uncurried {B' : A → Type _} (pq : Σ(p : w.1 = w'.1), w.2 =[p] w'.2)
  ...
  @[hott] def equiv_path_W (w w' : W a, B a) : (Σ(p : w.1 = w'.1), w.2 =[p] w'.2) ≃ (w = w') :=
  ...
  /- truncatedness -/
  open is_trunc pi
  @[hott] def is_trunc_W [instance] (n : trunc_index)
