...
namespace sum
  ...
  down (sum.encode p)
  @[hott] def empty_of_inl_eq_inr (p : inl a = inr b) : empty := down (sum.encode p)
  @[hott] def empty_of_inr_eq_inl (p : inr b = inl a) : empty := down (sum.encode p)
  ...
  variables {A' B' : Type _} (f : A → A') (g : B → B')
  @[hott] def sum_functor : A + B → A' + B'
  | sum_functor (inl a) := inl (f a)
  | sum_functor (inr b) := inr (g b)

  /- Equivalences -/

  @[hott] def is_equiv_sum_functor [instance] [Hf : is_equiv f] [Hg : is_equiv g]
  ...
  @[hott] def sum_equiv_sum_of_is_equiv [Hf : is_equiv f] [Hg : is_equiv g]
    : A + B ≃ A' + B' :=
  ...
  variables (A B)
  @[hott] theorem is_trunc_sum (n : trunc_index) [HA : is_trunc (n.+2) A]  [HB : is_trunc (n.+2) B]
    : is_trunc (n.+2) (A + B) :=
  begin
  ...
  @[hott] theorem is_trunc_sum_excluded (n : trunc_index) [HA : is_trunc n A]  [HB : is_trunc n B]
  ...
  variable {B}
  @[hott] def is_contr_sum_left [HA : is_contr A] (H : ¬B) : is_contr (A + B) :=
...
namespace decidable
  ...
  @[hott] def is_trunc_decidable (A : Type _) (n : trunc_index) [H : is_trunc n A] :
...
@[hott] def tsum {n : trunc_index} (A B : (n.+2)-Type _) : (n.+2)-Type _ :=
trunctype.mk (A + B) _
