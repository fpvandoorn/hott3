...
namespace arrow
  ...
  @[hott] def morphism_of_arrow [coercion] (f : arrow) : morphism (dom f) (cod f) :=
  ...
  @[hott] def on_fiber [reducible] (r : arrow_hom f g) (y : cod f)
    : fiber f y → fiber g (on_cod r y) :=
  fiber.rec (λx p, fiber.mk (on_dom r x) (commute r x ⬝ ap (on_cod r) p))

  structure is_retraction [class] (r : arrow_hom f g) : Type _ :=
    (sect : arrow_hom g f)
  ...
  @[hott] def retraction_on_fiber [reducible] (r : arrow_hom f g) [H : is_retraction r]
  ...
  -- @[hott] lemma 4.7.3
  @[hott] def retraction_on_fiber_is_retraction [instance] (r : arrow_hom f g) [H : is_retraction r]
...
namespace arrow
  ...
  variables {A A' B B' : Type _} (f : A → B) (f' : A' → B')
            (α : A → A') (β : B → B')
            [Hf : is_equiv f] [Hf' : is_equiv f']
  include Hf Hf'

  open function
  @[hott] def inv_commute_of_commute (p : f' ∘ α ~ β ∘ f) : f'⁻¹ ∘ β ~ α ∘ f⁻¹ :=
  begin
