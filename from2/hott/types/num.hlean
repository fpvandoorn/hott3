...
namespace pos_num
  @[hott] theorem succ_not_is_one (a : pos_num) : is_one (succ a) = ff :=
  pos_num.rec_on a rfl (take n iH, rfl) (take n iH, rfl)

  @[hott] theorem succ_one                : succ one = bit0 one
  @[hott] theorem succ_bit1 (a : pos_num) : succ (bit1 a) = bit0 (succ a)
  @[hott] theorem succ_bit0 (a : pos_num) : succ (bit0 a) = bit1 a

  @[hott] theorem ne_of_bit0_ne_bit0 {a b : pos_num} (H₁ : bit0 a ≠ bit0 b) : a ≠ b :=
  suppose a = b,
  absurd rfl (this ▸ H₁)

  @[hott] theorem ne_of_bit1_ne_bit1 {a b : pos_num} (H₁ : bit1 a ≠ bit1 b) : a ≠ b :=
  suppose a = b,
  absurd rfl (this ▸ H₁)

  @[hott] theorem pred_succ : Π (a : pos_num), pred (succ a) = a
  | one      := rfl
  ...
  @[hott] theorem one_add_one     : one + one = bit0 one
  @[hott] theorem one_add_bit0    : one + (bit0 a) = bit1 a
  @[hott] theorem one_add_bit1    : one + (bit1 a) = succ (bit1 a)
  @[hott] theorem bit0_add_one    : (bit0 a) + one = bit1 a
  @[hott] theorem bit1_add_one    : (bit1 a) + one = succ (bit1 a)
  @[hott] theorem bit0_add_bit0   : (bit0 a) + (bit0 b) = bit0 (a + b)
  @[hott] theorem bit0_add_bit1   : (bit0 a) + (bit1 b) = bit1 (a + b)
  @[hott] theorem bit1_add_bit0   : (bit1 a) + (bit0 b) = bit1 (a + b)
  @[hott] theorem bit1_add_bit1   : (bit1 a) + (bit1 b) = succ (bit1 (a + b))
  @[hott] theorem one_mul         : one * a = a
  end

  @[hott] theorem mul_one         : Π a, a * one = a
  | one      := rfl
  | (bit1 n) :=
  ...
  @[hott] theorem decidable_eq [instance] : Π (a b : pos_num), decidable (a = b)
  | one      one      := inl rfl
  ...
  @[hott] theorem lt_one_right_eq_ff : Π a : pos_num, a ≮ one
  | one      := rfl
  | (bit0 a) := rfl
  | (bit1 a) := rfl

  @[hott] theorem lt_one_succ_eq_tt : Π a : pos_num, one < succ a
  | one      := rfl
  | (bit0 a) := rfl
  | (bit1 a) := rfl

  @[hott] theorem lt_of_lt_bit0_bit0 {a b : pos_num} (H : bit0 a < bit0 b) : a < b := H
  @[hott] theorem lt_of_lt_bit0_bit1 {a b : pos_num} (H : bit1 a < bit0 b) : a < b := H
  @[hott] theorem lt_of_lt_bit1_bit1 {a b : pos_num} (H : bit1 a < bit1 b) : a < b := H
  @[hott] theorem lt_of_lt_bit1_bit0 {a b : pos_num} (H : bit0 a < bit1 b) : a < succ b := H

  @[hott] theorem lt_bit0_bit0_eq_lt (a b : pos_num) : lt (bit0 a) (bit0 b) = lt a b :=
  rfl

  @[hott] theorem lt_bit1_bit1_eq_lt (a b : pos_num) : lt (bit1 a) (bit1 b) = lt a b :=
  rfl

  @[hott] theorem lt_bit1_bit0_eq_lt (a b : pos_num) : lt (bit1 a) (bit0 b) = lt a b :=
  rfl

  @[hott] theorem lt_bit0_bit1_eq_lt_succ (a b : pos_num) : lt (bit0 a) (bit1 b) = lt a (succ b) :=
  rfl

  @[hott] theorem lt_irrefl : Π (a : pos_num), a ≮ a
  | one      := rfl
  | (bit0 a) :=
    begin
  ...
  @[hott] theorem ne_of_lt_eq_tt : Π {a b : pos_num}, a < b → a = b → empty
  ...
  @[hott] theorem lt_base : Π a : pos_num, a < succ a
  | one      := rfl
  | (bit0 a) :=
    begin
  ...
  @[hott] theorem lt_step : Π {a b : pos_num}, a < b → a < succ b
  | one      one      H := rfl
  | one      (bit0 b) H := rfl
  | one      (bit1 b) H := rfl
  ...
  @[hott] theorem lt_of_lt_succ_succ : Π {a b : pos_num}, succ a < succ b → a < b
  ...
  @[hott] theorem lt_succ_succ : Π {a b : pos_num}, a < b → succ a < succ b
  | one      one      H := absurd H ff_ne_tt
  | one      (bit0 b) H :=
    begin
  ...
  @[hott] theorem lt_of_lt_succ : Π {a b : pos_num}, succ a < b → a < b
  ...
  @[hott] theorem lt_of_lt_succ_of_ne : Π {a b : pos_num}, a < succ b → a ≠ b → a < b
  ...
  @[hott] theorem lt_trans : Π {a b c : pos_num}, a < b → b < c → a < c
  ...
  @[hott] theorem lt_antisymm : Π {a b : pos_num}, a < b → b ≮ a
  | one      one      H := rfl
  | one      (bit0 b) H := rfl
  | one      (bit1 b) H := rfl
  | (bit0 a) one      H := absurd H ff_ne_tt
  ...
  @[hott] theorem le_refl : Π a : pos_num, a ≤ a :=
  lt_base

  @[hott] theorem le_eq_lt_succ {a b : pos_num} : le a b = lt a (succ b) :=
  rfl

  @[hott] theorem not_lt_of_le : Π {a b : pos_num}, a ≤ b → b < a → empty
  ...
  @[hott] theorem le_antisymm : Π {a b : pos_num}, a ≤ b → b ≤ a → a = b
  | one      one      H₁ H₂ := rfl
  | one      (bit0 b) H₁ H₂ :=
  ...
  @[hott] theorem le_trans {a b c : pos_num} : a ≤ b → b ≤ c → a ≤ c :=
  begin
    intros H₁ H₂,
...
namespace num
  ...
  @[hott] theorem decidable_eq [instance] : Π (a b : num), decidable (a = b)
  | zero zero       := inl rfl
