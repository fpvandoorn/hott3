...
inductive list (T : Type _) : Type _ :=
| nil {} : list T
| cons   : T → list T → list T

@[hott] def pointed_list [instance] (A : Type _) : pointed (list A) :=
pointed.mk list.nil
...
@[hott] lemma cons_ne_nil (a : T) (l : list T) : a::l ≠ [] :=
by contradiction

@[hott] lemma head_eq_of_cons_eq {A : Type _} {h₁ h₂ : A} {t₁ t₂ : list A} :
      (h₁::t₁) = (h₂::t₂) → h₁ = h₂ :=
assume Peq, down (list.no_confusion Peq (assume Pheq Pteq, Pheq))

@[hott] lemma tail_eq_of_cons_eq {A : Type _} {h₁ h₂ : A} {t₁ t₂ : list A} :
      (h₁::t₁) = (h₂::t₂) → t₁ = t₂ :=
assume Peq, down (list.no_confusion Peq (assume Pheq Pteq, Pteq))

/- append -/

@[hott] def append : list T → list T → list T
| []       l := l
| (h :: s) t := h :: (append s t)

notation l₁ ++ l₂ := append l₁ l₂

@[hott] theorem append_nil_left (t : list T) : [] ++ t = t := idp

@[hott] theorem append_cons (x : T) (s t : list T) : (x::s) ++ t = x :: (s ++ t) := idp

@[hott] theorem append_nil_right : ∀ (t : list T), t ++ [] = t
| []       := rfl
| (a :: l) := calc
  (a :: l) ++ [] = a :: (l ++ []) : rfl
             ... = a :: l         : append_nil_right l

@[hott] theorem append.assoc : ∀ (s t u : list T), s ++ t ++ u = s ++ (t ++ u)
| []       t u := rfl
| (a :: l) t u :=
...
/- length -/
@[hott] def length : list T → nat
| []       := 0
| (a :: l) := length l + 1

@[hott] theorem length_nil : length (@nil T) = 0 := idp

@[hott] theorem length_cons (x : T) (t : list T) : length (x::t) = length t + 1 := idp

@[hott] theorem length_append : ∀ (s t : list T), length (s ++ t) = length s + length t
| []       t := calc
...
@[hott] theorem eq_nil_of_length_eq_zero : ∀ {l : list T}, length l = 0 → l = []
| []     H := rfl
| (a::s) H := by contradiction

@[hott] theorem ne_nil_of_length_eq_succ : ∀ {l : list T} {n : nat}, length l = succ n → l ≠ []
...
@[hott] def concat : Π (x : T), list T → list T
| a []       := [a]
| a (b :: l) := b :: concat a l

@[hott] theorem concat_nil (x : T) : concat x [] = [x] := idp

@[hott] theorem concat_cons (x y : T) (l : list T) : concat x (y::l)  = y::(concat x l) := idp

@[hott] theorem concat_eq_append (a : T) : ∀ (l : list T), concat a l = l ++ [a]
| []       := rfl
| (b :: l) :=
  show b :: (concat a l) = (b :: l) ++ (a :: []),
  by rewrite concat_eq_append

@[hott] theorem concat_ne_nil (a : T) : ∀ (l : list T), concat a l ≠ [] :=
by intro l; induction l; repeat contradiction

@[hott] theorem length_concat (a : T) : ∀ (l : list T), length (concat a l) = length l + 1
| []      := rfl
| (x::xs) := by rewrite [concat_cons, *length_cons, length_concat]

@[hott] theorem concat_append (a : T) : ∀ (l₁ l₂ : list T), concat a l₁ ++ l₂ = l₁ ++ a :: l₂
| []      := λl₂, rfl
| (x::xs) := λl₂, begin rewrite [concat_cons,append_cons, concat_append] end

@[hott] theorem append_concat (a : T)  : ∀(l₁ l₂ : list T), l₁ ++ concat a l₂ = concat a (l₁ ++ l₂)
| []      := λl₂, rfl
| (x::xs) := λl₂, begin rewrite [+append_cons, concat_cons, append_concat] end

/- last -/

@[hott] def last : Π l : list T, l ≠ [] → T
| []          h := absurd rfl h
| [a]         h := a
| (a₁::a₂::l) h := last (a₂::l) !cons_ne_nil

@[hott] lemma last_singleton (a : T) (h : [a] ≠ []) : last [a] h = a :=
rfl

@[hott] lemma last_cons_cons (a₁ a₂ : T) (l : list T) (h : a₁::a₂::l ≠ [])
  : last (a₁::a₂::l) h = last (a₂::l) !cons_ne_nil :=
rfl

@[hott] theorem last_congr {l₁ l₂ : list T} (h₁ : l₁ ≠ []) (h₂ : l₂ ≠ []) (h₃ : l₁ = l₂)
  : last l₁ h₁ = last l₂ h₂ :=
apd011 last h₃ !is_prop.elimo

@[hott] theorem last_concat {x : T} : ∀ {l : list T} (h : concat x l ≠ []), last (concat x l) h = x
| []          h := rfl
| [a]         h := rfl
| (a₁::a₂::l) h :=
  begin
...
@[hott] def reverse : list T → list T
| []       := []
| (a :: l) := concat a (reverse l)

@[hott] theorem reverse_nil : reverse (@nil T) = [] := idp

@[hott] theorem reverse_cons (x : T) (l : list T) : reverse (x::l) = concat x (reverse l) := idp

@[hott] theorem reverse_singleton (x : T) : reverse [x] = [x] := idp

@[hott] theorem reverse_append : ∀ (s t : list T), reverse (s ++ t) = (reverse t) ++ (reverse s)
| []         t2 := calc
...
@[hott] theorem reverse_reverse : ∀ (l : list T), reverse (reverse l) = l
| []       := rfl
| (a :: l) := calc
...
@[hott] theorem concat_eq_reverse_cons (x : T) (l : list T) : concat x l = reverse (x :: reverse l) :=
calc
  concat x l = concat x (reverse (reverse l)) : reverse_reverse
         ... = reverse (x :: reverse l)       : rfl

@[hott] theorem length_reverse : ∀ (l : list T), length (reverse l) = length l
| []      := rfl
| (x::xs) := begin unfold reverse, rewrite [length_concat, length_cons, length_reverse] end

/- head and tail -/

@[hott] def head [h : pointed T] : list T → T
| []       := pt
| (a :: l) := a

@[hott] theorem head_cons [h : pointed T] (a : T) (l : list T) : head (a::l) = a := idp

@[hott] theorem head_append [h : pointed T] (t : list T) : ∀ {s : list T}, s ≠ [] → head (s ++ t) = head s
| []       H := absurd rfl H
| (a :: s) H :=
  show head (a :: (s ++ t)) = head (a :: s),
  by  rewrite head_cons

@[hott] def tail : list T → list T
| []       := []
| (a :: l) := l

@[hott] theorem tail_nil : tail (@nil T) = [] := idp

@[hott] theorem tail_cons (a : T) (l : list T) : tail (a::l) = l := idp

@[hott] theorem cons_head_tail [h : pointed T] {l : list T} : l ≠ [] → (head l)::(tail l) = l :=
list.cases_on l
...
@[hott] theorem mem_nil_iff (x : T) : x ∈ [] ↔ empty :=
iff.intro down up

@[hott] theorem not_mem_nil (x : T) : x ∉ [] :=
iff.mp !mem_nil_iff

@[hott] theorem mem_cons (x : T) (l : list T) : x ∈ x :: l :=
sum.inl rfl

@[hott] theorem mem_cons_of_mem (y : T) {x : T} {l : list T} : x ∈ l → x ∈ y :: l :=
assume H, sum.inr H

@[hott] theorem mem_cons_iff (x y : T) (l : list T) : x ∈ y::l ↔ (x = y ⊎ x ∈ l) :=
iff.rfl

@[hott] theorem eq_or_mem_of_mem_cons {x y : T} {l : list T} : x ∈ y::l → x = y ⊎ x ∈ l :=
assume h, h

@[hott] theorem mem_singleton {x a : T} : x ∈ [a] → x = a :=
suppose x ∈ [a], sum.rec_on (eq_or_mem_of_mem_cons this)
  (suppose x = a, this)
  (suppose x ∈ [], absurd this !not_mem_nil)

@[hott] theorem mem_of_mem_cons_of_mem {a b : T} {l : list T} : a ∈ b::l → b ∈ l → a ∈ l :=
assume ainbl binl, sum.rec_on (eq_or_mem_of_mem_cons ainbl)
  (suppose a = b, by substvars; exact binl)
  (suppose a ∈ l, this)

@[hott] theorem mem_or_mem_of_mem_append {x : T} {s t : list T} : x ∈ s ++ t → x ∈ s ⊎ x ∈ t :=
...
@[hott] theorem mem_append_of_mem_or_mem {x : T} {s t : list T} : (x ∈ s ⊎ x ∈ t) → x ∈ s ++ t :=
list.rec_on s
...
@[hott] theorem mem_append_left {a : T} {l₁ : list T} (l₂ : list T) : a ∈ l₁ → a ∈ l₁ ++ l₂ :=
assume ainl₁, mem_append_of_mem_or_mem (sum.inl ainl₁)

@[hott] theorem mem_append_right {a : T} (l₁ : list T) {l₂ : list T} : a ∈ l₂ → a ∈ l₁ ++ l₂ :=
assume ainl₂, mem_append_of_mem_or_mem (sum.inr ainl₂)

@[hott] def decidable_mem [instance] [H : decidable_eq T] (x : T) (l : list T) : decidable (x ∈ l) :=
list.rec_on l
...
@[hott] theorem mem_of_ne_of_mem {x y : T} {l : list T} (H₁ : x ≠ y) (H₂ : x ∈ y :: l) : x ∈ l :=
sum.rec_on (eq_or_mem_of_mem_cons H₂) (λe, absurd e H₁) (λr, r)

@[hott] theorem ne_of_not_mem_cons {a b : T} {l : list T} : a ∉ b::l → a ≠ b :=
assume nin aeqb, absurd (sum.inl aeqb) nin

@[hott] theorem not_mem_of_not_mem_cons {a b : T} {l : list T} : a ∉ b::l → a ∉ l :=
assume nin nainl, absurd (sum.inr nainl) nin

@[hott] lemma not_mem_cons_of_ne_of_not_mem {x y : T} {l : list T} : x ≠ y → x ∉ l → x ∉ y::l :=
assume P1 P2, not.intro (assume Pxin, absurd (eq_or_mem_of_mem_cons Pxin) (not_sum P1 P2))

@[hott] lemma ne_and_not_mem_of_not_mem_cons {x y : T} {l : list T} : x ∉ y::l → x ≠ y × x ∉ l :=
assume P, prod.mk (ne_of_not_mem_cons P) (not_mem_of_not_mem_cons P)

@[hott] def sublist (l₁ l₂ : list T) := ∀ ⦃a : T⦄, a ∈ l₁ → a ∈ l₂

infix ⊆ := sublist

@[hott] theorem nil_sub (l : list T) : [] ⊆ l :=
λ b i, empty.elim (iff.mp (mem_nil_iff b) i)

@[hott] theorem sub.refl (l : list T) : l ⊆ l :=
λ b i, i

@[hott] theorem sub.trans {l₁ l₂ l₃ : list T} (H₁ : l₁ ⊆ l₂) (H₂ : l₂ ⊆ l₃) : l₁ ⊆ l₃ :=
λ b i, H₂ (H₁ i)

@[hott] theorem sub_cons (a : T) (l : list T) : l ⊆ a::l :=
λ b i, sum.inr i

@[hott] theorem sub_of_cons_sub {a : T} {l₁ l₂ : list T} : a::l₁ ⊆ l₂ → l₁ ⊆ l₂ :=
λ s b i, s b (mem_cons_of_mem _ i)

@[hott] theorem cons_sub_cons  {l₁ l₂ : list T} (a : T) (s : l₁ ⊆ l₂) : (a::l₁) ⊆ (a::l₂) :=
λ b Hin, sum.rec_on (eq_or_mem_of_mem_cons Hin)
  (λ e : b = a,  sum.inl e)
  (λ i : b ∈ l₁, sum.inr (s i))

@[hott] theorem sub_append_left (l₁ l₂ : list T) : l₁ ⊆ l₁++l₂ :=
λ b i, iff.mpr (mem_append_iff b l₁ l₂) (sum.inl i)

@[hott] theorem sub_append_right (l₁ l₂ : list T) : l₂ ⊆ l₁++l₂ :=
λ b i, iff.mpr (mem_append_iff b l₁ l₂) (sum.inr i)

@[hott] theorem sub_cons_of_sub (a : T) {l₁ l₂ : list T} : l₁ ⊆ l₂ → l₁ ⊆ (a::l₂) :=
λ (s : l₁ ⊆ l₂) (x : T) (i : x ∈ l₁), sum.inr (s i)

@[hott] theorem sub_app_of_sub_left (l l₁ l₂ : list T) : l ⊆ l₁ → l ⊆ l₁++l₂ :=
λ (s : l ⊆ l₁) (x : T) (xinl : x ∈ l),
  have x ∈ l₁, from s xinl,
  mem_append_of_mem_or_mem (sum.inl this)

@[hott] theorem sub_app_of_sub_right (l l₁ l₂ : list T) : l ⊆ l₂ → l ⊆ l₁++l₂ :=
λ (s : l ⊆ l₂) (x : T) (xinl : x ∈ l),
  have x ∈ l₂, from s xinl,
  mem_append_of_mem_or_mem (sum.inr this)

@[hott] theorem cons_sub_of_sub_of_mem {a : T} {l m : list T} : a ∈ m → l ⊆ m → a::l ⊆ m :=
...
@[hott] theorem app_sub_of_sub_of_sub {l₁ l₂ l : list T} : l₁ ⊆ l → l₂ ⊆ l → l₁++l₂ ⊆ l :=
...
@[hott] def find : T → list T → nat
| a []       := 0
| a (b :: l) := if a = b then 0 else succ (find a l)

@[hott] theorem find_nil (x : T) : find x [] = 0 := idp

@[hott] theorem find_cons (x y : T) (l : list T) : find x (y::l) = if x = y then 0 else succ (find x l) :=
idp

@[hott] theorem find_cons_of_eq {x y : T} (l : list T) : x = y → find x (y::l) = 0 :=
assume e, if_pos e

@[hott] theorem find_cons_of_ne {x y : T} (l : list T) : x ≠ y → find x (y::l) = succ (find x l) :=
assume n, if_neg n

/-@[hott] theorem find_of_not_mem {l : list T} {x : T} : ¬x ∈ l → find x l = length l :=
list.rec_on l
   (suppose ¬x ∈ [], _)
   (take y l,
...
@[hott] lemma find_le_length : ∀ {a} {l : list T}, find a l ≤ length l
| a []        := !le.refl
...
/-@[hott] lemma not_mem_of_find_eq_length : ∀ {a} {l : list T}, find a l = length l → a ∉ l
...
/-@[hott] lemma find_lt_length {a} {l : list T} (Pin : a ∈ l) : find a l < length l :=
begin
...
/- nth element -/
section nth
@[hott] def nth : list T → nat → option T
| []       n     := none
| (a :: l) 0     := some a
| (a :: l) (n+1) := nth l n

@[hott] theorem nth_zero (a : T) (l : list T) : nth (a :: l) 0 = some a := idp

@[hott] theorem nth_succ (a : T) (l : list T) (n : nat) : nth (a::l) (succ n) = nth l n := idp

@[hott] theorem nth_eq_some : ∀ {l : list T} {n : nat}, n < length l → Σ a : T, nth l n = some a
...
open decidable
@[hott] theorem find_nth [h : decidable_eq T] {a : T} : ∀ {l}, a ∈ l → nth l (find a l) = some a
...
@[hott] def inth [h : pointed T] (l : list T) (n : nat) : T :=
match nth l n with
| some a := a
| none   := pt
end

@[hott] theorem inth_zero [h : pointed T] (a : T) (l : list T) : inth (a :: l) 0 = a := idp

@[hott] theorem inth_succ [h : pointed T] (a : T) (l : list T) (n : nat) : inth (a::l) (n+1) = inth l n :=
idp
end nth

section ith
@[hott] def ith : Π (l : list T) (i : nat), i < length l → T
| nil     i        h := absurd h !not_lt_zero
| (x::xs) 0        h := x
| (x::xs) (succ i) h := ith xs i (lt_of_succ_lt_succ h)

@[hott] lemma ith_zero (a : T) (l : list T) (h : 0 < length (a::l)) : ith (a::l) 0 h = a :=
rfl

@[hott] lemma ith_succ (a : T) (l : list T) (i : nat) (h : succ i < length (a::l))
                      : ith (a::l) (succ i) h = ith l i (lt_of_succ_lt_succ h) :=
rfl
end ith

open decidable
@[hott] def has_decidable_eq {A : Type _} [H : decidable_eq A] : ∀ l₁ l₂ : list A, decidable (l₁ = l₂)
| []      []      := inl rfl
...
@[hott] theorem qeq_app : ∀ (l₁ : list A) (a : A) (l₂ : list A), l₁++(a::l₂) ≈ a|l₁++l₂
| []      a l₂ := qhead a l₂
| (x::xs) a l₂ := qcons x (qeq_app xs a l₂)

@[hott] theorem mem_head_of_qeq {a : A} {l₁ l₂ : list A} : l₁≈a|l₂ → a ∈ l₁ :=
take q, qeq.rec_on q
  (λ l, !mem_cons)
  (λ b l l' q r, sum.inr r)

@[hott] theorem mem_tail_of_qeq {a : A} {l₁ l₂ : list A} : l₁≈a|l₂ → ∀ x, x ∈ l₂ → x ∈ l₁ :=
take q, qeq.rec_on q
  (λ l x i, sum.inr i)
...
/-
@[hott] theorem mem_cons_of_qeq {a : A} {l₁ l₂ : list A} : l₁≈a|l₂ → ∀ x, x ∈ l₁ → x ∈ a::l₂ :=
take q, qeq.rec_on q
  (λ l x i, i)
...
@[hott] theorem length_eq_of_qeq {a : A} {l₁ l₂ : list A} : l₁≈a|l₂ → length l₁ = succ (length l₂) :=
take q, qeq.rec_on q
  (λ l, rfl)
  (λ b l l' q r, by rewrite [*length_cons, r])

@[hott] theorem qeq_of_mem {a : A} {l : list A} : a ∈ l → (Σl', l≈a|l') :=
list.rec_on l
...
@[hott] theorem qeq_split {a : A} {l l' : list A} : l'≈a|l → Σl₁ l₂, l = l₁++l₂ × l' = l₁++(a::l₂) :=
take q, qeq.rec_on q
  (λ t,
...
/-@[hott] theorem sub_of_mem_of_sub_of_qeq {a : A} {l : list A} {u v : list A} : a ∉ l → a::l ⊆ v → v≈a|u → l ⊆ u :=
...
@[hott] def firstn : nat → list A → list A
| 0     l      := []
| (n+1) []     := []
| (n+1) (a::l) := a :: firstn n l

@[hott] lemma firstn_zero : ∀ (l : list A), firstn 0 l = [] :=
by intros; reflexivity

@[hott] lemma firstn_nil : ∀ n, firstn n [] = ([] : list A)
| 0     := rfl
| (n+1) := rfl

@[hott] lemma firstn_cons : ∀ n (a : A) (l : list A), firstn (succ n) (a::l) = a :: firstn n l :=
by intros; reflexivity

@[hott] lemma firstn_all : ∀ (l : list A), firstn (length l) l = l
| []     := rfl
| (a::l) := begin unfold [length, firstn], rewrite firstn_all end

/-@[hott] lemma firstn_all_of_ge : ∀ {n} {l : list A}, n ≥ length l → firstn n l = l
| 0     []     h := rfl
| 0     (a::l) h := absurd h (not_le_of_gt !succ_pos)
| (n+1) []     h := rfl
| (n+1) (a::l) h := begin unfold firstn, rewrite [firstn_all_of_ge (le_of_succ_le_succ h)] end-/

/-@[hott] lemma firstn_firstn : ∀ (n m) (l : list A), firstn n (firstn m l) = firstn (min n m) l
...
@[hott] lemma length_firstn_le : ∀ (n) (l : list A), length (firstn n l) ≤ n
...
/-@[hott] lemma length_firstn_eq : ∀ (n) (l : list A), length (firstn n l) = min n (length l)
...
@[hott] def count (a : A) : list A → nat
| []      := 0
| (x::xs) := if a = x then succ (count xs) else count xs

@[hott] lemma count_nil (a : A) : count a [] = 0 :=
rfl

@[hott] lemma count_cons (a b : A) (l : list A) : count a (b::l) = if a = b then succ (count a l) else count a l :=
rfl

@[hott] lemma count_cons_eq (a : A) (l : list A) : count a (a::l) = succ (count a l) :=
if_pos rfl

@[hott] lemma count_cons_of_ne {a b : A} (h : a ≠ b) (l : list A) : count a (b::l) = count a l :=
if_neg h

@[hott] lemma count_cons_ge_count (a b : A) (l : list A) : count a (b::l) ≥ count a l :=
by_cases
...
@[hott] lemma count_singleton (a : A) : count a [a] = 1 :=
by rewrite count_cons_eq

@[hott] lemma count_append (a : A) : ∀ l₁ l₂, count a (l₁++l₂) = count a l₁ + count a l₂
...
@[hott] lemma count_concat (a : A) (l : list A) : count a (concat a l) = succ (count a l) :=
by rewrite [concat_eq_append, count_append, count_singleton]

@[hott] lemma mem_of_count_gt_zero : ∀ {a : A} {l : list A}, count a l > 0 → a ∈ l
...
/-@[hott] lemma count_gt_zero_of_mem : ∀ {a : A} {l : list A}, a ∈ l → count a l > 0
...
/-@[hott] lemma count_eq_zero_of_not_mem {a : A} {l : list A} (h : a ∉ l) : count a l = 0 :=
...
/- map -/
@[hott] def map (f : A → B) : list A → list B
| []       := []
| (a :: l) := f a :: map l

@[hott] theorem map_nil (f : A → B) : map f [] = [] := idp

@[hott] theorem map_cons (f : A → B) (a : A) (l : list A) : map f (a :: l) = f a :: map f l := idp

@[hott] lemma map_concat (f : A → B) (a : A) : Πl, map f (concat a l) = concat (f a) (map f l)
| nil    := rfl
| (b::l) := begin rewrite [concat_cons, +map_cons, concat_cons, map_concat] end

@[hott] lemma map_append (f : A → B) : Π l₁ l₂, map f (l₁++l₂) = (map f l₁)++(map f l₂)
| nil    := take l, rfl
| (a::l) := take l', begin rewrite [append_cons, *map_cons, append_cons, map_append] end

@[hott] lemma map_reverse (f : A → B) : Πl, map f (reverse l) = reverse (map f l)
| nil    := rfl
| (b::l) := begin rewrite [reverse_cons, +map_cons, reverse_cons, map_concat, map_reverse] end

@[hott] lemma map_singleton (f : A → B) (a : A) : map f [a] = [f a] := rfl

@[hott] theorem map_id : Π l : list A, map id l = l
| []      := rfl
| (x::xs) := begin rewrite [map_cons, map_id] end

@[hott] theorem map_id' {f : A → A} (H : Πx, f x = x) : Π l : list A, map f l = l
| []      := rfl
| (x::xs) := begin rewrite [map_cons, H, map_id'] end

@[hott] theorem map_map (g : B → C) (f : A → B) : Π l, map g (map f l) = map (g ∘ f) l
| []       := rfl
| (a :: l) :=
  show (g ∘ f) a :: map g (map f l) = map (g ∘ f) (a :: l),
  by rewrite (map_map l)

@[hott] theorem length_map (f : A → B) : Π l : list A, length (map f l) = length l
| []       := by esimp
| (a :: l) :=
  show length (map f l) + 1 = length l + 1,
  by rewrite (length_map l)

@[hott] theorem mem_map {A B : Type _} (f : A → B) : Π {a l}, a ∈ l → f a ∈ map f l
...
@[hott] theorem exists_of_mem_map {A B : Type _} {f : A → B} {b : B} :
...
@[hott] theorem eq_of_map_const {A B : Type _} {b₁ b₂ : B} : Π {l : list A}, b₁ ∈ map (const A b₂) l → b₁ = b₂
...
@[hott] def map₂ (f : A → B → C) : list A → list B → list C
| []      _       := []
| _       []      := []
| (x::xs) (y::ys) := f x y :: map₂ xs ys

/- filter -/
@[hott] def filter (p : A → Type _) [h : decidable_pred p] : list A → list A
| []     := []
| (a::l) := if p a then a :: filter l else filter l

@[hott] theorem filter_nil (p : A → Type _) [h : decidable_pred p] : filter p [] = [] := idp

@[hott] theorem filter_cons_of_pos {p : A → Type _} [h : decidable_pred p] {a : A} : Π l, p a → filter p (a::l) = a :: filter p l :=
λ l pa, if_pos pa

@[hott] theorem filter_cons_of_neg {p : A → Type _} [h : decidable_pred p] {a : A} : Π l, ¬ p a → filter p (a::l) = filter p l :=
λ l pa, if_neg pa

/-
@[hott] theorem of_mem_filter {p : A → Type _} [h : decidable_pred p] {a : A} : Π {l}, a ∈ filter p l → p a
| []     ain := absurd ain !not_mem_nil
| (b::l) ain := by_cases
  (assume pb  : p b,
...
@[hott] theorem mem_of_mem_filter {p : A → Type _} [h : decidable_pred p] {a : A} : Π {l}, a ∈ filter p l → a ∈ l
| []     ain := absurd ain !not_mem_nil
| (b::l) ain := by_cases
  (λ pb  : p b,
    have a ∈ b :: filter p l, by rewrite [filter_cons_of_pos _ pb at ain]; exact ain,
...
@[hott] theorem mem_filter_of_mem {p : A → Type _} [h : decidable_pred p] {a : A} : Π {l}, a ∈ l → p a → a ∈ filter p l
...
@[hott] theorem filter_sub {p : A → Type _} [h : decidable_pred p] (l : list A) : filter p l ⊆ l :=
λ a ain, mem_of_mem_filter ain

@[hott] theorem filter_append {p : A → Type _} [h : decidable_pred p] : Π (l₁ l₂ : list A), filter p (l₁++l₂) = filter p l₁ ++ filter p l₂
| []      l₂ := rfl
| (a::l₁) l₂ := by_cases
...
/- foldl & foldr -/
@[hott] def foldl (f : A → B → A) : A → list B → A
| a []       := a
| a (b :: l) := foldl (f a b) l

@[hott] theorem foldl_nil (f : A → B → A) (a : A) : foldl f a [] = a := idp

@[hott] theorem foldl_cons (f : A → B → A) (a : A) (b : B) (l : list B) : foldl f a (b::l) = foldl f (f a b) l := idp

@[hott] def foldr (f : A → B → B) : B → list A → B
| b []       := b
| b (a :: l) := f a (foldr b l)

@[hott] theorem foldr_nil (f : A → B → B) (b : B) : foldr f b [] = b := idp

@[hott] theorem foldr_cons (f : A → B → B) (b : B) (a : A) (l : list A) : foldr f b (a::l) = f a (foldr f b l) := idp

section foldl_eq_foldr
  -- foldl and foldr coincide when f is commutative and associative
  parameters {α : Type _} {f : α → α → α}
  hypothesis (Hcomm  : Π a b, f a b = f b a)
  hypothesis (Hassoc : Π a b c, f (f a b) c = f a (f b c))
  include Hcomm Hassoc

  @[hott] theorem foldl_eq_of_comm_of_assoc : Π a b l, foldl f a (b::l) = f b (foldl f a l)
  | a b  nil    := Hcomm a b
  | a b  (c::l) :=
    begin
  ...
  @[hott] theorem foldl_eq_foldr : Π a l, foldl f a l = foldr f a l
  | a nil      := rfl
  | a (b :: l) :=
    begin
      rewrite foldl_eq_of_comm_of_assoc,
...
@[hott] theorem foldl_append (f : B → A → B) : Π (b : B) (l₁ l₂ : list A), foldl f b (l₁++l₂) = foldl f (foldl f b l₁) l₂
| b []      l₂ := rfl
| b (a::l₁) l₂ := by rewrite [append_cons, *foldl_cons, foldl_append]

@[hott] theorem foldr_append (f : A → B → B) : Π (b : B) (l₁ l₂ : list A), foldr f b (l₁++l₂) = foldr f (foldr f b l₂) l₁
| b []      l₂ := rfl
