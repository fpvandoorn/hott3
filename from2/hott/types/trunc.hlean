...
namespace trunc_index
  ...
  @[hott] theorem not_succ_le_self {n : ℕ₋₂} : ¬n.+1 ≤ n :=
  begin
...
@[hott] def linear_weak_order_trunc_index [trans_instance] [reducible] :
...
namespace trunc_index
  ...
  @[hott] def trunc_index.decidable_le [instance] : Π(n m : ℕ₋₂), decidable (n ≤ m) :=
  begin
...
namespace is_trunc
  ...
  begin
    revert A B f Hf HA,
    induction n with n IH,
    { intros A B f Hf HA, induction Hf with g ε, fapply is_contr.mk,
      { exact f (center A)},
      { intro b, apply concat,
        { apply (ap f), exact (center_eq (g b))},
        { apply ε}}},
    { intros A B f Hf HA, induction Hf with g ε,
  ...
  @[hott] def trunctype_eq_equiv (n : ℕ₋₂) (A B : n-Type _) :
  ...
  @[hott] theorem is_trunc_trunctype [instance] (n : ℕ₋₂) : is_trunc n.+1 (n-Type _) :=
  begin
  ...
  @[hott] def teq_equiv_equiv {n : ℕ₋₂} {A B : n-Type _} : (A = B) ≃ (A ≃ B) :=
  trunctype_eq_equiv n A B ⬝e eq_equiv_equiv A B

  @[hott] def tua {n : ℕ₋₂} {A B : n-Type _} (f : A ≃ B) : A = B :=
  (trunctype_eq_equiv n A B)⁻¹ᶠ (ua f)

  @[hott] def tua_refl {n : ℕ₋₂} (A : n-Type _) : tua (@erfl A) = idp :=
  begin
  ...
  @[hott] def tua_trans {n : ℕ₋₂} {A B C : n-Type _} (f : A ≃ B) (g : B ≃ C)
  ...
  @[hott] def tua_symm {n : ℕ₋₂} {A B : n-Type _} (f : A ≃ B) : tua f⁻¹ᵉ = (tua f)⁻¹ :=
  begin
  ...
  @[hott] def tcast {n : ℕ₋₂} {A B : n-Type _} (p : A = B) (a : A) : B :=
  cast (ap trunctype.carrier p) a

  @[hott] def ptcast {n : ℕ₋₂} {A B : n-Type*} (p : A = B) : A →* B :=
  pcast (ap ptrunctype.to_pType p)

  @[hott] theorem tcast_tua_fn {n : ℕ₋₂} {A B : n-Type _} (f : A ≃ B) : tcast (tua f) = to_fun f :=
  begin
  ...
  @[hott] theorem is_set_of_relation.{u} {A : Type u} (R : A → A → Type u)
  ...
  -- use loopn in name
  @[hott] theorem is_trunc_iff_is_contr_loop_succ (n : ℕ) (A : Type _)
  ...
  -- use loopn in name
  @[hott] theorem is_trunc_iff_is_contr_loop (n : ℕ) (A : Type _)
    : is_trunc (n.-2.+1) A ↔ (Π(a : A), is_contr (Ω[n](pointed.Mk a))) :=
  begin
    induction n with n,
    { esimp [sub_two,loopn], apply iff.intro,
        intros H a, exact is_contr_of_inhabited_prop a,
  ...
  @[hott] def is_trunc_loopn (k : ℕ₋₂) (n : ℕ) (A : Type*) [H : is_trunc (k+n) A]
    : is_trunc k (Ω[n] A) :=
  begin
    revert k H, induction n with n IH: intros k H, exact _,
  ...
  @[hott] def pequiv_punit_of_is_contr (A : Type*) (H : is_contr A) : A ≃* punit :=
...
namespace trunc
  ...
  @[hott] def tidp [reducible] : trunc n (a₁ = a₁) :=
  tr idp
  ...
  begin
    ...
    { eapply (trunc.rec_on aa'), eapply (trunc.rec_on aa),
      intros a a' x, esimp [trunc.code, trunc.rec_on] at x,
  ...
  @[hott] def is_trunc_trunc_of_is_trunc [instance] [priority 500] (A : Type _)
  ...
  begin
    ...
    { clear n, intros n IH A m H, induction m with m,
  ...
  @[hott] def ptrunc (n : ℕ₋₂) (X : Type*) : n-Type* :=
...
namespace function
  ...
  @[hott] def is_surjective_of_is_equiv [instance] (f : A → B) [H : is_equiv f] : is_surjective f :=
