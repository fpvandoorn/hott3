...
namespace sigma
  ...
  @[hott] def eta3 : Π (u : Σa b c, D a b c), ⟨u.1, u.2.1, u.2.2.1, u.2.2.2⟩ = u
  | eta3 ⟨u₁, u₂, u₃, u₄⟩ := idp

  @[hott] def dpair_eq_dpair (p : a = a') (q : b =[p] b') : ⟨a, b⟩ = ⟨a', b'⟩ :=
  apd011 sigma.mk p q

  @[hott] def sigma_eq (p : u.1 = v.1) (q : u.2 =[p] v.2) : u = v :=
  by induction u; induction v; exact (dpair_eq_dpair p q)

  @[hott] def sigma_eq_right (q : b₁ = b₂) : ⟨a, b₁⟩ = ⟨a, b₂⟩ :=
  ap (dpair a) q
  ...
  @[hott] def eq2_pr2 {p q : u = v} (r : p = q) : p..2 =[eq2_pr1 r] q..2 :=
  ...
  @[hott] def is_equiv_sigma_eq [instance] (u v : Σa, B a)
  ...
  @[hott] def eq_pr1_idp (u : Σa, B a)           : (refl u) ..1 = refl (u.1)      := idp
  @[hott] def eq_pr1_con (p : u = v) (q : v = w) : (p ⬝ q)  ..1 = (p..1) ⬝ (q..1) := !ap_con
  @[hott] def eq_pr1_inv (p : u = v)             : p⁻¹      ..1 = (p..1)⁻¹        := !ap_inv
  ...
  @[hott] def sigma_eq2 {p q : u = v} (r : p..1 = q..1) (s : p..2 =[r] q..2)
    : p = q :=
  begin
  ...
  @[hott] def sigma_eq2_unc {p q : u = v} (rs : Σ(r : p..1 = q..1), p..2 =[r] q..2) : p = q :=
  ...
  @[hott] def total [reducible] {B' : A → Type _} (g : Πa, B a → B' a) : (Σa, B a) → (Σa, B' a) :=
  sigma_functor id g
  ...
  @[hott] def is_equiv_pr1 [instance] (B : A → Type _) [H : Π a, is_contr (B a)]
      : is_equiv (@pr1 A B) :=
  adjointify pr1
             (λa, ⟨a, !center⟩)
             (λa, idp)
  ...
  section
  @[hott] def is_equiv_sigma_rec [instance] (C : (Σa, B a) → Type _)
  ...
  @[hott] def is_equiv_coind [instance] (C : Πa, B a → Type _)
  ...
  @[hott] def subtype [reducible] {A : Type _} (P : A → Type _) [H : Πa, is_prop (P a)] :=
  Σ(a : A), P a
  ...
  /- truncatedness -/
  @[hott] theorem is_trunc_sigma (B : A → Type _) (n : trunc_index)
      [HA : is_trunc n A] [HB : Πa, is_trunc n (B a)] : is_trunc n (Σa, B a) :=
  begin
  revert A B HA HB,
  induction n with n IH,
  { intros A B HA HB, fapply is_trunc_equiv_closed_rev, apply sigma_equiv_of_is_contr_left},
...
namespace sigma
  ...
  @[hott] def pointed_sigma [instance] {A : Type _} (P : A → Type _) [G : pointed A]
  ...
  @[hott] def ppr2 {A : Type*} {B : A → Type*} (v : (Σ*(x : A), B x)) : B (ppr1 v) :=
  pr2 v
