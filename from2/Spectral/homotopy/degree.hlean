...
open fin eq equiv group algebra sphere.ops pointed nat int trunc is_equiv function circle
  ...
  @[hott] def cases_of_nat_abs_eq {z : ℤ} (n : ℕ) (p : nat_abs z = n)
  ...
  @[hott] def eq_one_or_eq_neg_one_of_mul_eq_one {n : ℤ} (m : ℤ) (p : n * m = 1) : n = 1 ⊎ n = -1 :=
  cases_of_nat_abs_eq 1
    (nat.eq_one_of_mul_eq_one (nat_abs m)
      ((int.nat_abs_mul n m)⁻¹ ⬝ ap int.nat_abs p))

  @[hott] def endomorphism_int_unbundled (f : ℤ → ℤ) [is_add_hom f] (n : ℤ) :
    f n = f 1 * n :=
  begin
...
namespace sphere
  ...
  attribute untrunc_of_is_trunc

  @[hott] def surf_eq_loop : @surf 1 = circle.loop := sorry

  -- @[hott] def π2S2_surf : π2S2 (tr surf) = 1 :> ℤ :=
  -- begin
...
--              (tr surf))
  ...
  @[hott] def πnSn_surf (n : ℕ) : πnSn (n+1) (tr surf) = 1 :> ℤ :=
  begin
    cases n with n IH,
  ...
  @[hott] def endomorphism_int (f : gℤ →g gℤ) (n : ℤ) : f n = f (1 : ℤ) *[ℤ] n :=
  @endomorphism_int_unbundled f (homomorphism.addstruct f) n

  @[hott] def endomorphism_equiv_Z {X : Group} (e : X ≃g gℤ) {one : X}
  ...
  @[hott] def deg_compose {n : ℕ} [H : is_succ n] (f g : S n →* S n) :
  ...
  @[hott] def deg_equiv {n : ℕ} [H : is_succ n] (f : S n ≃* S n) :
    deg f = 1 ⊎ deg f = -1 :=
  begin
    induction H with n,
