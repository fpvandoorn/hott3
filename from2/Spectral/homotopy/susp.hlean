...
namespace susp
  ...
  @[hott] def iterate_susp_iterate_susp_rev (n m : ℕ) (A : Type*) :
  ...
  @[hott] def iterate_susp_pequiv (n : ℕ) {X Y : Type*} (f : X ≃* Y) :
  ...
  open algebra nat
  @[hott] def iterate_susp_iterate_susp (n m : ℕ) (A : Type*) :
  ...
  @[hott] def plift_susp.{u v} : Π(A : Type*), plift.{u v} (susp A) ≃* susp (plift.{u v} A) :=
  begin
    intro A,
    calc
  ...
  @[hott] def is_contr_susp [instance] (A : Type _) [H : is_contr A] : is_contr (susp A) :=
  begin
  ...
  @[hott] def loop_susp_pintro_phomotopy {X Y : Type*} {f g : ⅀ X →* Y} (p : f ~* g) :
  ...
  @[hott] def suspend_psquare (p : psquare f₁₀ f₁₂ f₀₁ f₂₁) : psquare (⅀→ f₁₀) (⅀→ f₁₂) (⅀→ f₀₁) (⅀→ f₂₁) :=
sorry

  @[hott] def susp_to_loop_psquare (f₁₀ : A₀₀ →* A₂₀) (f₁₂ : A₀₂ →* A₂₂) (f₀₁ : susp A₀₀ →* A₀₂) (f₂₁ : susp A₂₀ →* A₂₂) : (psquare (⅀→ f₁₀) f₁₂ f₀₁ f₂₁) → (psquare f₁₀ (Ω→ f₁₂) ((loop_susp_pintro A₀₀ A₀₂) f₀₁) ((loop_susp_pintro A₂₀ A₂₂) f₂₁)) :=
  begin
    intro p,
  ...
  @[hott] def loop_to_susp_square (f₁₀ : A₀₀ →* A₂₀) (f₁₂ : A₀₂ →* A₂₂) (f₀₁ : A₀₀ →* Ω A₀₂) (f₂₁ : A₂₀ →* Ω A₂₂) : (psquare f₁₀ (Ω→ f₁₂) f₀₁ f₂₁) → (psquare (⅀→ f₁₀) f₁₂ ((susp_pelim A₀₀ A₀₂) f₀₁) ((susp_pelim A₂₀ A₂₂) f₂₁)) :=
  begin
    intro p,
