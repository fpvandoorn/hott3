...
namespace smash
  ...
  open pushout
  @[hott] def prod3_of_sum3 (A B C : Type*) : A × C ⊎ A × B ⊎ C × B → A × B × C :=
  begin
  ...
  @[hott] def fin3_of_sum3 (A B C : Type*) : A × C ⊎ A × B ⊎ C × B → A ⊎ B ⊎ C :=
  sum_functor (λac, ac.1) (sum_functor (λab, ab.2) (λcb, cb.1))

  @[hott] def smash3 (A B C : Type*) : Type* :=
  pointed.MK (pushout (prod3_of_sum3 A B C) (fin3_of_sum3 A B C)) (inl (pt, (pt, pt)))

  @[hott] def to_image {A B : Type _} (f : A → B) (a : A) : sigma (image f) :=
  ⟨f a, image.mk a idp⟩

  @[hott] def pushout_image_prod3_of_sum3 (A B C : Type*) : Type _ :=
  pushout (to_image (prod3_of_sum3 A B C)) (fin3_of_sum3 A B C)

  -- @[hott] def pushout_image_prod3_of_sum3_equiv (A B C : Type*) : pushout_image_prod3_of_sum3 A B C ≃ unit :=
  -- begin
  ...
  -- @[hott] def smash3_comm (A B C : Type*) : smash3 A B C ≃* smash3 C B A :=
  -- begin
  ...
  @[hott] def glue1 (a : A) (b : B) : inl (a, (b, pt)) = inr (inr (inl b)) :> smash3 A B C :=
  pushout.glue (inr (inl (a, b)))

  @[hott] def glue2 (b : B) (c : C) : inl (pt, (b, c)) = inr (inr (inr c)) :> smash3 A B C :=
  pushout.glue (inr (inr (c, b)))

  @[hott] def glue3 (c : C) (a : A) : inl (a, (pt, c)) = inr (inl a) :> smash3 A B C :=
  pushout.glue (inl (a, c))

  @[hott] def smash3_of_prod_smash (a : A) (bc : B ∧ C) : smash3 A B C :=
  begin
  ...
  @[hott] def smash3_of_smash_smash_gluer (bc : B ∧ C) :
  ...
  @[hott] def smash3_of_smash_smash (x : A ∧ (B ∧ C)) : smash3 A B C :=
  begin
  ...
  @[hott] def smash_smash_of_smash3 (x : smash3 A B C) : A ∧ (B ∧ C) :=
  begin
    induction x,
  ...
  attribute smash_smash_of_smash3_1

  @[hott] def smash_smash_of_smash3_of_prod_smash (a : A) (bc : B ∧ C) :
  ...
  @[hott] def smash_smash_equiv_smash3 (A B C : Type*) : A ∧ (B ∧ C) ≃* smash3 A B C :=
  begin
    fapply pequiv_of_equiv,
    { fapply equiv.MK,
...
--   sorry
  ...
  @[hott] def inl3 (a : A) (b : B) (c : C) : A ∧ (B ∧ C) :=
  smash.mk a (smash.mk b c)

  @[hott] def aux1 (a : A) : A ∧ (B ∧ C) := pt
  @[hott] def aux2 (b : B) : A ∧ (B ∧ C) := pt
  @[hott] def aux3 (c : C) : A ∧ (B ∧ C) := pt

  @[hott] def glue12 (a : A) (b : B) : inl3 a b (pt : C) = aux1 a :=
  ap (smash.mk a) (gluel' b pt) ⬝ gluel' a pt

  @[hott] def glue23 (b : B) (c : C) : inl3 (pt : A) b c = aux2 b :=
  gluer' (smash.mk b c) pt

  @[hott] def glue31 (c : C) (a : A) : inl3 a (pt : B) c = aux3 c :=
  ap (smash.mk a) (gluer' c pt) ⬝ gluel' a pt

  @[hott] def glue1_eq (a : A) : glue12 a pt = glue31 pt a :> (_ = _ :> (A ∧ (B ∧ C))) :=
  whisker_right _ (ap02 _ (!con.right_inv ⬝ !con.right_inv⁻¹))

  @[hott] def glue2_eq (b : B) : glue23 b pt = glue12 pt b :> (_ = _ :> (A ∧ (B ∧ C))) :=
  !con_idp⁻¹ ⬝ !ap_mk_right⁻¹ ◾ !con.right_inv⁻¹

  @[hott] def glue3_eq (c : C) : glue31 c pt = glue23 pt c :> (_ = _ :> (A ∧ (B ∧ C))) :=
  !ap_mk_right ◾ !con.right_inv ⬝ !con_idp

local attribute ap_mk_right [reducible]

  @[hott] def concat3 {A : Type _} {x y z : A} {p p' : x = y} {q q' : y = z}
    {r r' : p = p'} {s s' : q = q'} : r = r' → s = s' → r ◾ s = r' ◾ s' :=
  ap011 concat2

  @[hott] def glue_eq2 : glue3_eq (pt : A) ⬝ glue2_eq (pt : B) ⬝ glue1_eq (pt : C) = idp :=
  begin
  ...
  @[hott] def glue12_cancel (a : A) (b : B) : @glue12 A B C a b ⬝ (glue12 a pt)⁻¹ ⬝ ap (smash.mk a) (gluel pt) = ap (smash.mk a) (gluel b) :=
  begin
    unfold [glue12],
  ...
  @[hott] def glue12_cancel_pt_pt : @glue12_cancel A B C pt pt = whisker_right _ !con.right_inv ⬝ !idp_con :=
  sorry

  @[hott] def glue12_over {P : A ∧ (B ∧ C) → Type _} (Ppt : Πa b c, P (inl3 a b c))
    (P1 : Πa, P (aux1 a))
  ...
  @[hott] def glue12_over_pt_pt {P : A ∧ (B ∧ C) → Type _} (Ppt : Πa b c, P (inl3 a b c))
    (P1 : Πa, P (aux1 a))
    (P12 : Πa b, Ppt a b pt =[glue12 a b] P1 a) :
    glue12_over Ppt P1 P12 pt pt = pathover_tr (ap (smash.mk pt) (gluel pt)) (Ppt pt pt pt) :=
  sorry

  @[hott] def smash3_rec_mk {P : A ∧ (B ∧ C) → Type _} (Ppt : Πa b c, P (inl3 a b c))
  ...
  @[hott] def apd_constant' {A : Type _} {B : Type _} {a a' : A} (p : a = a') {b : B} : apd (λa, b) p = pathover_of_eq p idp :=
  by induction p; reflexivity

@[hott] def change_path_eq_of_eq_change_path' {A : Type _} {B : A → Type _} {a a₂ : A} {p p' : a = a₂} {b : B a} {b₂ : B a₂}
...
begin
  ...
  @[hott] def pathover_hconcato' {A : Type _} {B : A → Type _} {a₀₀ a₂₀ a₀₂ a₂₂ : A}
            /-a₀₀-/ {p₁₀ : a₀₀ = a₂₀} /-a₂₀-/
       {p₀₁ : a₀₀ = a₀₂} /-s₁₁-/ {p₂₁ : a₂₀ = a₂₂}
            /-a₀₂-/ {p₁₂ : a₀₂ = a₂₂} /-a₂₂-/
            {s₁₁ : square p₁₀ p₁₂ p₀₁ p₂₁}
  ...
  @[hott] def pathover_hconcato_right_inv {A : Type _} {B : A → Type _} {a₀₀ a₂₀ a₀₂ a₀₄ a₂₂ : A}
            /-a₀₀-/ {p₁₀ : a₀₀ = a₂₀} /-a₂₀-/
       {p₀₁ : a₀₀ = a₀₂} {p₀₃ : a₀₂ = a₀₄} /-s₁₁-/ {p₂₁ : a₂₀ = a₂₂}
            /-a₀₂-/ {p₁₂ : a₀₂ = a₂₂} /-a₂₂-/
  ...
  @[hott] def smash3_rec_23 {P : A ∧ (B ∧ C) → Type _} (Ppt : Πa b c, P (inl3 a b c))
    (P1 : Πa, P (aux1 a)) (P2 : Πb, P (aux2 b)) (P3 : Πc, P (aux3 c))
    (P12 : Πa b, Ppt a b pt =[glue12 a b] P1 a)
    (P23 : Πb c, Ppt pt b c =[glue23 b c] P2 b)
  ...
  @[hott] def smash3_rec {P : A ∧ (B ∧ C) → Type _} (Ppt : Πa b c, P (inl3 a b c))
    (P1 : Πa, P (aux1 a)) (P2 : Πb, P (aux2 b)) (P3 : Πc, P (aux3 c))
    (P12 : Πa b, Ppt a b pt =[glue12 a b] P1 a)
    (P23 : Πb c, Ppt pt b c =[glue23 b c] P2 b)
    (P31 : Πc a, Ppt a pt c =[glue31 c a] P3 c)
...
--   @[hott] def smash3_rec_mk {P : A ∧ (B ∧ C) → Type _} (Ppt : Πa b c, P (smash.mk a (smash.mk b c)))
...
--   @[hott] def smash3_rec {P : A ∧ (B ∧ C) → Type _} (Ppt : Πa b c, P (smash.mk a (smash.mk b c)))
--     (P12 : Πa b, Ppt a b pt =[glue12 a b] Ppt pt pt pt)
--     (P23 : Πb c, Ppt pt b c =[glue23 b c] Ppt pt pt pt)
--     (P31 : Πc a, Ppt a pt c =[glue31 c a] Ppt pt pt pt)
--     (x : A ∧ (B ∧ C)) : P x :=
