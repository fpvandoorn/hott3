...
exit
  ...
  @[hott] def red_susp_of_smash_pcircle (x : smash A S¹*) : red_susp A :=
  begin
  ...
  @[hott] def smash_pcircle_of_red_susp (x : red_susp A) : smash A S¹* :=
  begin
    induction x,
    { exact pt },
  ...
  @[hott] def smash_pcircle_of_red_susp_of_smash_pcircle_pt (a : A) (x : S¹*) :
  ...
  begin
    ...
    { exact abstract begin apply eq_pathover,
      ...
      -- make everything below this a @[hott] lemma defined by path induction?
      refine !con_idp⁻¹ ⬝pv _, refine !con.assoc⁻¹ ⬝ph _, apply whisker_bl, apply whisker_lb,
      apply whisker_tl, apply hrfl end end }
  end

  @[hott] def concat2o {A B : Type _} {f g h : A → B} {q : f ~ g} {r : g ~ h} {a a' : A}
    {p : a = a'} (s : q a =[p] q a') (t : r a =[p] r a') : q a ⬝ r a =[p] q a' ⬝ r a' :=
  by induction p; exact idpo

  @[hott] def apd_con_fn {A B : Type _} {f g h : A → B} {q : f ~ g} {r : g ~ h} {a a' : A}
    (p : a = a') : apd (λa, q a ⬝ r a) p = concat2o (apd q p) (apd r p) :=
  by induction p; reflexivity

  -- @[hott] def apd_con_fn_constant {A B : Type _} {f : A → B} {b b' : B} {q : Πa, f a = b}
  --   {r : b = b'} {a a' : A} (p : a = a') :
  --   apd (λa, q a ⬝ r) p = concat2o (apd q p) (pathover_of_eq _ idp) :=
  -- by induction p; reflexivity


  @[hott] def smash_pcircle_pequiv_red (A : Type*) : smash A S¹* ≃* red_susp A :=
  begin
  ...
  @[hott] def susp_of_smash_pcircle (x : smash A S¹*) : susp A :=
  begin
    induction x using smash.elim,
  ...
  @[hott] def smash_pcircle_of_susp (x : susp A) : smash A S¹* :=
  begin
    induction x,
    { exact pt },
    { exact pt },
 ...
 -- the definitions below compile, but take a long time to do so and have sorry's in them
  @[hott] def smash_pcircle_of_susp_of_smash_pcircle_pt (a : A) (x : S¹*) :
  ...
  -- @[hott] def smash_pcircle_of_susp_of_smash_pcircle_gluer_base (b : S¹*)
...
exit
  @[hott] def smash_pcircle_pequiv (A : Type*) : smash A S¹* ≃* susp A :=
  begin
    fapply pequiv_of_equiv,
    { fapply equiv.MK,
