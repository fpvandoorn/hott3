...
namespace pointed
  ...
  @[hott] def pumap (A : Type _) (B : Type*) : Type* :=
  pointed.MK (A → B) (const A pt)

  /- the pointed type of unpointed dependent maps -/
  @[hott] def pupi {A : Type _} (B : A → Type*) : Type* :=
  ...
  @[hott] def sigma_pumap {A : Type _} (B : A → Type _) (C : Type*) :
    ((Σa, B a) →ᵘ* C) ≃* Πᵘ*a, B a →ᵘ* C :=
  pequiv_of_equiv (equiv_sigma_rec _)⁻¹ᵉ idp

  @[hott] def phomotopy_mk_pupi {A : Type*} {B : Type _} {C : B → Type*}
  ...
  @[hott] def pupi_functor {A A' : Type _} {B : A → Type*} {B' : A' → Type*}
    (f : A' → A) (g : Πa, B (f a) →* B' a) : (Πᵘ*a, B a) →* (Πᵘ*a', B' a') :=
  pmap.mk (pi_functor f g) (eq_of_homotopy (λa, respect_pt (g a)))

  @[hott] def pupi_functor_right {A : Type _} {B B' : A → Type*}
    (g : Πa, B a →* B' a) : (Πᵘ*a, B a) →* (Πᵘ*a, B' a) :=
  pupi_functor id g

  @[hott] def pupi_functor_compose {A A' A'' : Type _}
  ...
  @[hott] def pupi_functor_pid (A : Type _) (B : A → Type*) :
  ...
  @[hott] def pupi_pequiv_right {A : Type _} {B B' : A → Type*} (f : Πa, B a ≃* B' a) :
    (Πᵘ*a, B a) ≃* (Πᵘ*a, B' a) :=
  pupi_pequiv erfl f

  @[hott] def loop_pupi {A : Type _} (B : A → Type*) : Ω (Πᵘ*a, B a) ≃* Πᵘ*a, Ω (B a) :=
  pequiv_of_equiv eq_equiv_homotopy idp

  -- @[hott] def loop_pupi_natural {A : Type _} {B B' : A → Type*} (f : Πa, B a →* B' a) :
  --   psquare (Ω→ (pupi_functor_right f)) (pupi_functor_right (λa, Ω→ (f a)))
  --           (loop_pupi B) (loop_pupi B') :=

  @[hott] def ap1_gen_pi {A A' : Type _} {B : A → Type _} {B' : A' → Type _}
...
--    exact ap (λx, apd10 (ap1_gen _ x x _) _) !eq_of_homotopy_idp
  ...
  @[hott] def ap1_gen_pi_idp {A A' : Type _} {B : A → Type _} {B' : A' → Type _}
...
-- print ap1_gen_pi
  ...
  begin
    fapply phomotopy_mk_pupi,
    { intros p a, exact ap1_gen_pi id f (λa, respect_pt (f a)) (λa, respect_pt (f a)) p a },
    { intro a, revert B' f, refine fiberwise_pointed_map_rec _ _, intros B' f,
      refine !ap1_gen_pi_idp ◾ (ap (λx, apd10 x _) !idp_con ⬝ !apd10_eq_of_homotopy) }
  end

  @[hott] def phomotopy_mk_pumap {A C : Type*} {B : Type _} {f g : A →* (B →ᵘ* C)}
    (p : f ~2 g) (q : p pt ⬝hty apd10 (respect_pt g) ~ apd10 (respect_pt f))
    : f ~* g :=
  phomotopy_mk_pupi p q

  @[hott] def pumap_functor {A A' : Type _} {B B' : Type*} (f : A' → A) (g : B →* B') :
    (A →ᵘ* B) →* (A' →ᵘ* B') :=
  pupi_functor f (λa, g)

  @[hott] def pumap_functor_compose {A A' A'' : Type _} {B B' B'' : Type*}
    (f : A'' → A') (f' : A' → A) (g' : B' →* B'') (g : B →* B') :
    pumap_functor (f' ∘ f) (g' ∘* g) ~* pumap_functor f g' ∘* pumap_functor f' g :=
  pupi_functor_compose f f' (λa, g') (λa, g)

  @[hott] def pumap_functor_pid (A : Type _) (B : Type*) :
    pumap_functor id (pid B) ~* pid (A →ᵘ* B) :=
  pupi_functor_pid A (λa, B)

  @[hott] def pumap_functor_phomotopy {A A' : Type _} {B B' : Type*} {f f' : A' → A} {g g' : B →* B'}
  ...
  @[hott] def loop_psigma_gen_natural {A A' : Type*} {B : A → Type _}
    {B' : A' → Type _} {b : B pt} {b' : B' pt} (f : A →* A')
  ...
  @[hott] def psigma_gen_functor_pcompose {A₁ A₂ A₃ : Type*}
    {B₁ : A₁ → Type _} {B₂ : A₂ → Type _} {B₃ : A₃ → Type _}
  ...
  @[hott] def psigma_gen_functor_phomotopy {A₁ A₂ : Type*}
    {B₁ : A₁ → Type _} {B₂ : A₂ → Type _} {b₁ : B₁ pt} {b₂ : B₂ pt} {f₁ f₂ : A₁ →* A₂}
    {g₁ : Π⦃a⦄, B₁ a → B₂ (f₁ a)} {g₂ : Π⦃a⦄, B₁ a → B₂ (f₂ a)}
  ...
  @[hott] def psigma_gen_functor_psquare {A₀₀ A₀₂ A₂₀ A₂₂ : Type*}
    {B₀₀ : A₀₀ → Type _} {B₀₂ : A₀₂ → Type _} {B₂₀ : A₂₀ → Type _} {B₂₂ : A₂₂ → Type _}
...
namespace pointed
  ...
  @[hott] def pointed_respect_pt [instance] {A B : Type*} (f : A →* B) :
    pointed (f pt = pt) :=
  pointed.mk (respect_pt f)

  @[hott] def ppi_of_phomotopy {A B : Type*} {f g : A →* B} (h : f ~* g) :
    ppi (λx, f x = g x) (respect_pt f ⬝ (respect_pt g)⁻¹) :=
  h

  @[hott] def phomotopy {A : Type*} {P : A → Type _} {x : P pt} (f g : ppi P x) : Type _ :=
  ppi (λa, f a = g a) (respect_pt f ⬝ (respect_pt g)⁻¹)

  variables {A : Type*} {P Q R : A → Type*} {f g h : Π*a, P a}
                        {B C D : A → Type _} {b₀ : B pt} {c₀ : C pt} {d₀ : D pt}
                        {k k' l m : ppi B b₀}

  @[hott] def pppi_equiv_pmap (A B : Type*) : (Π*(a : A), B) ≃ (A →* B) :=
  by reflexivity

  @[hott] def pppi_pequiv_ppmap (A B : Type*) : (Π*(a : A), B) ≃* ppmap A B :=
  by reflexivity

  @[hott] def apd10_to_fun_eq_of_phomotopy (h : f ~* g)
...
--  @[hott] def phomotopy_of_eq_of_phomotopy

  @[hott] def phomotopy_mk_ppi {A : Type*} {B : Type*} {C : B → Type*}
...
--   end
  ...
  @[hott] def ppi_loop_equiv : (k = k) ≃ Π*(a : A), Ω (pType.mk (B a) (k a)) :=
  begin
  ...
  variables {k l}
  -- @[hott] def eq_of_phomotopy (h : k ~* l) : k = l :=
  -- (ppi_eq_equiv k l)⁻¹ᵉ h

  @[hott] def ppi_functor_right {A : Type*} {B B' : A → Type _}
    {b : B pt} {b' : B' pt} (f : Πa, B a → B' a) (p : f pt b = b') (g : ppi B b)
    : ppi B' b' :=
  ppi.mk (λa, f a (g a)) (ap (f pt) (respect_pt g) ⬝ p)

  @[hott] def ppi_functor_right_compose {A : Type*} {B₁ B₂ B₃ : A → Type _}
  ...
  @[hott] def ppi_functor_right_id {A : Type*} {B : A → Type _}
  ...
  @[hott] def ppi_functor_right_phomotopy {g g' : Π(a : A), B a → C a}
  ...
  phomotopy.mk (λa, p a (f a) ⬝ ap (g' a) (q a))
    abstract begin
      ...
      { intros h h', exact !eq_equiv_eq_symm ⬝e !eq_equiv_homotopy2 },
      { reflexivity },
      induction g₀', induction f with f f₀, induction f₀, reflexivity
    end end

  @[hott] def ppi_functor_right_phomotopy_refl (g : Π(a : A), B a → C a)
  ...
  @[hott] def ppi_equiv_ppi_right {A : Type*} {B B' : A → Type _}
    {b : B pt} {b' : B' pt} (f : Πa, B a ≃ B' a) (p : f pt b = b') :
    ppi B b ≃ ppi B' b' :=
  ...
  @[hott] def pmap_compose_ppi2_refl (g : Π(a : A), P a →* Q a) (f : Π*(a : A), P a) :
  ...
  @[hott] def ppi_assoc (h : Π (a : A), Q a →* R a) (g : Π (a : A), P a →* Q a)
    (f : Π*a, P a) :
    pmap_compose_ppi (λa, h a ∘* g a) f ~* pmap_compose_ppi h (pmap_compose_ppi g f) :=
  begin
    fapply phomotopy.mk,
  ...
  @[hott] def pmap_compose_ppi_eq_of_phomotopy (g : Πa, P a →* Q a) {f f' : Π*a, P a} (p : f ~* f') :
  ...
  @[hott] def ppi_assoc_ppi_const_right (g : Πa, Q a →* R a) (f : Πa, P a →* Q a) :
  ...
  @[hott] def pppi_compose_left (g : Π(a : A), ppmap (P a) (Q a)) :
    (Π*(a : A), P a) →* Π*(a : A), Q a :=
  ...
  @[hott] def pppi_compose_left_pcompose (g : Π (a : A), Q a →* R a) (f : Π (a : A), P a →* Q a)
  ...
  @[hott] def pppi_compose_left_phomotopy {g g' : Π(a : A), ppmap (P a) (Q a)}
  ...
  @[hott] def psquare_pppi_compose_left {A : Type*} {B C D E : A → Type*}
  ...
  @[hott] def ppi_pequiv_right (g : Π(a : A), P a ≃* Q a) :
    (Π*(a : A), P a) ≃* Π*(a : A), Q a :=
  begin
...
namespace pointed
  ...
  @[hott] def pppi_sigma_char_natural {B B' : A → Type*} (f : Πa, B a →* B' a) :
  ...
  @[hott] def ppi_psigma.{u v w} {A : pType.{u}} {B : A → pType.{v}} (C : Πa, B a → Type w)
  ...
  @[hott] def ppmap_psigma {A B : Type*} (C : B → Type _) (c : C pt) :
...
--  psigma_gen_pequiv_psigma_gen (pppi_pequiv_ppmap A B) (λf, begin esimp, exact ppi_equiv_ppi_right (λa, _) _ end) _
  ...
  @[hott] def pfiber_pppi_compose_left {B C : A → Type*} (f : Πa, B a →* C a) :
  ...
  @[hott] def pfiber_ppcompose_left (f : B →* C) :
  ...
  -- @[hott] def pppi_ppmap {A C : Type*} {B : A → Type*} :
  --   ppmap (/- dependent smash of B -/) C ≃* Π*(a : A), ppmap (B a) C :=

  @[hott] def ppi_add_point_over {A : Type _} (B : A → Type*) :
    (Π*a, add_point_over B a) ≃ Πa, B a :=
  begin
    fapply equiv.MK,
    { intros f a, exact f (some a) },
  ...
  @[hott] def pppi_add_point_over {A : Type _} (B : A → Type*) :
    (Π*a, add_point_over B a) ≃* Πᵘ*a, B a :=
  pequiv_of_equiv (ppi_add_point_over B) idp

  @[hott] def ppmap_add_point {A : Type _} (B : Type*) :
    ppmap A₊ B ≃* A →ᵘ* B :=
  ...
  @[hott] def ppi_eq_equiv_natural_gen_lem {B C : A → Type _} {b₀ : B pt} {c₀ : C pt}
  ...
  @[hott] def ppi_eq_equiv_natural_gen_lem2 {B C : A → Type _} {b₀ : B pt} {c₀ : C pt}
  ...
  @[hott] def ppi_eq_equiv_natural_gen {B C : A → Type _} {b₀ : B pt} {c₀ : C pt}
    {f : Π(a : A), B a → C a} {f₀ : f pt b₀ = c₀} {k l : ppi B b₀}
    {k' l' : ppi C c₀} (p : ppi_functor_right f f₀ k ~* k')
    (q : ppi_functor_right f f₀ l ~* l') :
  ...
  @[hott] def ppi_eq_equiv_natural_gen_refl {B C : A → Type _}
  ...
  @[hott] def loop_pppi_pequiv {A : Type*} (B : A → Type*) :
    Ω (Π*a, B a) ≃* Π*(a : A), Ω (B a) :=
  pequiv_of_equiv !ppi_eq_equiv idp

  @[hott] def loop_pppi_pequiv_natural {A : Type*} {X Y : A → Type*} (f :  Π (a : A), X a →* Y a) :
    psquare
      ...
      (loop_pppi_pequiv Y) :=
  begin
    revert Y f, refine fiberwise_pointed_map_rec _ _, intros Y f,
    fapply phomotopy.mk,
...
/-  @[hott] def loop_pppi_pequiv2 {A : Type*} (B : A → Type*) : Ω (Π*a, B a) ≃* Π*(a : A), Ω (B a) :=
  begin
  ...
  @[hott] def loop_pppi_pequiv_natural2 {A : Type*} {X Y : A → Type*} (f :  Π (a : A), X a →* Y a) :
    psquare
      (Ω→ (pppi_compose_left f))
      (pppi_compose_left (λ a, Ω→ (f a)))
      (loop_pppi_pequiv2 X)
...
namespace is_conn
  ...
  @[hott] def is_contr_ppi_match (P : A → Type*) (H : Πa, is_trunc (n.+1) (P a))
    : is_contr (Π*(a : A), P a) :=
  begin
  ...
  @[hott] def is_trunc_pmap_of_is_conn (k l : ℕ₋₂) (B : Type*) (H2 : l ≤ n.+1+2+k)
  ...
  @[hott] def is_trunc_ppi (A : Type*) (n k : ℕ₋₂) (H : n ≤ k) (P : A → Type*)
