...
structure is_exact_t {A B : Type _} {C : Type*} (f : A → B) (g : B → C) :=
  ( im_in_ker : Π(a:A), g (f a) = pt)
  ( ker_in_im : Π(b:B), (g b = pt) → fiber f b)

structure is_exact {A B : Type _} {C : Type*} (f : A → B) (g : B → C) :=
  ( im_in_ker : Π(a:A), g (f a) = pt)
  ( ker_in_im : Π(b:B), (g b = pt) → image f b)

namespace algebra

@[hott] def is_exact_g {A B C : Group} (f : A →g B) (g : B →g C) :=
is_exact f g

@[hott] def is_exact_ag {A B C : AbGroup} (f : A →g B) (g : B →g C) :=
is_exact f g

@[hott] def is_exact_g.mk {A B C : Group} {f : A →g B} {g : B →g C}
  (H₁ : Πa, g (f a) = 1) (H₂ : Πb, g b = 1 → image f b) : is_exact_g f g :=
is_exact.mk H₁ H₂

@[hott] def is_exact.im_in_ker2 {A B : Type _} {C : Set*} {f : A → B} {g : B → C} (H : is_exact f g)
  {b : B} (h : image f b) : g b = pt :=
begin
  induction h with a p, exact ap g p⁻¹ ⬝ is_exact.im_in_ker H a
end

@[hott] def is_exact_homotopy {A B : Type _} {C : Type*} {f f' : A → B} {g g' : B → C}
...
@[hott] def is_exact_trunc_functor {A B : Type _} {C : Type*} {f : A → B} {g : B → C}
...
begin
  ...
  { intros b p, induction b with b, note q := !tr_eq_tr_equiv p, induction q with q,
    induction is_exact_t.ker_in_im H b q with a r,
    exact image.mk (tr a) (ap tr r) }
end

@[hott] def is_contr_middle_of_is_exact {A B : Type _} {C : Type*} {f : A → B} {g : B → C}
...
@[hott] def is_surjective_of_is_exact_of_is_contr {A B : Type _} {C : Type*} {f : A → B} {g : B → C}
  (H : is_exact f g) [is_contr C] : is_surjective f :=
λb, is_exact.ker_in_im H b !is_prop.elim

@[hott] def is_embedding_of_is_exact_g {A B C : Group} {g : B →g C} {f : A →g B}
  (gf : is_exact_g f g) [is_contr A] : is_embedding g :=
begin
  apply to_is_embedding_homomorphism, intros a p,
  induction is_exact.ker_in_im gf a p with x q,
  exact q⁻¹ ⬝ ap f !is_prop.elim ⬝ to_respect_one f
end

@[hott] def map_left_of_is_exact {G₃' G₃ G₂ : Type _} {G₁ : Type*}
...
@[hott] def map_left_of_is_exact_compute {G₃' G₃ G₂ : Type _} {G₁ : Type*}
  {g : G₃ → G₂} {g' : G₃' → G₂} {f : G₂ → G₁} (H1 : is_exact g f) (H2 : is_exact g' f)
  (Hg' : is_embedding g') (a : G₃) : g' (map_left_of_is_exact H1 H2 Hg' a) = g a :=
@point_eq _ _ g' _ _

@[hott] def map_left_of_is_exact_compose {G₃'' G₃' G₃ G₂ : Type _} {G₁ : Type*}
...
@[hott] def map_left_of_is_exact_id {G₃ G₂ : Type _} {G₁ : Type*}
...
@[hott] def map_left_of_is_exact_homotopy {G₃' G₃ G₂ : Type _} {G₁ : Type*}
...
@[hott] def homomorphism_left_of_is_exact_g {G₃' G₃ G₂ G₁ : Group}
...
begin
  ...
  { intros a a', refine @is_injective_of_is_embedding _ _ g' _ _ _ _,
    exact !point_eq ⬝ to_respect_mul g a a' ⬝
      (to_respect_mul g' _ _ ⬝ ap011 mul !point_eq !point_eq)⁻¹ }
end

@[hott] def isomorphism_left_of_is_exact_g {G₃' G₃ G₂ G₁ : Group}
...
@[hott] def is_exact_incl_of_subgroup {G H : Group} (f : G →g H) :
  is_exact (incl_of_subgroup (kernel f)) f :=
begin
  apply is_exact.mk,
  { intro x, cases x with x p, exact p },
  { intros x p, exact image.mk ⟨x, p⟩ idp }
end

@[hott] def isomorphism_kernel_of_is_exact {G₄ G₃ G₂ G₁ : Group}
...
@[hott] def is_exact_of_is_exact_at {N : succ_str} {A : chain_complex N} {n : N}
  (H : is_exact_at A n) : is_exact (cc_to_fn A (S n)) (cc_to_fn A n) :=
is_exact.mk (cc_is_chain_complex A n) H
end chain_complex

structure is_short_exact {A B : Type _} {C : Type*} (f : A → B) (g : B → C) :=
  (is_emb    : is_embedding f)
  (im_in_ker : Π(a:A), g (f a) = pt)
  (ker_in_im : Π(b:B), (g b = pt) → image f b)
  (is_surj   : is_surjective g)

structure is_short_exact_t {A B : Type _} {C : Type*} (f : A → B) (g : B → C) :=
  (is_emb    : is_embedding f)
  (im_in_ker : Π(a:A), g (f a) = pt)
  (ker_in_im : Π(b:B), (g b = pt) → fiber f b)
  (is_surj   : is_split_surjective g)

@[hott] lemma is_short_exact_of_is_exact {X A B C Y : Group}
...
@[hott] lemma is_short_exact_equiv {A B A' B' : Type _} {C C' : Type*}
...
begin
  ...
  { intros b p, note q := eq_of_inv_eq ((homotopy_top_of_hsquare' h₂ b)⁻¹ ⬝ p) ⬝ respect_pt eC,
...
@[hott] lemma is_exact_of_is_short_exact {A B : Type _} {C : Type*}
...
/- TODO: move and remove other versions -/
  ...
  @[hott] def is_surjective_qg_map {A : Group} (N : property A) [is_normal_subgroup A N] :
  ...
  @[hott] def is_surjective_ab_qg_map {A : AbGroup} (N : property A) [is_normal_subgroup A N] :
    is_surjective (ab_qg_map N) :=
  is_surjective_ab_qg_map _

  @[hott] def qg_map_eq_one {A : Group} {K : property A} [is_normal_subgroup A K] (g : A)
      (H : g ∈ K) :
    qg_map K g = 1 :=
  begin
  ...
  @[hott] def ab_qg_map_eq_one {A : AbGroup} {K : property A} [is_subgroup A K] (g : A)
      (H : g ∈ K) :
    ab_qg_map K g = 1 :=
  ab_qg_map_eq_one g H

@[hott] def is_short_exact_normal_subgroup {G : Group} (S : property G) [is_normal_subgroup G S] :
