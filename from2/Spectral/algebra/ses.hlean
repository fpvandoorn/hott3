...
@[hott] def SES_of_inclusion {A B : AbGroup} (f : A →g B) (Hf : is_embedding f) : SES A B (quotient_ab_group (image f)) :=
  begin
...
@[hott] def SES_of_subgroup {B : AbGroup} (S : property B) [is_subgroup B S] : SES (ab_subgroup S) B (quotient_ab_group S) :=
  begin
    fapply SES.mk,
    ...
    intros b p, fapply tr, fapply fiber.mk, fapply sigma.mk b, fapply rel_of_ab_qg_map_eq_one, exact p, reflexivity,
  end

@[hott] def SES_of_surjective_map {B C : AbGroup} (g : B →g C) (Hg : is_surjective g) : SES (ab_Kernel g) B C :=
  begin
    fapply SES.mk,
    ...
    intros b p, fapply tr, fapply fiber.mk, fapply sigma.mk, exact b, exact p, reflexivity,
  end

@[hott] def SES_of_homomorphism {A B : AbGroup} (f : A →g B) : SES (ab_Kernel f) A (ab_Image f) :=
  begin
    fapply SES.mk,
    ...
    intros a p, fapply tr, fapply fiber.mk, fapply sigma.mk, exact a,
    exact calc
...
@[hott] def SES_of_isomorphism_right {B C : AbGroup} (g : B ≃g C) : SES trivial_ab_group B C :=
  begin
    fapply SES.mk,
...
@[hott] def SES_iso_stable {A' B' C' : AbGroup} (f' : A' →g B') (g' : B' →g C') (α : A' ≃g A) (β : B' ≃g B) (γ : C' ≃g C) (Hαβ : f ∘g α ~ β ∘g f') (Hβγ : g ∘g β ~ γ ∘g g') : SES A' B' C' :=
  begin
    fapply SES.mk,
    exact f',
    exact g',
...
@[hott] def SES_of_triangle_left {A' : AbGroup} (α : A' ≃g A) (f' : A' →g B) (H : Π a' : A', f (α a') = f' a') : SES A' B C :=
begin
  fapply SES.mk,
  exact f',
  exact g,
...
--@[hott] def quotient_SES {A B C : AbGroup} (ses : SES A B C) :
--  quotient_ab_group (image_subgroup (SES.f ses)) ≃g C :=
--  begin
--    fapply ab_group_first_iso_thm B C (SES.g ses),
--  end

-- @[hott] def pre_right_extend_SES (to separate the following @[hott] def and replace C with B/A)

@[hott] def quotient_codomain_SES : B_mod_A ≃g C :=
  begin
...
@[hott] def quotient_triangle_SES : α ∘g q ~ g :=
  begin
    reflexivity
  end

@[hott] def quotient_triangle_extend_SES {C': AbGroup} (k : B →g C') :
  ...
  @[hott] def quotient_extend_unique_SES : is_contr (Σ (hC : C →g C'), hC ∘g g ~ g' ∘g hB) :=
  begin
...
@[hott] def quotient_extend_SES : quotient_ab_group (kernel_subgroup g) →g quotient_ab_group (kernel_subgroup g') :=
  begin
...
@[hott] def quotient_extend_SES_square : k ∘g (ab_qg_map (kernel_subgroup g)) ~ (ab_qg_map (kernel_subgroup g')) ∘g hB :=
  begin
    fapply quotient_group_compute
  end

@[hott] def right_extend_SES  : C →g C' :=
  α' ∘g k ∘g α⁻¹ᵍ

local abbreviation hC := right_extend_SES

@[hott] def right_extend_SES_square : hC ∘g g ~ g' ∘ hB :=
  begin
    exact calc
...
@[hott] def right_extend_SES_unique_map_aux (hC' : C →g C') (htpy2' : g' ∘g hB ~ hC' ∘g g) : k ∘g q ~  α'⁻¹ᵍ ∘g hC' ∘g α ∘g q :=
  begin
    exact calc
...
@[hott] def right_extend_SES_unique_map (hC' : C →g C') (htpy2' : hC' ∘g g ~ g' ∘g hB) : hC ~ hC' :=
  begin
    exact calc
...
@[hott] def right_extend_hom_SES : hom_SES ses ses' :=
  begin
    fapply hom_SES.mk,
    exact hA,
    exact hB,
