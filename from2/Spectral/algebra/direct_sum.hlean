...
namespace group
  ...
  section
    ...
    parameters {I : Type _} [is_set I] (Y : I → AbGroup)
    variables {A' : AbGroup} {Y' : I → AbGroup}

    @[hott] def dirsum_carrier : AbGroup := free_ab_group (Σi, Y i)
    local abbreviation ι := @free_ab_group_inclusion
    inductive dirsum_rel : dirsum_carrier → Type _ :=
    | rmk : Πi y₁ y₂, dirsum_rel (ι ⟨i, y₁⟩ * ι ⟨i, y₂⟩ *  (ι ⟨i, y₁ * y₂⟩)⁻¹)

    @[hott] def dirsum : AbGroup := quotient_ab_group_gen dirsum_carrier (λg, ∥dirsum_rel g∥)

    -- @[hott] def dirsum_carrier_incl (i : I) : Y i →g dirsum_carrier :=

    @[hott] def dirsum_incl (i : I) : Y i →g dirsum :=
    homomorphism.mk (λy, class_of (ι ⟨i, y⟩))
      begin intros g h, symmetry, apply gqg_eq_of_rel, apply tr, apply dirsum_rel.rmk end

    parameter {Y}
    @[hott] def dirsum.rec {P : dirsum → Type _} [H : Πg, is_prop (P g)]
    ...
    @[hott] def dirsum_homotopy {φ ψ : dirsum →g A'}
      (h : Πi (y : Y i), φ (dirsum_incl i y) = ψ (dirsum_incl i y)) : φ ~ ψ :=
    begin
      refine dirsum.rec _ _ _,
          exact h,
        refine !to_respect_zero ⬝ !to_respect_zero⁻¹,
      intros g₁ g₂ h₁ h₂, rewrite [* to_respect_mul, h₁, h₂]
    end

    @[hott] def dirsum_elim_resp_quotient (f : Πi, Y i →g A') (g : dirsum_carrier)
    ...
    @[hott] def dirsum_elim (f : Πi, Y i →g A') : dirsum →g A' :=
    gqg_elim _ (free_ab_group_elim (λv, f v.1 v.2)) (dirsum_elim_resp_quotient f)

    @[hott] def dirsum_elim_compute (f : Πi, Y i →g A') (i : I) (y : Y i) :
      dirsum_elim f (dirsum_incl i y) = f i y :=
    begin
      apply one_mul
    end

    @[hott] def dirsum_elim_unique (f : Πi, Y i →g A') (k : dirsum →g A')
  ...
  @[hott] def binary_dirsum (G H : AbGroup) : dirsum (bool.rec G H) ≃g G ×ag H :=
  ...
  variables {I J : Type _} [is_set I] [is_set J] {Y Y' Y'' : I → AbGroup}

  @[hott] def dirsum_functor (f : Πi, Y i →g Y' i) : dirsum Y →g dirsum Y' :=
  dirsum_elim (λi, dirsum_incl Y' i ∘g f i)

  @[hott] theorem dirsum_functor_compose (f' : Πi, Y' i →g Y'' i) (f : Πi, Y i →g Y' i) :
    dirsum_functor f' ∘g dirsum_functor f ~ dirsum_functor (λi, f' i ∘g f i) :=
  begin
    apply dirsum_homotopy,
    intros i y, reflexivity,
  end

  variable (Y)
  @[hott] def dirsum_functor_gid : dirsum_functor (λi, gid (Y i)) ~ gid (dirsum Y) :=
  begin
    apply dirsum_homotopy,
    intros i y, reflexivity,
  end
  ...
  @[hott] def dirsum_functor_mul (f f' : Πi, Y i →g Y' i) :
    homomorphism_mul (dirsum_functor f) (dirsum_functor f') ~
    dirsum_functor (λi, homomorphism_mul (f i) (f' i)) :=
  begin
    apply dirsum_homotopy,
    intros i y, exact sorry
  end

  @[hott] def dirsum_functor_homotopy (f f' : Πi, Y i →g Y' i) (p : f ~2 f') :
  ...
  @[hott] def dirsum_functor_left (f : J → I) : dirsum (Y ∘ f) →g dirsum Y :=
  dirsum_elim (λj, dirsum_incl Y (f j))

  @[hott] def dirsum_isomorphism (f : Πi, Y i ≃g Y' i) : dirsum Y ≃g dirsum Y' :=
...
namespace group
  ...
  @[hott] def dirsum_down_left.{u v w} {I : Type u} [is_set I] (Y : I → AbGroup.{w})
