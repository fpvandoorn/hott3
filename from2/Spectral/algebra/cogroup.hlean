...
section
  ...
  @[hott] def pair_phomotopy {f g : C →* A ×* B}
...
-- should be in wedge
@[hott] def or_of_wedge {A B : Type*} (w : wedge A B)
...
@[hott] def pdiag (A : Type*) : A →* (A ×* A) :=
pmap.mk (λ a, (a, a)) idp

section prod
  variables (A B : Type*)

  @[hott] def wpr1 (A B : Type*) : (A ∨ B) →* A :=
  pmap.mk (wedge.elim (pid A) (pconst B A) idp) idp

  @[hott] def wpr2 (A B : Type*) : (A ∨ B) →* B :=
  pmap.mk (wedge.elim (pconst A B) (pid B) idp) idp

  @[hott] def ppr1_pprod_of_wedge (A B : Type*)
  ...
  @[hott] def ppr2_pprod_of_wedge (A B : Type*)
...
@[hott] def co_h_space_of_counit_laws {A : Type*}
  (c : A →* (A ∨ A))
...
section
  ...
  @[hott] def counit_left : wpr1 A A ∘* comul A ~* pid A :=
  calc
    wpr1 A A ∘* comul A
  ...
  @[hott] def counit_right : wpr2 A A ∘* comul A ~* pid A :=
  calc
    wpr2 A A ∘* comul A
  ...
  @[hott] def is_conn_co_h_space : is_conn 0 A :=
  begin
...
section
  ...
  @[hott] def co_h_space_susp : co_h_space (⅀ A) :=
