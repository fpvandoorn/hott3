...
namespace group
  ...
  structure is_subgroup [class] (G : Group) (H : property G) : Type _ :=
    (one_mem : 1 ∈ H)
    (mul_mem : Π{g h}, g ∈ H → h ∈ H → g * h ∈ H)
    (inv_mem : Π{g}, g ∈ H → g⁻¹ ∈ H)

  @[hott] def is_prop_is_subgroup [instance] (G : Group) (H : property G) : is_prop (is_subgroup G H) :=
  ...
  @[hott] def trivial_subgroup [instance] (G : Group) : is_subgroup G '{1} :=
  begin
  ...
  @[hott] def full_subgroup [instance] (G : Group) : is_subgroup G univ :=
  begin
    fapply is_subgroup.mk,
    { apply logic.trivial },
  ...
  /-- Every group homomorphism f : G -> H determines a subgroup of H,
      ...
      following @[hott] def we define the image of f. Since a subgroup
      ...
      of the @[hott] def of the image of f. --/

  @[hott] def is_subgroup_image [instance] {G : Group} {H : Group} (f : G →g H) :
  ...
  @[hott] def kernel (φ : G₁ →g G₂) : property G₁ := { g | trunctype.mk (φ g = 1) _}

  @[hott] theorem mul_mem_kernel (φ : G₁ →g G₂) (g h : G₁) (H₁ : g ∈ kernel φ) (H₂ : h ∈ kernel φ) : g * h ∈ kernel φ :=
  calc
  ...
  @[hott] theorem inv_mem_kernel (φ : G₁ →g G₂) (g : G₁) (H : g ∈ kernel φ) : g⁻¹ ∈ kernel φ :=
  calc
    φ g⁻¹ = (φ g)⁻¹ : to_respect_inv
      ... = 1⁻¹     : H
      ... = 1       : one_inv

  @[hott] def is_subgroup_kernel [instance] (φ : G₁ →g G₂) : is_subgroup G₁ (kernel φ) :=
  ⦃ is_subgroup,
  ...
  /-- Now we should be able to show that if f is a homomorphism for which the kernel is trivial and the
     image is full, then f is an isomorphism, except that no one defined the proposition that f is an
      isomorphism :/ --/
  /- @[hott] def is_iso_from_kertriv_imfull {G H : Group} (f : G →g H) :
  ...
  @[hott] def is_normal.{u v} (G : Group) (N : property.{u v} G) : Prop :=
...
section
  ...
  @[hott] theorem is_normal_subgroup' (h : G) (r : g ∈ N) : h⁻¹ * g * h ∈ N :=
  inv_inv h ▸ subgroup_is_normal N h⁻¹ r

  @[hott] def is_normal_subgroup_ab {C : property A} (subgrpA : is_subgroup A C)
  ...
  @[hott] theorem is_normal_subgroup_rev (h : G) (r : h * g * h⁻¹ ∈ N) : g ∈ N :=
  ...
  @[hott] theorem normal_subgroup_insert (r : k ∈ N) (r' : g * h ∈ N) : g * (k * h) ∈ N :=
  ...
  @[hott] theorem is_normal_subgroup_kernel {G₁ G₂ : Group} (φ : G₁ →g G₂) (g : G₁) (h : G₁)
  ...
  @[hott] def sg {G : Group} (H : property G) : Type _ := subtype (λ x, x ∈ H)
  local attribute sg [reducible]

  @[hott] def subgroup_one : sg H := ⟨one, subgroup_one_mem H⟩
  @[hott] def subgroup_inv : sg H → sg H :=
  λv, ⟨v.1⁻¹, subgroup_inv_mem v.2⟩
  @[hott] def subgroup_mul : sg H → sg H → sg H :=
  λv w, ⟨v.1 * w.1, subgroup_mul_mem v.2 w.2⟩

  @[hott] def subgroup_elt (x : sg H) : G := x.1
  ...
  @[hott] theorem subgroup_mul_assoc (g₁ g₂ g₃ : sg H) : g₁ * g₂ * g₃ = g₁ * (g₂ * g₃) :=
  subtype_eq !mul.assoc

  @[hott] theorem subgroup_one_mul (g : sg H) : 1 * g = g :=
  subtype_eq !one_mul

  @[hott] theorem subgroup_mul_one (g : sg H) : g * 1 = g :=
  subtype_eq !mul_one

  @[hott] theorem subgroup_mul_left_inv (g : sg H) : g⁻¹ * g = 1 :=
  subtype_eq !mul.left_inv

  @[hott] theorem subgroup_mul_comm {G : AbGroup} {H : property G} [subgrpH : is_subgroup G H] (g h : sg H)
  ...
  @[hott] def subgroup : Group :=
  Group.mk _ (group_sg H)

  variable {H}

  @[hott] def ab_group_sg {G : AbGroup} (H : property G) [is_subgroup G H]
    : ab_group (sg H) :=
  ⦃ab_group, (group_sg H), mul_comm := subgroup_mul_comm⦄

  @[hott] def ab_subgroup {G : AbGroup} (H : property G) [is_subgroup G H]
    : AbGroup :=
  AbGroup.mk _ (ab_group_sg H)

  @[hott] def Kernel {G H : Group} (f : G →g H) : Group := subgroup (kernel f)

  @[hott] def ab_Kernel {G H : AbGroup} (f : G →g H) : AbGroup := ab_subgroup (kernel f)

  @[hott] def incl_of_subgroup {G : Group} (H : property G) [is_subgroup G H] :
    subgroup H →g G :=
  begin
  ...
  @[hott] def is_embedding_incl_of_subgroup {G : Group} (H : property G) [is_subgroup G H] :
  ...
  @[hott] def ab_Kernel_incl {G H : AbGroup} (f : G →g H) : ab_Kernel f →g G :=
  begin
    fapply incl_of_subgroup,
  end

  @[hott] def is_embedding_ab_kernel_incl {G H : AbGroup} (f : G →g H) : is_embedding (ab_Kernel_incl f) :=
  begin
    fapply is_embedding_incl_of_subgroup,
  end

  @[hott] def is_subgroup_of_is_subgroup {G : Group} {H1 H2 : property G} [is_subgroup G H1]
  ...
  @[hott] def Image {G H : Group} (f : G →g H) : Group :=
    subgroup (image f)

  @[hott] def ab_Image {G : AbGroup} {H : Group} (f : G →g H) : AbGroup :=
  ...
  @[hott] def image_incl {G H : Group} (f : G →g H) : Image f →g H :=
    incl_of_subgroup (image f)

@[hott] def ab_Image_incl {A B : AbGroup} (f : A →g B) : ab_Image f →g B := incl_of_subgroup (image f)

@[hott] def is_equiv_surjection_ab_image_incl {A B : AbGroup} (f : A →g B) (H : is_surjective f) : is_equiv (ab_Image_incl f ) :=
  begin
...
@[hott] def iso_surjection_ab_image_incl {A B : AbGroup} (f : A →g B) (H : is_surjective f) : ab_Image f ≃g B :=
  begin
  ...
  @[hott] def hom_lift {G H : Group} (f : G →g H) (K : property H) [is_subgroup H K] (Hyp : Π (g : G), K (f g)) : G →g subgroup K :=
  begin
    fapply homomorphism.mk,
    intro g,
    fapply sigma.mk,
    exact f g,
    fapply Hyp,
    intros g h, apply subtype_eq, esimp, apply respect_mul
  end

@[hott] def hom_factors_through_lift {G H : Group} (f : G →g H) (K : property H) [is_subgroup H K]  (Hyp : Π (g : G), K (f g)) :
...
@[hott] def ab_hom_lift {G H : AbGroup} (f : G →g H) (K : property H) [is_subgroup H K] (Hyp : Π (g : G), K (f g)) : G →g ab_subgroup K :=
  begin
    fapply homomorphism.mk,
    intro g,
    fapply sigma.mk,
    exact f g,
    fapply Hyp,
    intros g h, apply subtype_eq, apply respect_mul,
  end

@[hott] def ab_hom_factors_through_lift {G H : AbGroup} (f : G →g H) (K : property H) [is_subgroup H K] (Hyp : Π (g : G), K (f g)) :
f = incl_of_subgroup K ∘g hom_lift f K Hyp :=
  begin
  fapply homomorphism_eq,
    reflexivity
...
@[hott] def ab_hom_lift_kernel {A B C : AbGroup} (f : A →g B) (g : B →g C) (Hyp : Π (a : A), g (f a) = 1) : A →g ab_Kernel g :=
  begin
    fapply ab_hom_lift,
    exact f,
    intro a,
...
@[hott] def ab_hom_lift_kernel_factors {A B C : AbGroup} (f : A →g B) (g : B →g C) (Hyp : Π (a : A), g (f a) = 1) :
f = ab_Kernel_incl g ∘g ab_hom_lift_kernel f g Hyp :=
  begin
    fapply ab_hom_factors_through_lift,
  end

  @[hott] def image_lift {G H : Group} (f : G →g H) : G →g Image f :=
  begin
    fapply hom_lift f,
    intro g,
    apply tr,
...
@[hott] def ab_image_lift {G H : AbGroup} (f : G →g H) : G →g Image f :=
  begin
    fapply hom_lift f,
    intro g,
    apply tr,
  ...
  @[hott] def is_surjective_image_lift {G H : Group} (f : G →g H) : is_surjective (image_lift f) :=
  begin
    intro h,
  ...
  @[hott] def image_factor {G H : Group} (f : G →g H) : f = (image_incl f) ∘g (image_lift f) :=
  begin
    fapply homomorphism_eq,
    reflexivity
  end

  @[hott] def image_incl_injective {G H : Group} (f : G →g H) : Π (x y : Image f),
  ...
  @[hott] def image_incl_eq_one {G H : Group} (f : G →g H) :
  ...
  @[hott] def image_elim_@[hott] lemma {f₁ : G₁ →g G₂} {f₂ : G₁ →g G₃} (h : Π⦃g⦄, f₁ g = 1 → f₂ g = 1)
  ...
  open image
  @[hott] def image_elim {f₁ : G₁ →g G₂} (f₂ : G₁ →g G₃) (h : Π⦃g⦄, f₁ g = 1 → f₂ g = 1) :
    Image f₁ →g G₃ :=
  homomorphism.mk (total_image.elim_set f₂ (image_elim_@[hott] lemma h))
  begin
  ...
  @[hott] def image_homomorphism {A B C : AbGroup} (f : A →g B) (g : B →g C) :
  ...
  @[hott] def image_homomorphism_square {A B C : AbGroup} (f : A →g B) (g : B →g C) :
  ...
  @[hott] def subgroup_functor_fun (φ : G →g H) (h : Πg, g ∈ R → φ g ∈ S)
      (x : subgroup R) :
    subgroup S :=
  begin
  ...
  @[hott] def subgroup_functor (φ : G →g H)
    (h : Πg, g ∈ R → φ g ∈ S) : subgroup R →g subgroup S :=
  begin
    fapply homomorphism.mk,
    { exact subgroup_functor_fun φ h },
    { intros x₁ x₂, induction x₁ with g₁ hg₁, induction x₂ with g₂ hg₂,
      exact sigma_eq !to_respect_mul !is_prop.elimo }
  end

  @[hott] def ab_subgroup_functor {G H : AbGroup}
  ...
  @[hott] theorem subgroup_functor_compose (ψ : H →g K) (φ : G →g H)
  ...
  @[hott] def subgroup_functor_gid : subgroup_functor (gid G) (λg, id) ~ gid (subgroup R) :=
  begin
    intro g, induction g with g hg, reflexivity
  end

  @[hott] def subgroup_functor_mul {G H : AbGroup} {R : property G} [subgroupR : is_subgroup G R]
  ...
  @[hott] def subgroup_functor_homotopy {ψ φ : G →g H} (hψ : Πg, R g → S (ψ g))
  ...
  @[hott] def subgroup_of_subgroup_incl {R S : property G} [is_subgroup G R] [is_subgroup G S]
    (H : Π (g : G), g ∈ R → g ∈ S) : subgroup R →g subgroup S
  :=
  subgroup_functor (gid G) H

  @[hott] def is_embedding_subgroup_of_subgroup_incl {R S : property G} [is_subgroup G R] [is_subgroup G S]
  ...
  @[hott] def ab_subgroup_of_subgroup_incl {A : AbGroup} {R S : property A} [is_subgroup A R] [is_subgroup A S]
    (H : Π (a : A), a ∈ R → a ∈ S) : ab_subgroup R →g ab_subgroup S
  :=
  ab_subgroup_functor (gid A) H

  @[hott] def is_embedding_ab_subgroup_of_subgroup_incl {A : AbGroup} {R S : property A} [is_subgroup A R] [is_subgroup A S]
    (H : Π (a : A), a ∈ R → a ∈ S) : is_embedding (ab_subgroup_of_subgroup_incl H) :=
  begin
    fapply is_embedding_subgroup_of_subgroup_incl
  end

  @[hott] def ab_subgroup_iso {A : AbGroup} {R S : property A} [is_subgroup A R] [is_subgroup A S]
  ...
  @[hott] def ab_subgroup_iso_triangle {A : AbGroup} {R S : property A} [is_subgroup A R] [is_subgroup A S]
      (H : Π (a : A), a ∈ R → a ∈ S) (K : Π (a : A), a ∈ S → a ∈ R) :
