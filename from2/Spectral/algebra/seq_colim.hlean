...
namespace group
  ...
  section
    ...
    @[hott] def seq_colim_carrier : AbGroup := dirsum A
    inductive seq_colim_rel : seq_colim_carrier → Type _ :=
    | rmk : Πi a, seq_colim_rel ((dirsum_incl A i a) * (dirsum_incl A (i + 1) (f i a))⁻¹)

    @[hott] def seq_colim : AbGroup := quotient_ab_group_gen seq_colim_carrier (λa, ∥seq_colim_rel a∥)

    parameters {A f}
    @[hott] def seq_colim_incl (i : ℕ) : A i →g seq_colim :=
      gqg_map _ _ ∘g dirsum_incl A i

    @[hott] def seq_colim_quotient (h : Πi, A i →g A') (k : Πi a, h i a = h (succ i) (f i a))
    ...
    @[hott] def seq_colim_glue {i : @trunctype.mk 0 ℕ _} {a : A i} : seq_colim_incl i a = seq_colim_incl (succ i) (f i a) :=
    begin
    ...
    section
      ...
      @[hott] def seq_colim_unique (m : seq_colim →g A') :
  ...
  @[hott] def seq_colim_functor {A A' : ℕ → AbGroup}
  ...
  -- @[hott] def seq_colim_functor_compose {A A' A'' : ℕ → AbGroup}
