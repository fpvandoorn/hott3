...
namespace group
  ...
  /- Free Group of a set -/
  variables (X : Type _) [is_set X] {l l' : list (X ⊎ X)}
  namespace free_group

  inductive free_group_rel : list (X ⊎ X) → list (X ⊎ X) → Type _ :=
  ...
  @[hott] def free_group_carrier [reducible] : Type _ := set_quotient (λx y, ∥R X x y∥)
  ...
  @[hott] def free_group_one : FG X := class_of []
  @[hott] def free_group_inv : FG X → FG X :=
  quotient_unary_map (reverse ∘ map sum.flip)
                     (λl l', trunc_functor -1 (rel_respect_reverse ∘ rel_respect_flip))
  @[hott] def free_group_mul : FG X → FG X → FG X :=
  ...
  @[hott] theorem free_group_mul_assoc (g₁ g₂ g₃ : FG X) : g₁ * g₂ * g₃ = g₁ * (g₂ * g₃) :=
  begin
  ...
  @[hott] theorem free_group_one_mul (g : FG X) : 1 * g = g :=
  begin
    refine set_quotient.rec_prop _ g, clear g, intro g,
    exact ap class_of !append_nil_left
  end

  @[hott] theorem free_group_mul_one (g : FG X) : g * 1 = g :=
  begin
    refine set_quotient.rec_prop _ g, clear g, intro g,
    exact ap class_of !append_nil_right
  end

  @[hott] theorem free_group_mul_left_inv (g : FG X) : g⁻¹ * g = 1 :=
  begin
    refine set_quotient.rec_prop _ g, clear g, intro g,
...
--  export [reduce_hints] free_group
  variables (X)
  @[hott] def group_free_group : group (free_group_carrier X) :=
  ...
  @[hott] def free_group : Group :=
  Group.mk _ (group_free_group X)

  /- The universal property of the free group -/
  variables {X G}
  @[hott] def free_group_inclusion (x : X) : free_group X :=
  class_of [inl x]

  @[hott] def fgh_helper (f : X → G) (g : G) (x : X + X) : G :=
  g * sum.rec (λx, f x) (λx, (f x)⁻¹) x

  @[hott] theorem fgh_helper_respect_rel (f : X → G) (r : free_group_rel X l l')
  ...
  @[hott] theorem fgh_helper_mul (f : X → G) (l)
  ...
  @[hott] def free_group_hom (f : X → G) : free_group X →g G :=
  begin
  ...
  @[hott] def fn_of_free_group_hom (φ : free_group X →g G) : X → G :=
  φ ∘ free_group_inclusion

  variables (X G)
  @[hott] def free_group_hom_equiv_fn : (free_group X →g G) ≃ (X → G) :=
  begin
    fapply equiv.MK,
