...
/- fin -/
  ...
  -- @[hott] def stratified_succ2 {N : succ_str} {n : ℕ} (x : stratified_type N n)
  --   : stratified_type N n :=
  -- (nat.cases_on (pr2 x) (S (pr1 x)) (λa, pr1 x), cyclic_pred (pr2 x))

  -- @[hott] def stratified2 [reducible] (N : succ_str) (n : ℕ) : succ_str :=
  -- succ_str.mk (stratified_type N n) stratified_succ2


namespace chain_complex

@[hott] def stratified_succ_max {N : succ_str} {n : ℕ} (x : stratified N n) (p : val (pr2 x) = n)
  : S x = (S (pr1 x), 0) :=
begin
  ...
  @[hott] def splice_type {N M : succ_str} (G : N → chain_complex M) (m : M)
    (x : stratified N 2) : Set* :=
  G x.1 (m +' val x.2)

  @[hott] def splice_map {N M : succ_str} (G : N → chain_complex M) (m : M)
  ...
  @[hott] def is_chain_complex_splice_map {N M : succ_str} (G : N → chain_complex M) (m : M)
