...
namespace cohomology
  ...
  @[hott] def pred_fun {A : ℕ → Type*} (f : Πn, A n →* A (n+1)) (n : ℕ) : A (pred n) →* A n :=
  begin cases n with n, exact pconst (A 0) (A 0), exact f n end

  @[hott] def type_chain_complex_snat' (A : ℕ → Type*) (f : Πn, A n →* A (n+1))
  ...
  @[hott] def chain_complex_snat' (A : ℕ → Set*) (f : Πn, A n →* A (n+1))
  ...
  @[hott] def is_exact_at_t_snat' {A : ℕ → Type*} (f : Πn, A n →* A (n+1))
    (p : Πn (x : A n), f (n+1) (f n x) = pt) (q : Πn x, f (n+1) x = pt → fiber (f n) x) (n : ℕ)
    : is_exact_at_t (type_chain_complex_snat' A f p) (n+2) :=
  q n

  @[hott] def cofiber_sequence_helper (v : Σ(X Y : Type*), X →* Y)
    : Σ(Y Z : Type*), Y →* Z :=
  ⟨v.2.1, pcofiber v.2.2, pcod v.2.2⟩

  @[hott] def cofiber_sequence_helpern (v : Σ(X Y : Type*), X →* Y) (n : ℕ)
    : Σ(Z X : Type*), Z →* X :=
  ...
  @[hott] def cohomology_groups [reducible] : -3ℤ → AbGroup
  | (n, fin.mk 0 p) := H n X
  | (n, fin.mk 1 p) := H n Y
  | (n, fin.mk k p) := H n (pcofiber f)

--   @[hott] def cohomology_groups_pequiv_loop_spaces2 [reducible]
...
--   | (n, fin.mk (k+3) p) := begin exfalso, apply lt_le_antisymm H, apply le_add_left end
  ...
  @[hott] def coboundary (n : ℤ) : H (pred n) X →g H n (pcofiber f) :=
  H ^→ n (pcofiber_pcod f ∘* pcod (pcod f)) ∘g (Hsusp_neg H n X)⁻¹ᵍ

  @[hott] def cohomology_groups_fun : Π(n : -3ℤ), cohomology_groups (S n) →g cohomology_groups n
  ...
  -- @[hott] def cohomology_groups_fun_pcohomology_loop_spaces_fun2 [reducible]
  ...
  @[hott] def cohomology_groups_chain_0 (n : ℤ) (x : H n (pcofiber f)) : H ^→ n f (H ^→ n (pcod f) x) = 1 :=
  begin
  ...
  @[hott] def cohomology_groups_chain_1 (n : ℤ) (x : H (pred n) X) : H ^→ n (pcod f) (coboundary n x) = 1 :=
  begin
  ...
  @[hott] def cohomology_groups_chain_2 (n : ℤ) (x : H (pred n) Y) : coboundary n (H ^→ (pred n) f x) = 1 :=
  begin
    exact sorry
...
--Hsusp_neg_inv_natural H n (pcofiber_pcod f ∘* pcod (pcod f)) _
  ...
  @[hott] def cohomology_groups_chain : Π(n : -3ℤ) (x : cohomology_groups (S (S n))),
  ...
  @[hott] def LES_of_cohomology_groups : chain_complex -3ℤ :=
  chain_complex.mk (λn, cohomology_groups n) (λn, pmap_of_homomorphism (cohomology_groups_fun n)) cohomology_groups_chain

  @[hott] def is_exact_LES_of_cohomology_groups : is_exact LES_of_cohomology_groups :=
  begin
    intro n,
    exact sorry
  end
