...
@[hott] def unchoose (n : ℕ₋₂) {X : Type _} (A : X → Type _) : trunc n (Πx, A x) → Πx, trunc n (A x) :=
trunc.elim (λf x, tr (f x))

@[hott] def has_choice.{u} [class] (n : ℕ₋₂) (X : Type u) : Type u+1 :=
Π(A : X → Type u), is_equiv (unchoose n A)

@[hott] def choice_equiv.{u} {n : ℕ₋₂} {X : Type u} [H : has_choice n X] (A : X → Type u)
  : trunc n (Πx, A x) ≃ (Πx, trunc n (A x)) :=
equiv.mk _ (H A)

@[hott] def has_choice_of_succ (X : Type _) (H : Πk, has_choice (k.+1) X) (n : ℕ₋₂) : has_choice n X :=
begin
  cases n with n,
...
@[hott] def has_choice_empty [instance] (n : ℕ₋₂) : has_choice n empty :=
begin
...
@[hott] def has_choice_unit [instance] : Πn, has_choice n unit :=
begin
  intros n A, fapply adjointify,
...
@[hott] def has_choice_sum.{u} [instance] (n : ℕ₋₂) (A B : Type u)
...
@[hott] def has_choice_equiv_closed.{u} (n : ℕ₋₂) {A B : Type u} (f : A ≃ B) (hA : has_choice n B)
  : has_choice n A :=
begin
  induction f using rec_on_ua_idp, assumption
end

@[hott] def has_choice_bool [instance] (n : ℕ₋₂) : has_choice n bool :=
has_choice_equiv_closed n bool_equiv_unit_sum_unit _

@[hott] def has_choice_lift.{u v} [instance] (n : ℕ₋₂) (A : Type _) [has_choice n A] :
  has_choice n (lift.{u v} A) :=
sorry --has_choice_equiv_closed n !equiv_lift⁻¹ᵉ _

@[hott] def has_choice_punit [instance] (n : ℕ₋₂) : has_choice n punit := has_choice_unit n
@[hott] def has_choice_pbool [instance] (n : ℕ₋₂) : has_choice n pbool := has_choice_bool n
@[hott] def has_choice_plift [instance] (n : ℕ₋₂) (A : Type*) [has_choice n A]
  : has_choice n (plift A) := has_choice_lift n A
@[hott] def has_choice_psum [instance] (n : ℕ₋₂) (A B : Type*) [has_choice n A] [has_choice n B]
