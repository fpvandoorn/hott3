...
namespace homology
  ...
  section
    ...
    @[hott] theorem HH_base_indep (n : ℤ) {A : Type _} (a b : A)
    ...
    @[hott] theorem Hh_homotopy' (n : ℤ) {A B : Type*} (f : A → B) (p q : f pt = pt)
    ...
    @[hott] theorem Hh_homotopy (n : ℤ) {A B : Type*} (f g : A →* B) (h : f ~ g) : Hh theory n f ~ Hh theory n g := λ x,
    ...
    @[hott] def HH_isomorphism (n : ℤ) {A B : Type*} (e : A ≃* B)
    ...
    begin
      ...
      { refine dirsum_homotopy _, intros i y,
    ...
    @[hott] def Hadditive'_equiv (n : ℤ) {I : Type} [is_set I] (X : I → Type*)
      : dirsum (λi, HH theory n (X i)) ≃g HH theory n (⋁ X) :=
    isomorphism.mk (dirsum_elim (λi, Hh theory n (fwedge.pinl i))) (Hadditive' n X)

    @[hott] def Hfwedge (n : ℤ) {I : Type _} [is_set I] (X : I → Type*): HH theory n (⋁ X) ≃g dirsum (λi, HH theory n (X i)) :=
      (isomorphism.mk _ (Hadditive theory n X))⁻¹ᵍ

    @[hott] def Hwedge (n : ℤ) (A B : Type*) : HH theory n (wedge A B) ≃g HH theory n A ×g HH theory n B :=
  ...
  section
    ...
    @[hott] def Hplift_susp (n : ℤ) (A : Type*): HH theory (n + 1) (plift.{u v} (susp A)) ≃g HH theory n (plift.{u v} A) :=
    ...
    @[hott] def Hplift_wedge (n : ℤ) (A B : Type*): HH theory n (plift.{u v} (A ∨ B)) ≃g HH theory n (plift.{u v} A) ×g HH theory n (plift.{u v} B) :=
    ...
    @[hott] def Hplift_isomorphism (n : ℤ) {A B : Type*} (e : A ≃* B) : HH theory n (plift.{u v} A) ≃g HH theory n (plift.{u v} B) :=
      HH_isomorphism theory n (!pequiv_plift⁻¹ᵉ* ⬝e* e ⬝e* !pequiv_plift)

  end

/- homology theory associated to a prespectrum -/
@[hott] def homology (X : Type*) (E : prespectrum) (n : ℤ) : AbGroup :=
pshomotopy_group n (smash_prespectrum X E)

/- an alternative @[hott] def, which might be a bit harder to work with -/
@[hott] def homology_spectrum (X : Type*) (E : spectrum) (n : ℤ) : AbGroup :=
shomotopy_group n (smash_spectrum X E)

@[hott] def parametrized_homology {X : Type*} (E : X → spectrum) (n : ℤ) : AbGroup :=
sorry

@[hott] def ordinary_homology [reducible] (X : Type*) (G : AbGroup) (n : ℤ) : AbGroup :=
homology X (EM_spectrum G) n

@[hott] def ordinary_homology_Z [reducible] (X : Type*) (n : ℤ) : AbGroup :=
...
@[hott] def unpointed_homology (X : Type _) (E : spectrum) (n : ℤ) : AbGroup :=
H_ n[X₊, E]

@[hott] def homology_functor {X Y : Type*} {E F : prespectrum} (f : X →* Y)
  (g : E →ₛ F) (n : ℤ) : homology X E n →g homology Y F n :=
pshomotopy_group_fun n (smash_prespectrum_fun f g)

@[hott] def homology_theory_spectrum_is_exact.{u} (E : spectrum.{u}) (n : ℤ) {X Y : Type*}
...
@[hott] def homology_theory_spectrum.{u} (E : spectrum.{u}) : homology_theory.{u} :=
begin
