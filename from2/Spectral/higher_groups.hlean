...
namespace higher_group
  ...
  @[hott] lemma is_trunc_B (G : [n;k]Grp) : is_trunc (n+k) (B G) :=
  sorry

  /- some equivalences -/
  @[hott] def Grp_equiv (n k : ℕ) : [n;k]Grp ≃ (n+k)-Type*[k] :=
  sorry

  @[hott] def InfGrp_equiv (k : ℕ) : [∞;k]Grp ≃ Type*[k] :=
  sorry

  -- maybe to do: ωGrp ≃ Σ(X : spectrum), is_sconn n X

  /- Constructions -/
  @[hott] def Decat (G : [n+1;k]Grp) : [n;k]Grp :=
  ...
  @[hott] def Disc (G : [n;k]Grp) : [n+1;k]Grp :=
  Grp.mk (ptrunctype.mk G (show is_trunc (n.+1) G, from _) pt) (B G) (e G)

  @[hott] def Disc_adjoint_Decat (G : [n;k]Grp) (H : [n+1;k]Grp) :
    ppmap (B (Disc G)) (B H) ≃* ppmap (B G) (B (Decat H)) :=
  sorry
  /- To do: naturality -/

  @[hott] def Decat_Disc (G : [n;k]Grp) : Decat (Disc G) = G :=
  sorry

  @[hott] def InfDecat (n : ℕ) (G : [∞;k]Grp) : [n;k]Grp :=
  ...
  @[hott] def InfDisc (n : ℕ) (G : [n;k]Grp) : [∞;k]Grp :=
  InfGrp.mk G (B G) (e G)

  @[hott] def InfDisc_adjoint_InfDecat (G : [n;k]Grp) (H : [∞;k]Grp) :
    ppmap (iB (InfDisc n G)) (iB H) ≃* ppmap (B G) (B (InfDecat n H)) :=
  sorry
  /- To do: naturality -/

  @[hott] def InfDecat_InfDisc (G : [n;k]Grp) : InfDecat n (InfDisc n G) = G :=
  sorry

  @[hott] def Loop (G : [n+1;k]Grp) : [n;k+1]Grp :=
  ...
  @[hott] def Deloop (G : [n;k+1]Grp) : [n+1;k]Grp :=
  ...
  @[hott] def Forget (G : [n;k+1]Grp) : [n;k]Grp :=
  have is_conn (k.+1) (B G), from !is_conn_pconntype,
  ...
  @[hott] def Stabilize (G : [n;k]Grp) : [n;k+1]Grp :=
  ...
  @[hott] def ωForget (k : ℕ) (G : [n;ω]Grp) : [n;k]Grp :=
  ...
  @[hott] def nStabilize.{u} (H : k ≤ l) (G : Grp.{u} n k) : Grp.{u} n l :=
  begin
    induction H with l H IH, exact G, exact Stabilize IH
  end

  @[hott] theorem stabilization (H : k ≥ n + 2) : is_equiv (@Stabilize n k) :=
  sorry

  @[hott] def ωStabilize_of_le (H : k ≥ n + 2) (G : [n;k]Grp) : [n;ω]Grp :=
  ωGrp.mk (λl, sorry) (λl, sorry)

  /- for l ≤ k we want to define it as Ω[k-l] (B G),
     for H : l ≥ k we want to define it as nStabilize H G -/

  @[hott] def ωStabilize (G : [n;k]Grp) : [n;ω]Grp :=
