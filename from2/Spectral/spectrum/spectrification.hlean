...
namespace spectrum
  ...
  /- Prespectrification -/
  @[hott] def is_sconnected {N : succ_str} {X Y : gen_prespectrum N} (h : X →ₛ Y) : Type _ :=
  ...
  @[hott] def prespectrification {N : succ_str} (X : gen_prespectrum N) : gen_prespectrum N :=
  ...
  @[hott] def to_prespectrification_pfun {N : succ_str} (X : gen_prespectrum N) (n : N) : X n →* prespectrification X n :=
  glue X n

  -- And similarly we need the pointed homotopies witnessing that the squares commute
  @[hott] def to_prespectrification_psq {N : succ_str} (X : gen_prespectrum N) (n : N) :
  ...
  @[hott] def to_prespectrification {N : succ_str} (X : gen_prespectrum N) : X →ₛ prespectrification X :=
  begin
    fapply smap.mk,
  ...
  @[hott] def is_sequiv_smap_to_prespectrification_of_is_spectrum {N : succ_str} (E : gen_prespectrum N) (H : is_spectrum E) : is_sequiv_smap (to_prespectrification E) :=
  begin
    intro n, induction E, induction H, exact is_equiv_glue n,
  end

  -- If η : E → prespectrification E is an equivalence, then E is a spectrum.
  @[hott] def is_spectrum_of_is_sequiv_smap_to_prespectrification {N : succ_str} (E : gen_prespectrum N) (H : is_sequiv_smap (to_prespectrification E)) : is_spectrum E :=
  begin
  ...
  @[hott] def is_sconnected_to_prespectrification_inv_pfun {N : succ_str} {X : gen_prespectrum N} {E : gen_spectrum N} : (X →ₛ E) → Π (n : N), prespectrification X n →* E n :=
  λ (f : X →ₛ E) n, (equiv_glue E n)⁻¹ᵉ* ∘* Ω→ (f (S n))

  -- In the following we define the commuting squares of the extension
  @[hott] def is_sconnected_to_prespectrification_inv_psq {N : succ_str} {X : gen_prespectrum N} {E : gen_spectrum N} (f : X →ₛ E) (n : N) :
  ...
  -- Now we bundle the @[hott] def into a map (X →ₛ E) → (prespectrification X →ₛ E)
  @[hott] def is_sconnected_to_prespectrification_inv {N : succ_str} (X : gen_prespectrum N) (E : gen_spectrum N)
  ...
  @[hott] def is_sconnected_to_prespectrification_isretr_pfun {N : succ_str} {X : gen_prespectrum N} {E : gen_spectrum N} (f : prespectrification X →ₛ E) (n : N) : to_fun (is_sconnected_to_prespectrification_inv X E (f ∘ₛ to_prespectrification X)) n ~* to_fun f n := begin exact sorry end

  @[hott] def is_sconnected_to_prespectrification_isretr_psq {N : succ_str} {X : gen_prespectrum N} {E : gen_spectrum N} (f : prespectrification X →ₛ E) (n : N) :
  ...
  @[hott] def is_sconnected_to_prespectrification_isretr {N : succ_str} (X : gen_prespectrum N) (E : gen_spectrum N) (f : prespectrification X →ₛ E) : is_sconnected_to_prespectrification_inv X E (f ∘ₛ to_prespectrification X) = f :=
  begin
  ...
  @[hott] def is_sconnected_to_prespectrification_issec_pfun {N : succ_str} {X : gen_prespectrum N} {E : gen_spectrum N} (g : X →ₛ E) (n : N) :
  ...
  @[hott] def is_sconnected_to_prespectrification_issec_psq {N : succ_str} {X : gen_prespectrum N} {E : gen_spectrum N} (g : X →ₛ E) (n : N) :
  ...
  @[hott] def is_sconnected_to_prespectrification_issec {N : succ_str} (X : gen_prespectrum N) (E : gen_spectrum N) (g : X →ₛ E) :
  ...
  @[hott] def is_sconnected_to_prespectrify {N : succ_str} (X : gen_prespectrum N) :
  ...
  -- Conjecture
  @[hott] def is_spectrum_of_local (X : gen_prespectrum +ℕ) (Hyp : is_equiv (λ (f : prespectrification (psp_sphere) →ₛ X), f ∘ₛ to_prespectrification (psp_sphere))) : is_spectrum X :=
  begin
    fapply is_spectrum.mk,
    exact sorry
  end
  ...
  open chain_complex
  @[hott] def spectrify_type_term {N : succ_str} (X : gen_prespectrum N) (n : N) (k : ℕ) : Type* :=
  Ω[k] (X (n +' k))

  @[hott] def spectrify_type_fun' {N : succ_str} (X : gen_prespectrum N) (n : N) (k : ℕ) :
    Ω[k] (X n) →* Ω[k+1] (X (S n)) :=
  ...
  @[hott] def spectrify_type_fun_zero {N : succ_str} (X : gen_prespectrum N) (n : N) :
    spectrify_type_fun X n 0 ~* glue X n :=
  !pid_pcompose

  @[hott] def spectrify_type {N : succ_str} (X : gen_prespectrum N) (n : N) : Type* :=
  ...
  @[hott] def spectrify_type_fun'_succ {N : succ_str} (X : gen_prespectrum N) (n : N) (k : ℕ) :
    spectrify_type_fun' X n (succ k) ~* Ω→ (spectrify_type_fun' X n k) :=
  begin
    refine !ap1_pcompose⁻¹*
  end

  @[hott] def spectrify_pequiv {N : succ_str} (X : gen_prespectrum N) (n : N) :
  ...
  @[hott] def spectrify {N : succ_str} (X : gen_prespectrum N) : gen_spectrum N :=
  spectrum.MK (spectrify_type X) (spectrify_pequiv X)

  @[hott] def spectrify_map {N : succ_str} {X : gen_prespectrum N} : X →ₛ spectrify X :=
  begin
    fapply smap.mk,
  ...
  @[hott] def spectrify.elim_n {N : succ_str} {X : gen_prespectrum N} {Y : gen_spectrum N}
  ...
  @[hott] def spectrify.elim {N : succ_str} {X : gen_prespectrum N} {Y : gen_spectrum N}
  ...
  @[hott] def phomotopy_spectrify.elim {N : succ_str} {X : gen_prespectrum N} {Y : gen_spectrum N}
  ...
  @[hott] def spectrify_fun {N : succ_str} {X Y : gen_prespectrum N} (f : X →ₛ Y) : spectrify X →ₛ spectrify Y :=
