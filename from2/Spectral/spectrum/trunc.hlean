...
@[hott] def ptrunc_maxm2_change_int {k l : ℤ} (p : k = l) (X : Type*)
  : ptrunc (maxm2 k) X ≃* ptrunc (maxm2 l) X :=
ptrunc_change_index (ap maxm2 p) X

@[hott] def is_trunc_maxm2_change_int {k l : ℤ} (X : pType) (p : k = l)
  : is_trunc (maxm2 k) X → is_trunc (maxm2 l) X :=
by induction p; exact id

@[hott] def is_trunc_maxm2_loop {k : ℤ} {A : Type*} (H : is_trunc (maxm2 (k+1)) A) :
...
@[hott] def loop_ptrunc_maxm2_pequiv {k : ℤ} {l : ℕ₋₂} (p : maxm2 (k+1) = l) (X : Type*) :
...
@[hott] def loop_ptrunc_maxm2_pequiv_ptrunc_elim' {k : ℤ} {l : ℕ₋₂} (p : maxm2 (k+1) = l)
...
@[hott] def loop_ptrunc_maxm2_pequiv_ptr {k : ℤ} {l : ℕ₋₂} (p : maxm2 (k+1) = l) (A : Type*) :
...
@[hott] def is_trunc_of_is_trunc_maxm2 (k : ℤ) (X : Type _)
  : is_trunc (maxm2 k) X → is_trunc (max0 k) X :=
λ H, @is_trunc_of_le X _ _ (maxm2_le_maxm0 k) H

@[hott] def ptrunc_maxm2_pred {n m : ℤ} (A : Type*) (p : n - 1 = m) :
...
@[hott] def ptrunc_maxm2_pred_nat {n : ℕ} {m l : ℤ} (A : Type*)
...
@[hott] def is_strunc [reducible] (k : ℤ) (E : spectrum) : Type _ :=
Π (n : ℤ), is_trunc (maxm2 (k + n)) (E n)

@[hott] def is_strunc_change_int {k l : ℤ} (E : spectrum) (p : k = l)
  : is_strunc k E → is_strunc l E :=
begin induction p, exact id end

@[hott] def is_strunc_of_le {k l : ℤ} (E : spectrum) (H : k ≤ l)
  : is_strunc k E → is_strunc l E :=
begin
...
@[hott] def is_strunc_pequiv_closed {k : ℤ} {E F : spectrum} (H : Πn, E n ≃* F n)
  (H2 : is_strunc k E) : is_strunc k F :=
λn, is_trunc_equiv_closed (maxm2 (k + n)) (H n)

@[hott] def is_strunc_sunit (n : ℤ) : is_strunc n sunit :=
begin
  intro k, apply is_trunc_lift, apply is_trunc_unit
end

open option
@[hott] def is_strunc_add_point_spectrum {X : Type _} {Y : X → spectrum} {s₀ : ℤ}
...
@[hott] def is_strunc_EM_spectrum (G : AbGroup)
...
@[hott] def trivial_shomotopy_group_of_is_strunc (E : spectrum)
...
@[hott] def strunc (k : ℤ) (E : spectrum) : spectrum :=
...
@[hott] def strunc_change_int {k l : ℤ} (E : spectrum) (p : k = l) :
  strunc k E →ₛ strunc l E :=
begin induction p, reflexivity end

@[hott] def is_strunc_strunc (k : ℤ) (E : spectrum)
  : is_strunc k (strunc k E) :=
λ n, is_trunc_trunc (maxm2 (k + n)) (E n)

@[hott] def is_strunc_strunc_pred (X : spectrum) (k : ℤ) : is_strunc k (strunc (k - 1) X) :=
λn, @(is_trunc_of_le _ (maxm2_monotone (add_le_add_right (sub_one_le k) n))) !is_strunc_strunc

@[hott] def is_strunc_strunc_of_is_strunc (k : ℤ) {l : ℤ} {E : spectrum} (H : is_strunc l E)
  : is_strunc l (strunc k E) :=
λ n, !is_trunc_trunc_of_is_trunc

@[hott] def str (k : ℤ) (E : spectrum) : E →ₛ strunc k E :=
...
@[hott] def strunc_elim {k : ℤ} {E F : spectrum} (f : E →ₛ F)
...
@[hott] def strunc_functor (k : ℤ) {E F : spectrum} (f : E →ₛ F) :
...
@[hott] def genspectrum_of_truncspectrum [coercion] (n : ℤ) : n-spectrum → gen_spectrum +ℤ :=
...
section
  ...
  @[hott] def is_conn_maxm1_of_maxm2 (A : Type*) (n : ℤ)
  ...
  @[hott] def is_trunc_maxm2_of_maxm1 (A : Type*) (n : ℤ)
  ...
  @[hott] def is_trunc_maxm2_ppi (k l : ℤ) (H3 : l ≤ n+1+k) (P : A → Type*)
  ...
  @[hott] def is_strunc_spi_of_is_conn (k l : ℤ) (H3 : l ≤ n+1+k) (P : A → spectrum)
...
@[hott] def is_strunc_spi_of_le {A : Type*} (k n : ℤ) (H : n ≤ k) (P : A → spectrum)
...
@[hott] def is_strunc_spi {A : Type*} (n : ℤ) (P : A → spectrum) (H : Πa, is_strunc n (P a))
  : is_strunc n (spi A P) :=
is_strunc_spi_of_le n n !le.refl P H

@[hott] def is_strunc_sp_cotensor (n : ℤ) (A : Type*) {Y : spectrum} (H : is_strunc n Y)
  : is_strunc n (sp_cotensor A Y) :=
is_strunc_pequiv_closed (λn, !pppi_pequiv_ppmap) (is_strunc_spi n (λa, Y) (λa, H))

@[hott] def is_strunc_sp_ucotensor (n : ℤ) (A : Type _) {Y : spectrum} (H : is_strunc n Y)
